...
def _function_has_overlap_plan(func) -> bool:
    return (
        isinstance(func, PrimFunc)
        and func.attrs is not None
        and func.attrs.get("tl.overlap_plan") is not None
    )


def _module_has_overlap_plan(mod: IRModule) -> bool:
    """Whether any PrimFunc carries a typed OverlapPlan."""

    return any(_function_has_overlap_plan(func) for _, func in mod.functions.items())
...
def CUDAPassPipelineBodyPrologue(mod: IRModule, target: Target) -> IRModule:
    ...
    # OverlapPlan planner, if installed, attaches a typed schedule here.
    try:
        from OverlapPlaner.planner import apply_active_schedule as apply_overlap_plan
    except ImportError:

        def apply_overlap_plan(mod: IRModule, target: Target) -> IRModule:
            del target
            return mod

    mod = apply_overlap_plan(mod, target)
    # print(mod)
    mod = tilelang.transform.LowerOverlapPlan()(mod)
    # print(mod)
    ...
    # Functions that already carry a lowered OverlapPlan are skipped inside
    # the pass.
    ...
    mod = tilelang.transform.Simplify()(mod)
    mod = tilelang.transform.UnrollFragmentPingPong()(mod)
    mod = tilelang.transform.Simplify()(mod)
...
def CUDAPassPipelineBody(mod: IRModule, target: Target) -> IRModule:
    ...
    has_overlap_plan = _module_has_overlap_plan(mod)
    mod = tilelang.transform.FinalizeOverlapPlan()(mod)
    ...
    enable_aggressive_merge = should_enable_aggressive_merge(
    ...
    if allow_warp_specialized(pass_ctx=pass_ctx, target=target) and not has_overlap_plan:

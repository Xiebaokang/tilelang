...
def select_wgmma_inst_n(warp_col_tiles: int) -> int:
    """Select the widest legal WGMMA N that exactly tiles the N extent.

    Hopper WGMMA accepts N in [8, 256] in multiples of 8.  Using
    ``gcd(warp_col_tiles, 256)`` unnecessarily decomposes legal widths such as
    96, 112, 160, 176, and 192 into much narrower instructions.

    Keep the existing selection for N % 16 == 8 because those layouts are not
    generally correct when more than one N atom is emitted, and integer WGMMA
    does not instantiate every such width.
    """
    fallback = gcd(warp_col_tiles, 256)
    if warp_col_tiles % 16 != 0:
        return fallback
    for candidate in range(256, fallback, -16):
        if warp_col_tiles % candidate == 0:
            return candidate
    return fallback

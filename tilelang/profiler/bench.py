...
def do_bench_iterations(
    ...
    """Benchmark a CUDA function with fixed warmup and measured iterations.

    Unlike :func:`do_bench`, ``warmup`` and ``rep`` are iteration counts.  No
    runtime-estimation launches are inserted, so the function is invoked
    exactly ``warmup + rep`` times.
    """
    ...
    if device_idx is not None:
        with torch.cuda.device(device_idx):
            return _do_bench_iterations_impl(
                fn,
                warmup=warmup,
                rep=rep,
                quantiles=quantiles,
...
def _make_cache(
    device_idx: int | None, cache_size: int, fast_flush: bool
) -> torch.Tensor:
    ...
    return torch.empty(
        cache_numel, dtype=cache_dtype, device=_cache_device(device_idx)
    )


def _do_bench_iterations_impl(
    fn: Callable,
    *,
    warmup: int,
    rep: int,
    ...
    cache = _make_cache(device_idx, cache_size, fast_flush)
    for _ in range(warmup):
        fn()
    return _bench_with_cuda_events(
        fn, cache, rep, quantiles, return_mode, device_idx
    )

...
def LowerOverlapPlan():
    """Validate and materialize a typed OverlapPlan.

    The pass reads ``tl.overlap_plan``, matches operation/buffer ObjectRefs
    to the input TIR, then lowers group guards, planned buffer storage, and
    synchronization channels. ``LayoutInference`` and ``LowerTileOp``
    subsequently consume each group scope as a local thread domain.
    """

    return _ffi_api.LowerOverlapPlan()  # type: ignore


def FinalizeOverlapPlan():
    """Remove transient OverlapPlan metadata after tile-op lowering."""

    return _ffi_api.FinalizeOverlapPlan()  # type: ignore


def UnrollFragmentPingPong():
    """Unroll fragment ping-pong IfThenElse dispatch by V after software pipeline.

    LowerOverlapPlan emits uniform ``if k % V == slot`` across named fragment
    slots. This pass expands the remaining kernel loop into V straight-line
    copies so Simplify can fold the branches. The unroll factor is V, not K.
    """

    return _ffi_api.UnrollFragmentPingPong()  # type: ignore

...
def has_internal_prim_func(func: Callable[_P, _T]) -> bool:
    ...
    for item in ast.walk(tree):
        if isinstance(item, ast.FunctionDef):
            ...
            for decor in decors:
                target = decor.func if isinstance(decor, ast.Call) else decor
                if isinstance(target, ast.Attribute) and target.attr == "prim_func":
                    from tilelang.language.eager import prim_func

                    if _try_eval(target, nonlocals, func.__globals__) is prim_func:
                        return True
    return False

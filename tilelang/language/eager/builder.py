...
def _patch_prim_func_attrs(pf: PrimFunc, builder: Builder) -> PrimFunc:
    ...
    if builder.func_auto_overlap:
        pf = pf.with_attr("tl.auto_overlap", IntImm("int32", 1))
    return pf
...
class TirTemplate(Generic[_P, _T]):
    ...
    def get_tir(self, tensor_args, given_tensor_args, kwargs):
        ...
        builder.func_auto_overlap = self.auto_overlap
...
class JITFunc(Generic[_P, _T]):
    ...
    def _build_tir_template(self, *args, **kwargs) -> TirTemplate[_P, _T]:
        ...
        if self.mode == "lazy":
            ...
            template = TirTemplate.create(self.orig_func.__name__, pf, builder.constexpr_var, self.ir_gen)
            template.auto_overlap = self.auto_overlap
            return template
        else:
...
def prim_func(
    ...
    *,
    ...
    """Build a PrimFunc from a TileLang kernel.

    Parameters
    ----------
    auto_overlap : bool
        Request kernel-wide OverlapPlan lowering. Group assignment applies to
        the whole kernel, so this is a PrimFunc flag rather than a pipeline-loop
        annotation.
    """
    ...
    def impl(func: Callable[_P, _T]) -> PrimFunc[_P, _T] | Callable[_P, PrimFunc[_P, _T]]:
        ...
        if eager_jit:
            ...
            return JITFunc(
                ...
                tensor_args_defaults,
                ir_gen,
                auto_overlap=bool(auto_overlap),
            )
        else:
            try:
                builder = Builder()
                builder.func_auto_overlap = bool(auto_overlap)

...
def Pipelined(
    ...
    Kernel-wide OverlapPlan lowering is requested with
    ``@T.prim_func(auto_overlap=True)``, not on this loop. Group assignment
    applies to the whole kernel. Any ``T.Pipelined`` loop is a pipeline region
    for OverlapPlan staging; ``num_stages`` is not required and is ignored
    when ``auto_overlap`` is set.

...
public:
  ...
  void SetSmemOffsetMap(Map<String, IntImm> smem_offset_map) {
    smem_offset_map_ = std::move(smem_offset_map);
  }

  void SetSmemPlannedArenaBytes(Integer planned_arena_bytes) {
    smem_planned_arena_bytes_ = std::move(planned_arena_bytes);
  }
...
private:
  ...
  Map<String, IntImm> smem_offset_map_;
  Optional<Integer> smem_planned_arena_bytes_;
  ...
  tirx::Stmt SplitDeviceFunc(tirx::Stmt body, tvm::Target device_target) {
    ...
    if (!smem_offset_map_.empty()) {
      device_attrs.Set(tl::kSmemOffsetMap, smem_offset_map_);
    }
    if (smem_planned_arena_bytes_.defined()) {
      device_attrs.Set(tl::kSmemPlannedArenaBytes,
                       smem_planned_arena_bytes_.value());
    }
    if (code_block_source_) {
...
tirx::PrimFunc SplitHostDevice(tirx::PrimFunc func, IRModule *device_mod,
  ...
  if (auto opt = func->GetAttr<Map<String, IntImm>>(tl::kSmemOffsetMap)) {
    splitter.SetSmemOffsetMap(opt.value());
    func = tvm::WithoutAttr(std::move(func), tl::kSmemOffsetMap);
  }
  if (auto opt = func->GetAttr<Integer>(tl::kSmemPlannedArenaBytes)) {
    splitter.SetSmemPlannedArenaBytes(opt.value());
    func = tvm::WithoutAttr(std::move(func), tl::kSmemPlannedArenaBytes);
  }

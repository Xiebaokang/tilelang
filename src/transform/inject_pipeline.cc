...
Map<Buffer, Buffer> ExpandPipelineBarriers(
    ...
    const VarSet &planned_barrier_data_vars) {
  if (num_stages <= 1)
    return {};
  ...
  for (const Buffer &buf : all_referenced) {
    if (planned_barrier_data_vars.count(buf->data)) {
      continue;
    }
 ...
 *
 * For each TMA copy: convert tl.tileop.copy to tl.tileop.tma_copy with an
 * independent logical barrier.  ExpandPipelineBarriers later gives every
 * logical barrier one slot per physical pipeline stage.
 *
 * Prepend mbarrier_wait_parity to blocks that actually read a buffer written
 * by the TMA copy.  In particular, a TMA producer and its first consumer may
 * legally have the same software-pipeline stage, so stage number alone is not
 * a valid way to locate the wait.
 *
 ...
 */
class ExternallyManagedTmaDetector : public StmtExprVisitor {
public:
  static bool Check(const Stmt &stmt) {
    ExternallyManagedTmaDetector detector;
    detector(stmt);
...
private:
  void VisitExpr_(const CallNode *op) final {
    static const Op &tma_copy_op = Op::Get("tl.tileop.tma_copy");
    if (op->op.same_as(tma_copy_op) && op->annotations.count("barrier")) {
      found_ = true;
      return;
    }
    StmtExprVisitor::VisitExpr_(op);
...
Buffer RewritePipelineTmaBarriers(
  ...
  for (size_t i = 0; i < original_order.size(); ++i) {
    if (!is_zero(tma_copies[i]) &&
        !ExternallyManagedTmaDetector::Check(original_order[i]->body)) {
      tma_indices.push_back(i);
    }
  }
  // LowerOverlapPlan attaches the authoritative overlap mbarrier
  // directly to its TMA copies.  Those copies must not acquire a second
  // pipeline_mbar here: its waits would have no matching transaction arrival.
  if (tma_indices.empty()) {
    return Buffer();
  }

  // Keep the logical barriers in one allocation.  Barrier `t` belongs only
  // to TMA producer `tma_indices[t]`; the later expansion lays the allocation
  // out as [pipeline_stage][logical_tma_barrier].
  Buffer barrier_buf = CreateMBarrierBuffer(
      "pipeline_mbar", static_cast<int>(tma_indices.size()));
  ...
  // Record the real data consumers before replacing any blocks.  A wait is
  // attached to every reading block; repeated waits for the same completed
  // phase are safe and retain correctness when more than one independent
  // consumer reads the TMA result.
  std::vector<std::vector<int>> waits_before(original_order.size());
  for (size_t barrier_id = 0; barrier_id < tma_indices.size(); ++barrier_id) {
    size_t producer_idx = tma_indices[barrier_id];
    const SBlock &producer = original_order[producer_idx];
    BufferSet produced_buffers;
    for (const BufferRegion &write : producer->writes) {
      produced_buffers.insert(write->buffer);
    }
    for (size_t consumer_idx = 0; consumer_idx < original_order.size();
         ++consumer_idx) {
      if (consumer_idx == producer_idx) {
        continue;
      }
      bool reads_tma_result = false;
      for (const BufferRegion &read : original_order[consumer_idx]->reads) {
        if (produced_buffers.count(read->buffer)) {
          reads_tma_result = true;
          break;
        }
      }
      if (reads_tma_result) {
        waits_before[consumer_idx].push_back(static_cast<int>(barrier_id));
      }
    }
  }

  // Phase 1: each TMA producer owns and arrives on its logical barrier.
  for (size_t barrier_id = 0; barrier_id < tma_indices.size(); ++barrier_id) {
    size_t producer_idx = tma_indices[barrier_id];
    SBlock old_block = original_order[producer_idx];
    CopyToTmaCopyRewriter rewriter(barrier_buf,
                                   /*barrier_id=*/IntImm(DataType::Int(32),
                                                        barrier_id),
                                   /*emit_arrive=*/true);
    Stmt new_body = rewriter(old_block->body);
    ...
    pipeline_info.erase(old_block);
    pipeline_info.emplace(new_block, anno);
    original_order.Set(producer_idx, new_block);
  }

  // Phase 2: wait immediately before each actual TMA-data consumer.  This
  // covers same-stage consumers as well as cross-stage consumers.
  for (size_t i = 0; i < original_order.size(); ++i) {
    if (waits_before[i].empty()) {
      continue;
    }
    SBlock old_block = original_order[i];
    Array<Stmt> wait_stmts;
    for (int barrier_id : waits_before[i]) {
      PrimExpr barrier_ref =
          MakeBarrierRef(barrier_buf, IntImm(DataType::Int(32), barrier_id));
      PrimExpr ns = IntImm(DataType::Int(32), num_stages);
...
private:
  ...
  bool HasNonIdentityOrder(const Array<SBlock> &scheduled_order,
                           const PipelineInfo &pipeline_info) const {
    for (size_t i = 0; i < scheduled_order.size(); ++i) {
      if (pipeline_info.at(scheduled_order[i]).order != static_cast<int>(i)) {
        return true;
      }
    }
    return false;
  ...
  Stmt VisitStmt_(const ForNode *op) final {
    ...
    if (auto planned_version_buffers =
            for_node->annotations.Get(kPipelinePlannedVersionBuffers)) {
      VarSet planned_data_vars;
      for (const Var &data_var :
           Downcast<Array<Var>>(planned_version_buffers.value())) {
        planned_data_vars.insert(data_var);
      }
      Array<Buffer> automatically_versioned_allocs;
      for (const Buffer &buffer : pipeline_allocs) {
        if (!planned_data_vars.count(buffer->data)) {
          automatically_versioned_allocs.push_back(buffer);
        }
      }
      pipeline_allocs = std::move(automatically_versioned_allocs);
    }
    ...
    // A single-stage schedule has no cross-iteration overlap, but it can still
    // carry a meaningful within-iteration issue order.  Preserve the old
    // no-op fast path only when that order already matches the source order;
    // otherwise PipelineRewriter performs the requested reorder without
    // expanding the loop across iterations.
    if (!HasOverlappableStages(pipeline_info)) {
      if (!HasNonIdentityOrder(scheduled_order, pipeline_info)) {
      ...
      // Stage numbers are relative.  Once every operation has the same stage,
      // normalize it to zero so the rewriter performs only the requested
      // within-iteration reorder instead of creating a shifted pipeline.
      for (auto &item : pipeline_info) {
        item.second.stage = 0;
      }
    }
    ...
    // each one to use an independent logical pipeline barrier, then insert
    // mbarrier_wait_parity before every block that reads its result.
    ...
    // Must run BEFORE local_allocs is copied from block_local_allocs.
    {
      ...
      VarSet planned_barrier_data_vars;
      if (auto planned_barriers =
              for_node->annotations.Get(kPipelinePlannedBarrierBuffers)) {
        for (const Var &data_var :
             Downcast<Array<Var>>(planned_barriers.value())) {
          planned_barrier_data_vars.insert(data_var);
        }
      }

...
struct PipelineStageInfo {
  ...
  bool has_explicit_barrier_arrive = false;
...
public:
  ...
  bool IsAsyncProducerCandidate(const PipelineStageInfo &pinfo) const {
    if (pinfo.conditional_execution || pinfo.has_explicit_barrier_arrive) {
      return false;
    }
    if (pinfo.IsTmaCopy()) {
      return false;
  ...
  void ClassifyExplicitPlanTmaCopy(const Stmt &stmt,
    ...
    const auto *copy = copy_tile_op.value().as<CopyNode>();
    if (copy == nullptr || !IsGlobalLikeBuffer(copy->src) ||
        !IsSharedBuffer(copy->dst)) {
      return;
    }
    auto selection = cuda::ClassifyWarpSpecializedProducerCopy(*copy, target_);
    pinfo->tma_copy =
        selection.supported && cuda::CopyInstIsTMA(selection.inst);
  }
  ...
  void MaybeAnnotateLegacyAsyncPipelineLoop(const Array<Stmt> &pipeline_stmts,
    ...
    for (size_t i = 0; i < pipeline_stmts.size(); ++i) {
      ...
      ClassifyExplicitPlanTmaCopy(pipeline_stmts[i], &pinfo);
    ...
    EmitTmaCopyAnnotations(pipeline_stage_infos, annotations);
  ...
  PipelineStageInfo MakePipelineStageInfo(Stmt stmt, int idx) {
    ...
    PostOrderVisit(block->body, [&](const ObjectRef &node) {
      const auto *call = node.as<CallNode>();
      if (call != nullptr &&
          call->op.same_as(builtin::ptx_arrive_barrier())) {
        pinfo.has_explicit_barrier_arrive = true;
      }
    });
    ...
    pinfo.tma_copy = pure_copy_stage && !pinfo.conditional_execution &&
                     !pinfo.has_explicit_barrier_arrive &&
...
private:
  ...
  Stmt VisitStmt_(const ForNode *loop) final {
    ...
    MakeStageAnalyzer().EmitTmaCopyAnnotations(pipeline_stage_infos,
                                               &annotations);

...
private:
  ...
  Range CurrentThreadBounds() const {
    if (!group_thread_ranges_.empty()) {
      const PrimExpr &thread_count = group_thread_ranges_.back().second;
      return Range::FromMinExtent(make_const(thread_count.dtype(), 0),
                                  thread_count);
    }
  ...
  PrimExpr CurrentThreadIndex() const {
    if (thread_binding_.defined()) {
      if (!group_thread_ranges_.empty()) {
        return thread_binding_->var - group_thread_ranges_.back().first;
      }
  ...
  std::vector<std::pair<PrimExpr, PrimExpr>> group_thread_ranges_;
...
tvm::transform::Pass LowerTileOp() {
  auto pass_func = [=](PrimFunc f, const IRModule &m, const PassContext &ctx) {
    f = LowerTileOpPass::Substitute(std::move(f));
    return f;
  };

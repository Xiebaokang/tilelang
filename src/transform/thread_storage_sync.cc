  ...
  Stmt ProcessSharedSync(const CallNode *op, const std::string &scope) {
    ...
    if (thread_count % warp_size_ != 0) {
      ...
      LOG(FATAL) << "[ThreadSync] Cannot lower a required shared-memory sync "
                 ...
                 << "), with thread bounds tx=[" << bound_tx->min_value << ", "
                 << bound_tx->max_value << "], ty=[" << bound_ty->min_value
                 << ", " << bound_ty->max_value << "], tz=["
                 << bound_tz->min_value << ", " << bound_tz->max_value << "]. "
...
struct TileLangThreadSyncPlanner : public ConstrVisitor {
  ...
  enum AccessType : uint8_t {
    ...
    kReadAcquire,
    // Completion of TMA writes associated with one mbarrier.
    kTmaAcquire,
    // Completion of all outstanding WGMMA shared-memory reads.
    kWgmmaRelease
  };
  ...
  struct AccessEntry {
    ...
    /*! \brief The mbarrier token associated with a TMA write/acquire event. */
    PrimExpr async_token;
    /*! \brief Whether this shared-memory read feeds an asynchronous WGMMA. */
    bool is_wgmma_read = false;
    ...
    /*! \brief Warp-specialized group owning this access, or -1 outside WS.
     *
     * Cross-group communication inside kWarpSpecializationScope is ordered by
     * explicit mbarriers.  A normal thread barrier inserted into only one
     * branch cannot synchronize another group and is therefore both redundant
     * and misleading.  Same-group accesses still participate in the ordinary
     * ThreadSync dependency analysis.
     */
    int64_t warp_specialized_group = -1;
  };
  ...
  /*! \brief Shared-memory access state represented by one WGMMA descriptor. */
  struct WgmmaDescriptorState {
  ...
  void VisitExpr_(const BufferLoadNode *op) final {
    ...
    if (Enabled(buf.get(), scope)) {
      ...
      e.warp_specialized_group = current_warp_specialized_group_;
      e.buffer = buf;
  ...
  void VisitStmt_(const AttrStmtNode *op) override {
    if (op->attr_key == tvm::tl::attr::coproc_scope) {
      ...
      env_threads_.pop_back();
    } else if (op->attr_key == tvm::tl::attr::kWarpSpecializationScope ||
               op->attr_key ==
                   tvm::tl::attr::kThreadSyncWarpSpecializationScope) {
      const auto *groups = op->node.as<ffi::ArrayObj>();
      ICHECK(groups != nullptr)
          << tvm::tl::attr::kWarpSpecializationScope
          << " expects an array of group thread counts";
      int64_t next_group = 0;
      VisitWarpSpecializedBranches(op->body,
                                   static_cast<int64_t>(groups->size()),
                                   &next_group);
      ICHECK_EQ(next_group, static_cast<int64_t>(groups->size()))
          << "warp-specialized branch count does not match its thread-count "
             "metadata";
    } else {
      ConstrVisitor::VisitStmt_(op);
    }
  ...
  void VisitWarpSpecializedBranches(const Stmt &statement,
                                    int64_t group_count,
                                    int64_t *next_group) {
    // InjectFenceProxy may hoist fence.proxy.async (and the WS pass may insert
    // a block-wide syncthreads) as a SeqStmt prefix of the producer/consumer
    // if.  Those statements run on every thread and must not consume a group.
    if (const auto *seq = statement.as<SeqStmtNode>()) {
      ICHECK(!seq->seq.empty());
      for (int i = 0; i + 1 < static_cast<int>(seq->seq.size()); ++i) {
        VisitStmt(seq->seq[i]);
      }
      VisitWarpSpecializedBranches(seq->seq.back(), group_count, next_group);
      return;
    }
    ICHECK_LT(*next_group, group_count);
    const auto *branch = statement.as<IfThenElseNode>();
    if (branch == nullptr) {
      int64_t previous_group = current_warp_specialized_group_;
      current_warp_specialized_group_ = (*next_group)++;
      VisitStmt(statement);
      current_warp_specialized_group_ = previous_group;
      return;
    }

    {
      auto guard = MakeGuard(branch->condition);
      int64_t previous_group = current_warp_specialized_group_;
      current_warp_specialized_group_ = (*next_group)++;
    ...
    ICHECK(branch->else_case.defined())
        << "warp-specialized scope is missing a group branch";
    auto guard = MakeGuard(tirx::Not(branch->condition));
    if (*next_group + 1 == group_count) {
      int64_t previous_group = current_warp_specialized_group_;
      current_warp_specialized_group_ = (*next_group)++;
      VisitStmt(branch->else_case.value());
      current_warp_specialized_group_ = previous_group;
      return;
    }
    VisitWarpSpecializedBranches(branch->else_case.value(), group_count,
                                 next_group);
  }
  ...
  void VisitStmt_(const ForNode *op) final {
    ...
    scope_.pop_back();
    if (!s.access.empty()) {
      PrimExpr last_iteration =
          op->min + op->extent - make_const(op->extent.dtype(), 1);
      Map<Var, PrimExpr> last_iteration_substitution = {
          {op->loop_var, last_iteration}};
      for (AccessEntry &e : s.access) {
        if (e.async_token.defined()) {
          e.async_token =
              Substitute(e.async_token, last_iteration_substitution);
        }
      }
  ...
  void VisitExpr_(const CallNode *op) final {
    ...
    if (is_tma_load) {
      ...
      PrimExpr previous_tma_token = current_tma_token_;
      current_tma_token_ = op->args[1];
      tma_depth_++;
      for (const auto &a : op->args) {
        this->VisitExpr(a);
      }
      tma_depth_--;
      current_tma_token_ = std::move(previous_tma_token);
      return;
    }

    auto get_descriptor_var = [](const PrimExpr &expr) -> Optional<Var> {
    ...
    // Descriptor initialization identifies a shared-memory operand, but does
    // not itself issue the asynchronous WGMMA read. Save the access under the
    // descriptor variable and materialize it at the ptx_wgmma_* call below.
    if (op->op.same_as(tl::initialize_wgmma_descriptor())) {
      ICHECK_EQ(op->args.size(), 5U);
      this->VisitExpr(op->args[0]);
      size_t access_begin = curr_stmt_.access.size();
      this->VisitExpr(op->args[1]);
      Optional<Var> descriptor_var = get_descriptor_var(op->args[0]);
      if (descriptor_var.defined()) {
        std::vector<AccessEntry> descriptor_reads(
            curr_stmt_.access.begin() + access_begin, curr_stmt_.access.end());
        for (AccessEntry &read : descriptor_reads) {
          read.is_wgmma_read = true;
        }
        curr_stmt_.access.erase(curr_stmt_.access.begin() + access_begin,
                                curr_stmt_.access.end());
        WgmmaDescriptorState state;
        state.reads = std::move(descriptor_reads);
        state.byte_offset = IntImm(DataType::Int(32), 0);
        wgmma_descriptor_states_[descriptor_var.value()] = std::move(state);
      }
    ...
    if (op->op.same_as(tl::increase_descriptor_offset())) {
      ...
      Optional<Var> descriptor_var = get_descriptor_var(op->args[0]);
      if (descriptor_var.defined()) {
        auto it = wgmma_descriptor_states_.find(descriptor_var.value());
        if (it != wgmma_descriptor_states_.end()) {
          PrimExpr byte_offset = op->args[1];
          if (byte_offset.dtype() != it->second.byte_offset.dtype()) {
            byte_offset = Cast(it->second.byte_offset.dtype(), byte_offset);
          }
          it->second.byte_offset =
              it->second.byte_offset + std::move(byte_offset);
        }
      }
      ConstrVisitor::VisitExpr_(op);
      return;
    ...
    auto parse_wgmma_shape = [](const PrimExpr &shape_expr) {
      const auto *shape_imm = shape_expr.as<StringImmNode>();
      ICHECK(shape_imm != nullptr) << "Expected a static WGMMA shape";
      const std::string &shape = shape_imm->value;
      ICHECK(!shape.empty() && shape[0] == 'm');
      size_t n_pos = shape.find('n', 1);
      size_t k_pos = shape.find('k', n_pos + 1);
      ICHECK(n_pos != std::string::npos && k_pos != std::string::npos)
          << "Invalid WGMMA shape: " << shape;
      return std::vector<int>{std::stoi(shape.substr(1, n_pos - 1)),
                              std::stoi(shape.substr(n_pos + 1,
                                                     k_pos - n_pos - 1)),
                              std::stoi(shape.substr(k_pos + 1))};
    };
    auto append_wgmma_descriptor_reads = [&](const PrimExpr &descriptor,
                                             const PrimExpr &call_offset,
                                             int access_elements) {
      Optional<Var> descriptor_var = get_descriptor_var(descriptor);
      if (!descriptor_var.defined()) {
        return;
      }
      auto it = wgmma_descriptor_states_.find(descriptor_var.value());
      ...
      for (AccessEntry read : it->second.reads) {
        int element_bytes = read.dtype.bytes() * read.dtype.lanes();
        ICHECK_GT(element_bytes, 0);
        ICHECK(call_offset.dtype().is_int() || call_offset.dtype().is_uint())
            << "Expected an integer WGMMA descriptor offset, but got "
            << call_offset << " from descriptor " << descriptor;
        // ptx_wgmma_* offsets are encoded in 16-byte descriptor units.  The
        // macro generator commonly expresses them as shift_right(bytes, 4),
        // which EvalSet cannot bound.  Normalize that form to floordiv before
        // constructing the shared-memory byte interval.
        PrimExpr call_byte_offset;
        if (const auto *shift = call_offset.as<CallNode>();
            shift != nullptr && shift->op.same_as(builtin::shift_right()) &&
            shift->args.size() == 2U) {
          const auto *bits = shift->args[1].as<IntImmNode>();
          ICHECK(bits != nullptr && bits->value >= 0 && bits->value < 31);
          int64_t divisor = int64_t{1} << bits->value;
          call_byte_offset =
              indexdiv(shift->args[0],
                       make_const(shift->args[0].dtype(), divisor)) *
              make_const(shift->args[0].dtype(), 16);
        } else {
          call_byte_offset =
              call_offset * make_const(call_offset.dtype(), 16);
        }
        if (call_byte_offset.dtype() != it->second.byte_offset.dtype()) {
          call_byte_offset =
              Cast(it->second.byte_offset.dtype(), call_byte_offset);
        }
        PrimExpr byte_offset = it->second.byte_offset + call_byte_offset;
        PrimExpr element_offset = indexdiv(
            byte_offset,
            make_const(it->second.byte_offset.dtype(), element_bytes));
        Array<arith::IntSet> shifted_touched;
        shifted_touched.reserve(read.touched.size());
        for (const arith::IntSet &touched : read.touched) {
          PrimExpr offset = element_offset;
          if (offset.dtype() != touched.min().dtype()) {
            offset = Cast(touched.min().dtype(), offset);
          }
          PrimExpr access_begin = touched.min() + offset;
          PrimExpr access_end = access_begin +
                                make_const(access_begin.dtype(),
                                           access_elements - 1);
          shifted_touched.push_back(
              arith::IntSet::Interval(access_begin, access_end));
        }
        read.touched = std::move(shifted_touched);
        curr_stmt_.access.emplace_back(std::move(read));
      }
    };
    if (op->op.same_as(tl::ptx_wgmma_ss())) {
      ICHECK_EQ(op->args.size(), 15U);
      std::vector<int> shape = parse_wgmma_shape(op->args[0]);
      append_wgmma_descriptor_reads(op->args[6], op->args[7],
                                    shape[0] * shape[2]);
      append_wgmma_descriptor_reads(op->args[8], op->args[9],
                                    shape[1] * shape[2]);
      ConstrVisitor::VisitExpr_(op);
      return;
    }
    if (op->op.same_as(tl::ptx_wgmma_rs())) {
      ICHECK_EQ(op->args.size(), 14U);
      std::vector<int> shape = parse_wgmma_shape(op->args[0]);
      append_wgmma_descriptor_reads(op->args[7], op->args[8],
                                    shape[1] * shape[2]);
      ConstrVisitor::VisitExpr_(op);
      return;
    }
    if (op->op.same_as(tl::ptx_wgmma_sp_ss())) {
      ICHECK_EQ(op->args.size(), 18U);
      std::vector<int> shape = parse_wgmma_shape(op->args[0]);
      append_wgmma_descriptor_reads(op->args[6], op->args[7],
                                    shape[0] * shape[2]);
      append_wgmma_descriptor_reads(op->args[11], op->args[12],
                                    shape[1] * shape[2]);
      ConstrVisitor::VisitExpr_(op);
      return;
    }
    if (op->op.same_as(tl::ptx_wgmma_sp_rs())) {
      ICHECK_EQ(op->args.size(), 17U);
      std::vector<int> shape = parse_wgmma_shape(op->args[0]);
      append_wgmma_descriptor_reads(op->args[10], op->args[11],
                                    shape[1] * shape[2]);
      ConstrVisitor::VisitExpr_(op);
      return;
    }

    if (op->op.same_as(tl::mbarrier_wait_parity())) {
      ...
      e.type = kTmaAcquire;
      e.scope = sync_scope_;
      e.async_token = op->args[0];
      curr_stmt_.access.emplace_back(std::move(e));
      ConstrVisitor::VisitExpr_(op);
      return;
    }

    auto is_wgmma_wait_zero = [&]() {
      if (!op->op.same_as(tl::warpgroup_wait()) &&
          !op->op.same_as(tl::wait_wgmma())) {
        return false;
      }
      if (op->args.size() != 1U) {
        return false;
      }
      const auto *wait_count = op->args[0].as<IntImmNode>();
      return wait_count != nullptr && wait_count->value == 0;
    }();
    if (is_wgmma_wait_zero) {
      ICHECK(allow_append_);
      AccessEntry e{.cset = {constr_stack_}};
      e.threads = env_threads();
      e.type = kWgmmaRelease;
      e.scope = sync_scope_;
      curr_stmt_.access.emplace_back(std::move(e));
      ConstrVisitor::VisitExpr_(op);
      return;
    ...
    if (op->op.same_as(builtin::address_of())) {
      ...
      if (Enabled(buffer_var, scope)) {
        ...
        if (flag->value & 2) {
          ...
          e.is_async_copy = (tma_depth_ > 0 || cp_async_depth_ > 0);
          if (tma_depth_ > 0) {
            e.async_token = current_tma_token_;
          }
          curr_stmt_.access.emplace_back(e);
        }
      }
  ...
  std::vector<AccessEntry> Summarize(std::vector<StmtEntry> seq,
    ...
    auto tokens_equal = [](const PrimExpr &lhs, const PrimExpr &rhs) {
    ...
    auto apply_completion_event = [&](const AccessEntry &event,
                                      const ForNode *next_iteration_loop) {
      if (event.type == kTmaAcquire) {
        PrimExpr event_token = event.async_token;
        if (next_iteration_loop != nullptr) {
          PrimExpr step = make_const(next_iteration_loop->loop_var.dtype(), 1);
          event_token = Substitute(
              event_token,
              {{next_iteration_loop->loop_var,
                next_iteration_loop->loop_var + step}});
        }
        writes.erase(
            std::remove_if(writes.begin(), writes.end(),
                           [&](const AccessEntry &write) {
                             return write.is_async_copy &&
                                    tokens_equal(write.async_token,
                                                 event_token);
                           }),
            writes.end());
      } else if (event.type == kWgmmaRelease) {
        reads.erase(std::remove_if(reads.begin(), reads.end(),
                                   [](const AccessEntry &read) {
                                     return read.is_wgmma_read;
                                   }),
                    reads.end());
      }
    };
    ...
    for (size_t i = 0; i < seq.size(); ++i) {
      ...
      for (const AccessEntry &acc : s.access) {
        if (acc.type == kRead) {
          ...
          reads.clear();
          writes.clear();
        } else if (acc.type == kTmaAcquire ||
                   acc.type == kWgmmaRelease) {
          apply_completion_event(acc, nullptr);
        }
      }
        ...
        for (const AccessEntry &acc : s.access) {
          if (acc.type == kRead) {
            ...
            writes.clear();
                     ...
                     acc.type == kWgmmaRelease) {
            apply_completion_event(acc, loop);
          }
        }
        if (need_loop_sync) {
          if (!has_read_in_scope) {
...
private:
  ...
  // Current branch of kWarpSpecializationScope, or -1 outside that scope.
  int64_t current_warp_specialized_group_{-1};
  // Nesting depth of tma_load/tma_load_im2col calls
  int tma_depth_{0};
  // The mbarrier operand for the TMA load currently being visited.
  PrimExpr current_tma_token_;
  ...
  // Shared-memory access state represented by initialized WGMMA descriptors.
  std::unordered_map<Var, WgmmaDescriptorState, ObjectPtrHash, ObjectPtrEqual>
      wgmma_descriptor_states_;
  ...
  bool TouchedRangesAreDisjoint(const AccessEntry &lhs,
    ...
    bool same_access_type =
        (lhs.type == kWrite && rhs.type == kWrite) ||
        (lhs.type == kRead && rhs.type == kRead);
    ...
    if (loop != nullptr) {
      ...
      Map<Var, PrimExpr> loop_shift = {
          {loop->loop_var, loop->loop_var + step}};
      rhs_min = Substitute(rhs_min, loop_shift);
      rhs_max = Substitute(rhs_max, loop_shift);
      curr_cset = curr_cset.Substitute(loop_shift);
    ...
    for (unsigned idx = 0; idx != 3; ++idx) {
      ...
      Var curr_var = same_access_type
                         ? prev_var
                         : Var(info.name_curr, old_curr_var.dtype());
    ...
    if (analyzer.CanProve(tirx::Or(lhs_max < rhs_min, rhs_max < lhs_min))) {
      return true;
    }

    // Small statically bounded loops are common around stmatrix stores.  Their
    // addresses contain several floordiv/floormod terms, which the symbolic
    // bound prover often cannot compare even though every adjacent iteration
    // writes a disjoint tile.  Check each loop-carried pair directly before
    // falling back to the conservative overlap result.
    if (loop != nullptr) {
      const auto *loop_min = loop->min.as<IntImmNode>();
      const auto *loop_extent = loop->extent.as<IntImmNode>();
      constexpr int64_t kMaxEnumeratedLoopExtent = 64;
      if (loop_min != nullptr && loop_extent != nullptr &&
          loop_extent->value > 1 &&
          loop_extent->value <= kMaxEnumeratedLoopExtent) {
        bool disjoint_for_every_iteration = true;
        for (int64_t offset = 0; offset + 1 < loop_extent->value; ++offset) {
          PrimExpr iteration = make_const(
              loop->loop_var.dtype(), loop_min->value + offset);
          Map<Var, PrimExpr> substitution = {{loop->loop_var, iteration}};
          arith::Analyzer iteration_analyzer;
          PrimExpr iteration_lhs_min = iteration_analyzer.Simplify(
              Substitute(lhs_min, substitution));
          PrimExpr iteration_lhs_max = iteration_analyzer.Simplify(
              Substitute(lhs_max, substitution));
          PrimExpr iteration_rhs_min = iteration_analyzer.Simplify(
              Substitute(rhs_min, substitution));
          PrimExpr iteration_rhs_max = iteration_analyzer.Simplify(
              Substitute(rhs_max, substitution));
          bool disjoint = iteration_analyzer.CanProve(tirx::Or(
              iteration_lhs_max < iteration_rhs_min,
              iteration_rhs_max < iteration_lhs_min));
          if (!disjoint) {
            disjoint_for_every_iteration = false;
            break;
          }
        }
        if (disjoint_for_every_iteration) {
          return true;
        }
      }
    }

    // Circular buffers commonly use complementary expressions such as
    // k % 2 and (k + 1) % 2.  The symbolic-bound prover does not establish
    // their disjointness directly.  Prove it soundly by checking every
    // residue of one small modulo variable.
    std::unordered_map<const VarNode *, std::pair<Var, int64_t>> modulo_vars;
    auto collect_modulo_vars = [&](const PrimExpr &expr) {
      PostOrderVisit(expr, [&](const ObjectRef &node) {
        const auto *floor_mod = node.as<FloorModNode>();
        if (floor_mod == nullptr) {
          return;
        }
        const auto *modulus = floor_mod->b.as<IntImmNode>();
        if (modulus == nullptr || modulus->value <= 1 || modulus->value > 8) {
          return;
        }
        PostOrderVisit(floor_mod->a, [&](const ObjectRef &operand) {
          if (const auto *var_node = operand.as<VarNode>()) {
            auto [it, inserted] = modulo_vars.emplace(
                var_node,
                std::make_pair(GetRef<Var>(var_node), modulus->value));
            if (!inserted && it->second.second != modulus->value) {
              it->second.second = 0;
            }
          }
        });
    ...
    collect_modulo_vars(lhs_min);
    collect_modulo_vars(lhs_max);
    collect_modulo_vars(rhs_min);
    collect_modulo_vars(rhs_max);
    for (const auto &[_, candidate] : modulo_vars) {
      const Var &modulo_var = candidate.first;
      int64_t modulus = candidate.second;
      if (modulus == 0) {
        continue;
      }
      class SimpleModuloUseChecker : public ExprVisitor {
      public:
        SimpleModuloUseChecker(const VarNode *target, int64_t modulus)
            : target_(target), modulus_(modulus) {}
      ...
      private:
        bool IsTarget(const PrimExpr &expr) const {
          return expr.get() == target_;
        }

        bool IsIntegerConstant(const PrimExpr &expr) const {
          return expr.as<IntImmNode>() != nullptr;
        }

        bool IsSimpleAffineTarget(const PrimExpr &expr) const {
          if (IsTarget(expr)) {
            return true;
          }
          if (const auto *add = expr.as<AddNode>()) {
            return (IsTarget(add->a) && IsIntegerConstant(add->b)) ||
                   (IsIntegerConstant(add->a) && IsTarget(add->b));
          }
          if (const auto *sub = expr.as<SubNode>()) {
            return IsTarget(sub->a) && IsIntegerConstant(sub->b);
          }
          return false;
        }
        ...
        void VisitExpr_(const FloorModNode *op) final {
          const auto *modulus = op->b.as<IntImmNode>();
          if (modulus != nullptr && modulus->value == modulus_ &&
              IsSimpleAffineTarget(op->a)) {
            return;
          }
      ...
      bool is_periodic =
          SimpleModuloUseChecker(modulo_var.get(), modulus).Check(lhs_min) &&
          SimpleModuloUseChecker(modulo_var.get(), modulus).Check(lhs_max) &&
          SimpleModuloUseChecker(modulo_var.get(), modulus).Check(rhs_min) &&
          SimpleModuloUseChecker(modulo_var.get(), modulus).Check(rhs_max);
      if (!is_periodic) {
        continue;
      }
      bool disjoint_for_every_residue = true;
      for (int64_t residue = 0; residue < modulus; ++residue) {
        PrimExpr replacement = make_const(modulo_var.dtype(), residue);
        Map<Var, PrimExpr> substitution = {{modulo_var, replacement}};
        arith::Analyzer residue_analyzer;
        PrimExpr residue_lhs_min =
            residue_analyzer.Simplify(Substitute(lhs_min, substitution));
        PrimExpr residue_lhs_max =
            residue_analyzer.Simplify(Substitute(lhs_max, substitution));
        PrimExpr residue_rhs_min =
            residue_analyzer.Simplify(Substitute(rhs_min, substitution));
        PrimExpr residue_rhs_max =
            residue_analyzer.Simplify(Substitute(rhs_max, substitution));
        bool disjoint = residue_analyzer.CanProve(
                            residue_lhs_max < residue_rhs_min,
                            arith::ProofStrength::kSymbolicBound) ||
                        residue_analyzer.CanProve(
                            residue_rhs_max < residue_lhs_min,
                            arith::ProofStrength::kSymbolicBound);
        if (!disjoint) {
          disjoint_for_every_residue = false;
          break;
        }
      }
      if (disjoint_for_every_residue) {
        return true;
      }
    }
    return false;
  ...
  bool FindConflict(const AccessEntry &prev, const AccessEntry &curr,
                    const ForNode *loop) {
    // Warp-specialized groups execute on disjoint thread domains and use
    // explicit mbarriers for communication. A partial thread barrier emitted
    // into either branch cannot order the other group, so cross-group hazards
    // must not create an automatic ThreadSync barrier here.
    if (prev.warp_specialized_group >= 0 &&
        curr.warp_specialized_group >= 0 &&
        prev.warp_specialized_group != curr.warp_specialized_group) {
      return false;
    }
    ...
    if (prev.is_pointer_access || curr.is_pointer_access) {
      // A tvm_access_ptr access has no structured buffer indices, while a
      // normal BufferLoad/BufferStore does.  Both still carry physical
      // element intervals (including shared-memory alias offsets), so use
      // those intervals for pointer/pointer and mixed pointer/buffer pairs.
      if (TouchedRangesAreDisjoint(prev, curr, loop)) {
        return false;
      }

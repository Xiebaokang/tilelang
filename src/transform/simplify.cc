...
private:
  ...
  Stmt VisitStmt_(const ForNode *op) final {
    // Simplify min/extent/step before assuming the loop is inhabited.
    // IRMutatorWithAnalyzer::VisitStmt_(For) records `extent > 0` and then
    // visits the extent. ContextSingletonFloorDivFolder would then collapse a
    // 0-or-1 symbolic bound such as `blockIdx.y // 4` (y in [0, 8)) to 1,
    // which extra-executes software-pipeline iterations.
    ...
    return constraint_scope_.WithNewScope([&]() -> Stmt {
      ...
      constraint_scope_.Current().Emplace(analyzer_, extent > 0);
      Stmt body = this->VisitStmt(op->body);
      if (min.same_as(op->min) && extent.same_as(op->extent) &&
          body.same_as(op->body) && step.same_as(op->step)) {

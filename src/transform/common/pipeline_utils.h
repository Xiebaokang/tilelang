...
/*! Buffers whose physical version count was fixed by an external schedule. */
static constexpr const char *kPipelinePlannedVersionBuffers =
    "tl_pipeline_planned_version_buffers";
/*! Barrier buffers whose stage slots were fixed by a program schedule. */
static constexpr const char *kPipelinePlannedBarrierBuffers =
    "tl_pipeline_planned_barrier_buffers";
/*! Physical thread interval assigned to one program-schedule group. */
static constexpr const char *kProgramScheduleGroupScope =
    "tl.overlap_plan.group_scope";

/*! \brief Parse `(first_thread, thread_count)` from a group-scope AttrStmt. */
inline std::pair<PrimExpr, PrimExpr>
GetProgramScheduleGroupThreadRange(const AttrStmtNode *attr) {
  ICHECK_EQ(attr->attr_key, kProgramScheduleGroupScope);
  ffi::Array<IntImm> range = Downcast<ffi::Array<IntImm>>(attr->node);
  ICHECK_EQ(range.size(), 2U)
      << kProgramScheduleGroupScope
      << " must carry (first_thread, thread_count)";
  ICHECK_GE(range[0]->value, 0);
  ICHECK_GT(range[1]->value, 0);

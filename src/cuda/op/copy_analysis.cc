...
TVM_FFI_STATIC_INIT_BLOCK() {
  ...
  refl::GlobalDef().def(
      ...
      [](tirx::Stmt stmt, tvm::Target target) -> ffi::String {
        ...
        if (copy == nullptr) {
          return ffi::String("unknown");
        }
        cuda::CopyInstSelection selection =
            cuda::ClassifyWarpSpecializedProducerCopy(*copy, std::move(target));
        if (!selection.supported) {
          return ffi::String("unsupported");
        }
        if (cuda::CopyInstIsTMA(selection.inst)) {
          return ffi::String("tma");
        }
        if (cuda::CopyInstIsCPAsync(selection.inst)) {
          return ffi::String("cp_async");
        }
        return ffi::String("simt");
      });
}

...
// Program-schedule TMA elects one leader inside a warp group, but
// tl_shuffle_elect<N> is block-global: warp_idx % (N/32) == 0 also matches
// later groups (warp 12 in a 12+4 split).  Restrict to local warp 0 when the
// copy asked for a single group-local leader.
PrimExpr MakeTmaLeaderCondition(const CopyNode &op,
                                const LowerArgs &lower_args) {
  PrimExpr elect =
      MakeTmaLeaderCondition(GetLeaderScopeThreads(op, lower_args));
  if (!GetBoolAnnotation(op, "leader_first_warp")) {
    return elect;
  }
  ICHECK(lower_args.thread_index.defined())
      << "T.tma_copy leader_first_warp requires a bound thread index";
  DataType index_dtype = lower_args.thread_index.dtype();
  PrimExpr local_warp =
      FloorDiv(lower_args.thread_index, IntImm(index_dtype, 32));
  return And(std::move(elect), local_warp == IntImm(index_dtype, 0));
}
...
Stmt Copy::LowerBulk(const CopyNode &op, const LowerArgs &lower_args,
  ...
  if (is_load && barrier_base_id >= 0) {
    ...
    Stmt producer = IfThenElse(
        MakeTmaLeaderCondition(op, lower_args),
  ...
  tma_copy = IfThenElse(
      MakeTmaLeaderCondition(op, lower_args), tma_copy);

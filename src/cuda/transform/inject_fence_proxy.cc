  ...
  Stmt VisitStmt_(const AttrStmtNode *op) final {
    ...
    if (op->attr_key == attr::kWarpSpecializationScope) {
      if (const auto *seq = body.as<SeqStmtNode>()) {
        if (!seq->seq.empty() && seq->seq.back().as<IfThenElseNode>()) {
          Array<Stmt> stmts;
          stmts.reserve(seq->seq.size());
          for (int i = 0; i + 1 < static_cast<int>(seq->seq.size()); ++i) {
            stmts.push_back(seq->seq[i]);
          }
          stmts.push_back(AttrStmt(op->node, op->attr_key, value,
                                   seq->seq.back(), op->span));

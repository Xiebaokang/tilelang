...
static bool StmtWritesShared(const Stmt &stmt) {
  // Any GMEM->SMEM prelude copy that stays on the full thread extent needs a
  // block-wide sync before the WS split.  Whether LowerTileOp later emits
  // TMA or SIMT, producer warps participate in the write and then leave
  // the consumer path.  TMA copies rewritten onto mbarrier (see
  // common_prelude_rewrites_) are excluded by the caller.
  bool found = false;
  PostOrderVisit(stmt, [&](const ObjectRef &node) {
    if (found) {
      return;
    ...
    if (const auto *eval = node.as<EvaluateNode>()) {
      Optional<Call> call =
          GetEvaluateCallInSimpleWrapper(GetRef<Stmt>(eval));
      if (!call.defined()) {
        return;
      }
      auto tile_op = ParseOperator(call.value());
...
private:
  ...
  public:
    ...
    Optional<Stmt> VisitStmt_(const SeqStmtNode *op) final {
      ...
      if (KeptPreludeHasSimtSharedWrite(op, loop_idx)) {
        // Producer warps participate in the shared prelude copies, then take
        // the TMA branch.  Without a block-wide sync here, consumer WGMMA
        // can read stale shared from those warps (example_mamba_chunk_scan
        // fails once the grid exceeds one wave).
        new_seq.push_back(MakeSharedStorageSync());
      }
  ...
  private:
    ...
    bool KeptPreludeHasSimtSharedWrite(const SeqStmtNode *op,
                                       int loop_idx) const {
      for (int i = 0; i < loop_idx; ++i) {
        if (ClassifyPreludeStmt(op->seq[i], rewriter_->buffer_data_to_buffer_,
                                rewriter_->shared_prelude_live_seed_,
                                rewriter_->producer_prelude_live_seed_,
                                rewriter_->consumer_prelude_live_seed_) !=
            PreludeStmtPlacement::kKeepSharedPrelude) {
          continue;
        }
        if (rewriter_->common_prelude_rewrites_.count(op->seq[i])) {
          continue;
        }
        if (StmtWritesShared(op->seq[i])) {
          return true;
        }
      }
      return false;
...
tvm::transform::Pass ProducerConsumerWarpSpecialized() {
  using namespace tirx::transform;
    ...
    if (f->GetAttr<ObjectRef>(attr::kOverlapPlan).defined()) {
      return f;
    }
    // Skip if disabled.

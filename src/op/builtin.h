...
// Internal scope retained until ThreadSync so program-schedule group branches
// remain distinguishable after generic warp-specialization lowering passes.
static constexpr const char *kThreadSyncWarpSpecializationScope =
    "tl.thread_sync.warp_specialization_scope";
...
// Typed overlap schedule attached to one PrimFunc.
static constexpr const char *kOverlapPlan = "tl.overlap_plan";
// PrimFunc attribute: request kernel-wide OverlapPlan lowering. Group
// assignment applies to the whole kernel, not a single pipeline loop.
static constexpr const char *kAutoOverlap = "tl.auto_overlap";
// Loop annotation: this For was created by T.Pipelined. Present even when
// num_stages is 0, so OverlapPlan can treat the loop as a pipeline region.
static constexpr const char *kPipelinedLoop = "tl.pipelined";
} // namespace attr
...
// PrimFunc attribute: Map<String, IntImm> of planned byte offsets for the
// shared.dyn buffers already packed by a whole-program scheduler. Merge
// applies these offsets instead of re-running liveness analysis. Written by
// LowerOverlapPlan from BufferPlan.byte_offset, propagated through
// SplitHostDevice, consumed by MergeSharedMemoryAllocations. Unplanned
// residual allocations are appended after kSmemPlannedArenaBytes.
static constexpr const char *kSmemOffsetMap = "tl.smem_offset_map";
static constexpr const char *kSmemPlannedArenaBytes =
    "tl.smem_planned_arena_bytes";

...
Fragment ParallelOpNode::ComputeLoopLayoutFromBuffer(
  ...
  if (IsCommonAccessIndice(buffer)) {
    ...
    ICHECK_EQ(GetAccessInfo(buffer).indices.size(), src_layout->InputDim())
        << "layout rank mismatch while inferring T.Parallel from buffer `"
        << buffer->name << "`: access has "
        << GetAccessInfo(buffer).indices.size() << " indices but layout has "
        << src_layout->InputDim() << " input dimensions";
    Var rep("_rep");
    auto rep_iter =

"""Overlaper search kernel adapted from examples/flash_attention/example_mha_bwd_bhsd.py."""
...
def add_arguments(parser) -> None:
    ...
    group.add_argument("--mha-bwd-block-m", type=int, nargs="+", default=[64, 128])
    group.add_argument("--mha-bwd-block-n", type=int, nargs="+", default=[32, 64, 128])
...
def ref_program(Q, K, V, dO, lse, Delta, dQ, is_causal):
    dim = Q.size(-1)
    seq_len = Q.size(2)
    sm_scale = dim**-0.5
    scale = sm_scale * 1.44269504
    ...
    dV = torch.einsum("bhqk,bhqd->bhkd", weights, dO.float()).to(V.dtype)
    dP = torch.einsum("bhqd,bhkd->bhqk", dO.float(), V.float())
    dS = weights * (dP - Delta.unsqueeze(-1)) * sm_scale
    dK = torch.einsum("bhqk,bhqd->bhkd", dS, Q.float()).to(K.dtype)
    return dK, dV

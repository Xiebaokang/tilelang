"""Overlaper search kernel adapted from examples/deepseek_mla/example_mla_decode.py.
...
``[block_H, dim]``, online softmax, then ``S @ KV``. Overlaper-specific pieces
are ``auto_wsp=True`` and WGMMA M-tile thread counts (``block_H // 64 * 128``).
"""
...
def configurations(options: Options) -> list[TileConfig]:
    ...
    for block_h, block_n in product(
        ...
        #     continue
        # # Hopper WGMMA covers 64 rows of M per warp group (128 threads).
        # if block_h % 64:
        #     continue
        print(block_h, block_n)
...
def build(options: Options, config: TileConfig) -> SearchWorkload:
    ...
    if not configurations(options):
        raise ValueError("mla produced no valid block_h tiles for this head count")
    prim_func = flashattn(
        batch,
        heads,
        kv_heads,

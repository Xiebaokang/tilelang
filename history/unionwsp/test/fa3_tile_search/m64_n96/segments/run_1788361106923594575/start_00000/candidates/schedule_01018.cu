    ...
    if (((int)threadIdx.x) < 256) {
      ...
      for (int k_1 = 0; k_1 < 86; ++k_1) {
        ...
        for (int i_13 = 0; i_13 < 12; ++i_13) {
          ...
          *(uint4*)(((half_t*)V_shared) + ((((((((((int)threadIdx.x) & 15) >> 3) * 6144) + (i_13 * 512)) + (((((int)threadIdx.x) & 127) >> 4) * 64)) + (((((((int)threadIdx.x) & 127) >> 6) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 32)) + (((((((int)threadIdx.x) & 63) >> 5) + ((((int)threadIdx.x) & 3) >> 1)) & 1) * 16)) + (((((((int)threadIdx.x) & 31) >> 4) + (((int)threadIdx.x) & 1)) & 1) * 8))) = condval_1;
        }
        tl::fence_proxy_async();

    ...
    *(float2*)(scores_max + 0) = make_float2(broadcast_var_3, broadcast_var_3);
    ...
    if (((int)threadIdx.x) < 256) {
      ...
      for (int i_21 = 0; i_21 < 8; ++i_21) {
        *(uint4*)(((half_t*)Q_shared) + ((((((((((int)threadIdx.x) & 15) >> 3) * 4096) + (i_21 * 512)) + (((((int)threadIdx.x) & 127) >> 4) * 64)) + (((((((int)threadIdx.x) & 127) >> 6) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 32)) + (((((((int)threadIdx.x) & 63) >> 5) + ((((int)threadIdx.x) & 3) >> 1)) & 1) * 16)) + (((((((int)threadIdx.x) & 31) >> 4) + (((int)threadIdx.x) & 1)) & 1) * 8))) = *(uint4*)(Q + (((((((int)blockIdx.y) * 1048576) + (((int)blockIdx.x) * 8192)) + (i_21 * 1024)) + (((int)threadIdx.x) * 8)) - 1024));
      }
      tl::fence_proxy_async();

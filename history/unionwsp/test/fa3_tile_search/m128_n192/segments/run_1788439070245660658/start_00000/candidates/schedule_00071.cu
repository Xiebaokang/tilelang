    ...
    *(float2*)(scores_max + 0) = make_float2(broadcast_var_3, broadcast_var_3);
    ...
    for (int k_1 = 0; k_1 < 43; ++k_1) {
      ...
      for (int i_23 = 0; i_23 < 24; ++i_23) {
        ...
        uint4 condval_2;
        if ((((k_1 * 4) + ((i_23 + 2) / 6)) < 171)) {
          condval_2 = *(uint4*)(V + (((((((int)blockIdx.y) * 1048576) + (k_1 * 24576)) + (i_23 * 1024)) + (((int)threadIdx.x) * 8)) - 2048));
        } else {
        ...
        *(uint4*)(((half_t*)V_shared) + ((((((((k_1 & 1) * 24576) + (((((int)threadIdx.x) & 15) >> 3) * 12288)) + (i_23 * 512)) + (((((int)threadIdx.x) & 127) >> 4) * 64)) + (((((((int)threadIdx.x) & 127) >> 6) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 32)) + (((((((int)threadIdx.x) & 63) >> 5) + ((((int)threadIdx.x) & 3) >> 1)) & 1) * 16)) + (((((((int)threadIdx.x) & 31) >> 4) + (((int)threadIdx.x) & 1)) & 1) * 8))) = condval_2;
      }
      tl::fence_proxy_async();

    ...
    *(float2*)(scores_max + 0) = make_float2(broadcast_var_2, broadcast_var_2);
    ...
    for (int k = 0; k < 42; ++k) {
      if (tl::tl_shuffle_elect<256>()) {
        ...
        pipeline_mbar[(((k & 1) * 2) + 1)].arrive_and_expect_tx(49152);
        tl::tma_load(V_desc, pipeline_mbar[(((k & 1) * 2) + 1)], (&(((half_t*)V_shared)[0])), 0, (k * 192), ((int)blockIdx.y), 0);
        tl::tma_load(V_desc, pipeline_mbar[(((k & 1) * 2) + 1)], (&(((half_t*)V_shared)[12288])), 64, (k * 192), ((int)blockIdx.y), 0);
      }

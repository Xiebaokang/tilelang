    ...
    *(float2*)(scores_max + 0) = make_float2(broadcast_var_5, broadcast_var_5);
    ...
    if (((int)threadIdx.x) < 384) {
      ...
      for (int k_2 = 0; k_2 < 43; ++k_2) {
        if (1 <= k_2) {
          program_schedule_mbar[4].wait(((k_2 + 1) & 1));
        }
        #pragma unroll
        for (int i_32 = 0; i_32 < 24; ++i_32) {
          ...
          *(uint4*)(((half_t*)K_shared) + ((((((((((int)threadIdx.x) & 15) >> 3) * 12288) + (i_32 * 512)) + (((((int)threadIdx.x) & 127) >> 4) * 64)) + (((((((int)threadIdx.x) & 127) >> 6) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 32)) + (((((((int)threadIdx.x) & 63) >> 5) + ((((int)threadIdx.x) & 3) >> 1)) & 1) * 16)) + (((((((int)threadIdx.x) & 31) >> 4) + (((int)threadIdx.x) & 1)) & 1) * 8))) = condval_2;
        }
        tl::fence_proxy_async();

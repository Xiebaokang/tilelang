    ...
    for (int k = 0; k < 64; ++k) {
      ...
      *(float2*)(scores_max + 0) = make_float2(broadcast_var_3, broadcast_var_3);
      ...
      for (int i_11 = 0; i_11 < 16; ++i_11) {
        *(uint4*)(((half_t*)V_shared) + ((((((((((int)threadIdx.x) & 15) >> 3) * 8192) + (i_11 * 512)) + ((((int)threadIdx.x) >> 4) * 64)) + ((((((int)threadIdx.x) >> 6) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 32)) + (((((((int)threadIdx.x) & 63) >> 5) + ((((int)threadIdx.x) & 3) >> 1)) & 1) * 16)) + (((((((int)threadIdx.x) & 31) >> 4) + (((int)threadIdx.x) & 1)) & 1) * 8))) = *(uint4*)(V + ((((((int)blockIdx.y) * 1048576) + (k * 16384)) + (i_11 * 1024)) + (((int)threadIdx.x) * 8)));
      }
      {

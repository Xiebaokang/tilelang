    ...
    *(float2*)(scores_max + 0) = make_float2(broadcast_var_2, broadcast_var_2);
    for (int k = 0; k < 86; ++k) {
      ...
      for (int i_2 = 0; i_2 < 6; ++i_2) {
        ...
        uint4 condval;
        if ((((k * 3) + (i_2 >> 1)) < 256)) {
          condval = *(uint4*)(K + ((((((int)blockIdx.y) * 1048576) + (k * 12288)) + (i_2 * 2048)) + (((int)threadIdx.x) * 8)));
        } else {
        ...
        *(uint4*)(((half_t*)K_shared) + ((((((((((int)threadIdx.x) & 15) >> 3) * 6144) + (i_2 * 1024)) + ((((int)threadIdx.x) >> 4) * 64)) + (((((((int)threadIdx.x) & 127) >> 6) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 32)) + (((((((int)threadIdx.x) & 63) >> 5) + ((((int)threadIdx.x) & 3) >> 1)) & 1) * 16)) + (((((((int)threadIdx.x) & 31) >> 4) + (((int)threadIdx.x) & 1)) & 1) * 8))) = condval;
      }
      #pragma unroll
      ...
      *(float2*)(scores_max + 0) = make_float2(broadcast_var_4, broadcast_var_4);
      ...
      for (int i_4 = 0; i_4 < 2; ++i_4) {
        ...
        for (int rv = 0; rv < 24; ++rv) {
          scores_max_clear[i_4] = max(scores_max_clear[i_4], acc_s[((((rv % 12) * 4) + (i_4 * 2)) + (rv / 12))]);
        }

    ...
    *(float2*)(scores_max + 0) = make_float2(broadcast_var_3, broadcast_var_3);
    ...
    for (int k = 0; k < 85; ++k) {
      if (tl::tl_shuffle_elect<256>()) {
        pipeline_mbar[(k & 1)].arrive_and_expect_tx(24576);
        tl::tma_load(V_desc, pipeline_mbar[(k & 1)], (&(((half_t*)V_shared)[0])), 0, (k * 96), ((int)blockIdx.y), 0);
        tl::tma_load(V_desc, pipeline_mbar[(k & 1)], (&(((half_t*)V_shared)[6144])), 64, (k * 96), ((int)blockIdx.y), 0);
      }

  ...
  if (((int)threadIdx.x) < 256) {
    ...
    *(float2*)(scores_max + 0) = make_float2(broadcast_var_3, broadcast_var_3);
    ...
    for (int k = 0; k < 84; ++k) {
      if (tl::tl_shuffle_elect<256>()) {
        pipeline_mbar[(k % 3)].arrive_and_expect_tx(24576);
        tl::tma_load(V_desc, pipeline_mbar[(k % 3)], (&(((half_t*)V_shared)[0])), 0, (k * 96), ((int)blockIdx.y), 0);
        tl::tma_load(V_desc, pipeline_mbar[(k % 3)], (&(((half_t*)V_shared)[6144])), 64, (k * 96), ((int)blockIdx.y), 0);
      }
      ...
      *(float2*)(scores_max_prev + 0) = *(float2*)(scores_max + 0);
      ...
      program_schedule_mbar[(((k + 1) & 1) + 2)].arrive();
    ...
    if (tl::tl_shuffle_elect<256>()) {
      ...
      tl::tma_load(V_desc, pipeline_mbar[0], (&(((half_t*)V_shared)[6144])), 64, 8064, ((int)blockIdx.y), 0);
    }
    ...
    *(float2*)(scores_max + 0) = make_float2(broadcast_var_5, broadcast_var_5);
    ...
    if (tl::tl_shuffle_elect<256>()) {
      ...
      tl::tma_load(V_desc, pipeline_mbar[1], (&(((half_t*)V_shared)[0])), 0, 8160, ((int)blockIdx.y), 0);
      tl::tma_load(V_desc, pipeline_mbar[1], (&(((half_t*)V_shared)[6144])), 64, 8160, ((int)blockIdx.y), 0);
    }
    ...
    for (int k_1 = 0; k_1 < 86; ++k_1) {
      if (2 <= k_1) {
        program_schedule_mbar[((k_1 & 1) + 2)].wait((((k_1 >> 1) + 1) & 1));
      }
      #pragma unroll

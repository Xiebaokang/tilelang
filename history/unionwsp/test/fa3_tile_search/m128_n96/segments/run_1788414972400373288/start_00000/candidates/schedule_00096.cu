...
extern "C" __global__ void __launch_bounds__(384, 1) main_kernel(__grid_constant__ const CUtensorMap K_desc, half_t* __restrict__ Output, const half_t* __restrict__ Q, __grid_constant__ const CUtensorMap V_desc) {
  ...
  if (((int)threadIdx.x) < 256) {
    ...
    *(float2*)(scores_max + 0) = make_float2(broadcast_var_2, broadcast_var_2);
    ...
    for (int k = 0; k < 85; ++k) {
      if (tl::tl_shuffle_elect<256>()) {
        pipeline_mbar[(((k + 1) & 1) * 2)].arrive_and_expect_tx(24576);
        tl::tma_load(K_desc, pipeline_mbar[(((k + 1) & 1) * 2)], (&(((half_t*)K_shared)[(((k + 1) % 3) * 12288)])), 0, ((k * 96) + 96), ((int)blockIdx.y), 0);
        tl::tma_load(K_desc, pipeline_mbar[(((k + 1) & 1) * 2)], (&(((half_t*)K_shared)[((((k + 1) % 3) * 12288) + 6144)])), 64, ((k * 96) + 96), ((int)blockIdx.y), 0);
    ...
    *(float2*)(scores_max_prev + 0) = *(float2*)(scores_max + 0);
    ...
    pipeline_mbar[2].wait(0);
    {
      ...
      tl::increase_descriptor_offset<int>(desc_b_2, 24576);
      tl::warpgroup_fence_operand(reinterpret_cast<float*>(acc_s + 0), 96);
      tl::warpgroup_arrive();
      #pragma unroll

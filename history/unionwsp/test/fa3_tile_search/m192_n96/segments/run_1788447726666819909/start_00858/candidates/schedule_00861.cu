...
extern "C" __global__ void __launch_bounds__(640, 1) main_kernel(const half_t* __restrict__ K, __grid_constant__ const CUtensorMap Output_desc, const half_t* __restrict__ Q, const half_t* __restrict__ V) {
  ...
  void* O_shared = ((void*)((char*)buf_dyn_shmem + 73728));
    ...
    if (((int)threadIdx.x) < 512) {
      ...
      for (int k_1 = 0; k_1 < 86; ++k_1) {
        ...
        for (int i_14 = 0; i_14 < 12; ++i_14) {
          ...
          *(uint4*)(((half_t*)K_shared) + ((((((((((int)threadIdx.x) & 15) >> 3) * 6144) + (i_14 * 512)) + (((((int)threadIdx.x) & 127) >> 4) * 64)) + (((((((int)threadIdx.x) & 127) >> 6) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 32)) + (((((((int)threadIdx.x) & 63) >> 5) + ((((int)threadIdx.x) & 3) >> 1)) & 1) * 16)) + (((((((int)threadIdx.x) & 31) >> 4) + (((int)threadIdx.x) & 1)) & 1) * 8))) = condval_3;
        }
        tl::fence_proxy_async();

    ...
    if (((int)threadIdx.x) < 512) {
      ...
      for (int k_2 = 0; k_2 < 86; ++k_2) {
        ...
        for (int i_32 = 0; i_32 < 12; ++i_32) {
          ...
          *(uint4*)(((half_t*)K_shared) + ((((((((((int)threadIdx.x) & 15) >> 3) * 6144) + (i_32 * 512)) + (((((int)threadIdx.x) & 127) >> 4) * 64)) + (((((((int)threadIdx.x) & 127) >> 6) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 32)) + (((((((int)threadIdx.x) & 63) >> 5) + ((((int)threadIdx.x) & 3) >> 1)) & 1) * 16)) + (((((((int)threadIdx.x) & 31) >> 4) + (((int)threadIdx.x) & 1)) & 1) * 8))) = condval_3;
        }
        tl::fence_proxy_async();

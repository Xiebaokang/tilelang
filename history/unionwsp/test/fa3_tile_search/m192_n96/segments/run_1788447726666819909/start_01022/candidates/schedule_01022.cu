  ...
  if (((int)threadIdx.x) < 384) {
    ...
    *(float2*)(scores_max + 0) = make_float2(broadcast_var_6, broadcast_var_6);
    ...
    for (int i_34 = 0; i_34 < 12; ++i_34) {
      ...
      *(uint4*)(((half_t*)V_shared) + ((((((((((int)threadIdx.x) & 15) >> 3) * 6144) + (i_34 * 512)) + (((((int)threadIdx.x) & 127) >> 4) * 64)) + (((((((int)threadIdx.x) & 127) >> 6) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 32)) + (((((((int)threadIdx.x) & 63) >> 5) + ((((int)threadIdx.x) & 3) >> 1)) & 1) * 16)) + (((((((int)threadIdx.x) & 31) >> 4) + (((int)threadIdx.x) & 1)) & 1) * 8))) = condval_3;
    }
    tl::fence_proxy_async();
    program_schedule_mbar[2].arrive();
  }

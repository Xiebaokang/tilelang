...
extern "C" __global__ void __launch_bounds__(512, 1) main_kernel(const half_t* __restrict__ K, half_t* __restrict__ Output, const half_t* __restrict__ Q, __grid_constant__ const CUtensorMap V_desc) {
  ...
  if (((int)threadIdx.x) < 256) {
    ...
    *(float2*)(scores_max + 0) = make_float2(broadcast_var_2, broadcast_var_2);
    ...
    for (int k = 0; k < 63; ++k) {
      if (tl::tl_shuffle_elect<256>()) {
        pipeline_mbar[((k + 1) & 1)].arrive_and_expect_tx(32768);
        tl::tma_load(V_desc, pipeline_mbar[((k + 1) & 1)], (&(((half_t*)V_shared)[(((k + 1) % 3) * 16384)])), 0, ((k * 128) + 128), ((int)blockIdx.y), 0);
        tl::tma_load(V_desc, pipeline_mbar[((k + 1) & 1)], (&(((half_t*)V_shared)[((((k + 1) % 3) * 16384) + 8192)])), 64, ((k * 128) + 128), ((int)blockIdx.y), 0);
      }

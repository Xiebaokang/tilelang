...
extern "C" __global__ void __launch_bounds__(512, 1) main_kernel(const half_t* __restrict__ K, half_t* __restrict__ Output, const half_t* __restrict__ Q, const half_t* __restrict__ V) {
  ...
  if (((int)threadIdx.x) < 256) {
    ...
    *(float2*)(scores_max + 0) = make_float2(broadcast_var_3, broadcast_var_3);
    ...
    program_schedule_mbar[11].arrive();
    *(float2*)(scores_max_prev + 0) = *(float2*)(scores_max + 0);
    #pragma unroll
    ...
    if (((int)threadIdx.x) < 384) {
      ...
      for (int k_2 = 0; k_2 < 64; ++k_2) {
        ...
        for (int i_32 = 0; i_32 < 16; ++i_32) {
          *(uint4*)(((half_t*)K_shared) + ((((((((k_2 & 1) * 16384) + (((((int)threadIdx.x) & 15) >> 3) * 8192)) + (i_32 * 512)) + (((((int)threadIdx.x) & 127) >> 4) * 64)) + (((((((int)threadIdx.x) & 127) >> 6) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 32)) + (((((((int)threadIdx.x) & 63) >> 5) + ((((int)threadIdx.x) & 3) >> 1)) & 1) * 16)) + (((((((int)threadIdx.x) & 31) >> 4) + (((int)threadIdx.x) & 1)) & 1) * 8))) = *(uint4*)(K + (((((((int)blockIdx.y) * 1048576) + (k_2 * 16384)) + (i_32 * 1024)) + (((int)threadIdx.x) * 8)) - 3072));
        }
        tl::fence_proxy_async();

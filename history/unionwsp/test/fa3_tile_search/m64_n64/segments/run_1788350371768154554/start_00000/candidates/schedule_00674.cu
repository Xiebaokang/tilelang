    ...
    if (((int)threadIdx.x) < 256) {
      ...
      for (int k_2 = 0; k_2 < 128; ++k_2) {
        ...
        for (int i_32 = 0; i_32 < 8; ++i_32) {
          *(uint4*)(((half_t*)V_shared) + ((((((((k_2 & 1) * 8192) + (((((int)threadIdx.x) & 15) >> 3) * 4096)) + (i_32 * 512)) + (((((int)threadIdx.x) & 127) >> 4) * 64)) + (((((((int)threadIdx.x) & 127) >> 6) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 32)) + (((((((int)threadIdx.x) & 63) >> 5) + ((((int)threadIdx.x) & 3) >> 1)) & 1) * 16)) + (((((((int)threadIdx.x) & 31) >> 4) + (((int)threadIdx.x) & 1)) & 1) * 8))) = *(uint4*)(V + (((((((int)blockIdx.y) * 1048576) + (k_2 * 8192)) + (i_32 * 1024)) + (((int)threadIdx.x) * 8)) - 2048));
        }
        tl::fence_proxy_async();

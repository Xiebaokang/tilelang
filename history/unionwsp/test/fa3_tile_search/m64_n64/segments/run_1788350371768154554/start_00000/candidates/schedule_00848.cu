    ...
    if (((int)threadIdx.x) < 256) {
      ...
      for (int k_1 = 0; k_1 < 127; ++k_1) {
        ...
        for (int i_32 = 0; i_32 < 8; ++i_32) {
          *(uint4*)(((half_t*)K_shared) + ((((((((((int)threadIdx.x) & 15) >> 3) * 4096) + (i_32 * 512)) + (((((int)threadIdx.x) & 127) >> 4) * 64)) + (((((((int)threadIdx.x) & 127) >> 6) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 32)) + (((((((int)threadIdx.x) & 63) >> 5) + ((((int)threadIdx.x) & 3) >> 1)) & 1) * 16)) + (((((((int)threadIdx.x) & 31) >> 4) + (((int)threadIdx.x) & 1)) & 1) * 8))) = *(uint4*)(K + (((((((int)blockIdx.y) * 1048576) + (k_1 * 8192)) + (i_32 * 1024)) + (((int)threadIdx.x) * 8)) + 7168));
        }
        tl::fence_proxy_async();
        program_schedule_mbar[1].arrive();
        if (2 <= k_1) {

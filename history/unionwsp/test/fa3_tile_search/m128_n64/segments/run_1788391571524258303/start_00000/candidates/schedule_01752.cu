    ...
    *(float4*)(scores_max + 0) = make_float4(broadcast_var_2, broadcast_var_2, broadcast_var_2, broadcast_var_2);
    ...
    for (int k = 0; k < 126; ++k) {
      if (tl::tl_shuffle_elect<128>()) {
        ...
        tl::tma_load(K_desc, pipeline_mbar[(((k + 1) % 3) * 2)], (&(((half_t*)K_shared)[(((k + 1) & 1) * 8192)])), 0, ((k * 64) + 64), ((int)blockIdx.y), 0);
        tl::tma_load(K_desc, pipeline_mbar[(((k + 1) % 3) * 2)], (&(((half_t*)K_shared)[((((k + 1) & 1) * 8192) + 4096)])), 64, ((k * 64) + 64), ((int)blockIdx.y), 0);

#if defined(_MSC_VER) && !defined(__clang__) && _MSC_VER < 1940
#define _tl_orig_alignas alignas
#define alignas(N) _tl_orig_alignas((N) <= 64 ? (N) : 64)
#include <cuda.h>
#undef alignas
#define alignas _tl_orig_alignas
#endif
#include <tl_templates/cuda/instruction/wgmma.h>
#include <tl_templates/cuda/intrin.h>
#include <tl_templates/cuda/barrier.h>
#include <tl_templates/cuda/copy_sm90.h>
#include <math_constants.h>
#include <tl_templates/cuda/reduce.h>
#include <tl_templates/cuda/scan.h>
#include <tl_templates/cuda/ldsm.h>
#include <tl_templates/cuda/threadblock_swizzle.h>
#include <tl_templates/cuda/debug.h>
#ifdef ENABLE_BF16
#include <tl_templates/cuda/cuda_bf16_fallbacks.cuh>
#endif

extern "C" __global__ void main_kernel(const half_t* __restrict__ K, half_t* __restrict__ Output, const half_t* __restrict__ Q, const half_t* __restrict__ V);
extern "C" __global__ void __launch_bounds__(384, 1) main_kernel(const half_t* __restrict__ K, half_t* __restrict__ Output, const half_t* __restrict__ Q, const half_t* __restrict__ V) {
  extern __shared__ __align__(1024) uchar buf_dyn_shmem[];
  void* V_shared = ((void*)((char*)buf_dyn_shmem + 0));
  void* K_shared = ((void*)((char*)buf_dyn_shmem + 49152));
  void* Q_shared = ((void*)((char*)buf_dyn_shmem + 81920));
  void* O_shared = ((void*)((char*)buf_dyn_shmem + 114688));
  __shared__ __align__(16) uint64_t program_schedule_mbar_mem[12];
  auto program_schedule_mbar = reinterpret_cast<Barrier*>(program_schedule_mbar_mem);
  float acc_o[128];
  float logsum[4];
  float scores_max[4];
  float acc_s[64];
  float scores_max_prev[4];
  float scores_max_clear[4];
  float scores_scale[4];
  float scores_sum[4];
  half_t acc_s_cast[64];
  if (tl::tl_shuffle_elect<0>()) {
    program_schedule_mbar[0].init(128);
    program_schedule_mbar[1].init(128);
    program_schedule_mbar[2].init(128);
    program_schedule_mbar[3].init(128);
    program_schedule_mbar[4].init(128);
    program_schedule_mbar[5].init(128);
    program_schedule_mbar[6].init(128);
    program_schedule_mbar[7].init(128);
    program_schedule_mbar[8].init(128);
    program_schedule_mbar[9].init(128);
    program_schedule_mbar[10].init(128);
    program_schedule_mbar[11].init(128);
  }
  tl::fence_barrier_init();
  __syncthreads();
  if (((int)threadIdx.x) < 128) {
    tl::warpgroup_reg_alloc<240>();
    #pragma unroll
    for (int i = 0; i < 32; ++i) {
      float broadcast_var = 0x0p+0f/*0.000000e+00*/;
      *(float4*)(acc_o + (i * 4)) = make_float4(broadcast_var, broadcast_var, broadcast_var, broadcast_var);
    }
    float broadcast_var_1 = 0x0p+0f/*0.000000e+00*/;
    *(float4*)(logsum + 0) = make_float4(broadcast_var_1, broadcast_var_1, broadcast_var_1, broadcast_var_1);
    float broadcast_var_2 = -CUDART_INF_F;
    *(float4*)(scores_max + 0) = make_float4(broadcast_var_2, broadcast_var_2, broadcast_var_2, broadcast_var_2);
    for (int k = 0; k < 128; ++k) {
      #pragma unroll
      for (int i_1 = 0; i_1 < 64; ++i_1) {
        acc_s[i_1] = 0x0p+0f/*0.000000e+00*/;
      }
      if (k == 0) {
        program_schedule_mbar[0].wait(0);
      }
      program_schedule_mbar[((k & 1) + 1)].wait(((k & 3) >> 1));
      {
        tl::GmmaDescriptor desc_a;
        tl::GmmaDescriptor desc_b;
        tl::initialize_wgmma_descriptor<1, 1, 64>(desc_a, (&(((half_t*)Q_shared)[0])));
        tl::initialize_wgmma_descriptor<1, 1, 64>(desc_b, (&(((half_t*)K_shared)[0])));
        tl::increase_descriptor_offset<int>(desc_b, ((k & 1) * 16384));
        tl::warpgroup_fence_operand(reinterpret_cast<float*>(acc_s + 0), 64);
        tl::warpgroup_arrive();
        #pragma unroll
        for (int i_2 = 0; i_2 < 2; ++i_2) {
          #pragma unroll
          for (int ki = 0; ki < 8; ++ki) {
            tl::wgmma_ss<tl::DataType::kFloat16, tl::DataType::kFloat16, tl::DataType::kFloat32, 64, 64, 16, false, false, 1, 1>(uint64_t(desc_a + (((((ki >> 2) * 16384) + (i_2 * 8192)) + ((ki & 3) * 32)) >> 4)), uint64_t(desc_b + ((((ki >> 2) * 8192) + ((ki & 3) * 32)) >> 4)), ((uint32_t*)(acc_s + (i_2 * 32))), 1);
          }
        }
        tl::warpgroup_commit_batch();
        tl::warpgroup_wait<0>();
        tl::warpgroup_fence_operand(reinterpret_cast<float*>(acc_s + 0), 64);
      }
      program_schedule_mbar[((k & 1) + 7)].arrive();
      *(float4*)(scores_max_prev + 0) = *(float4*)(scores_max + 0);
      float broadcast_var_3 = -CUDART_INF_F;
      *(float4*)(scores_max + 0) = make_float4(broadcast_var_3, broadcast_var_3, broadcast_var_3, broadcast_var_3);
      #pragma unroll
      for (int i_3 = 0; i_3 < 4; ++i_3) {
        scores_max_clear[i_3] = -CUDART_INF_F;
        #pragma unroll
        for (int rv = 0; rv < 16; ++rv) {
          scores_max_clear[i_3] = max(scores_max_clear[i_3], acc_s[(((((i_3 >> 1) * 32) + ((rv & 7) * 4)) + ((i_3 & 1) * 2)) + (rv >> 3))]);
        }
        scores_max_clear[i_3] = tl::AllReduce<tl::MaxOp, 4, 1, 0, tl::NamedBarrier<128>>::run(scores_max_clear[i_3]);
        scores_max[i_3] = max(scores_max[i_3], scores_max_clear[i_3]);
      }
      #pragma unroll
      for (int i_4 = 0; i_4 < 4; ++i_4) {
        scores_max[i_4] = max(scores_max[i_4], scores_max_prev[i_4]);
      }
      #pragma unroll
      for (int i_5 = 0; i_5 < 4; ++i_5) {
        scores_scale[i_5] = exp2f(((scores_max_prev[i_5] * 0x1.0527dbd5cafffp-3f/*1.275174e-01*/) - (scores_max[i_5] * 0x1.0527dbd5cafffp-3f/*1.275174e-01*/)));
      }
      #pragma unroll
      for (int i_6 = 0; i_6 < 64; ++i_6) {
        acc_s[i_6] = exp2f(((acc_s[i_6] * 0x1.0527dbd5cafffp-3f/*1.275174e-01*/) - (scores_max[(((i_6 >> 5) * 2) + ((i_6 & 3) >> 1))] * 0x1.0527dbd5cafffp-3f/*1.275174e-01*/)));
      }
      #pragma unroll
      for (int i_7 = 0; i_7 < 4; ++i_7) {
        scores_sum[i_7] = 0x0p+0f/*0.000000e+00*/;
        #pragma unroll
        for (int rv_1 = 0; rv_1 < 16; ++rv_1) {
          scores_sum[i_7] = (scores_sum[i_7] + acc_s[(((((i_7 >> 1) * 32) + ((rv_1 & 7) * 4)) + ((i_7 & 1) * 2)) + (rv_1 >> 3))]);
        }
        scores_sum[i_7] = tl::AllReduce<tl::SumOp, 4, 1, 0, tl::NamedBarrier<128>>::run(scores_sum[i_7]);
      }
      #pragma unroll
      for (int i_8 = 0; i_8 < 4; ++i_8) {
        logsum[i_8] = ((logsum[i_8] * scores_scale[i_8]) + scores_sum[i_8]);
      }
      #pragma unroll
      for (int i_9 = 0; i_9 < 16; ++i_9) {
        uint2 __1;
        float4 v_ = *(float4*)(acc_s + (((((i_9 & 3) >> 1) * 32) + ((i_9 >> 2) * 8)) + ((i_9 & 1) * 4)));
        ((half2*)(&__1))[0] = __float22half2_rn(((float2*)(&v_))[0]);
        ((half2*)(&__1))[1] = __float22half2_rn(((float2*)(&v_))[1]);
        *(uint2*)(acc_s_cast + (i_9 * 4)) = __1;
      }
      #pragma unroll
      for (int i_10 = 0; i_10 < 128; ++i_10) {
        acc_o[i_10] = (acc_o[i_10] * scores_scale[(((i_10 >> 6) * 2) + ((i_10 & 3) >> 1))]);
      }
      program_schedule_mbar[((k % 3) + 3)].wait(((k % 6) / 3));
      {
        tl::GmmaDescriptor desc_b_1;
        tl::initialize_wgmma_descriptor<1, 512, 64>(desc_b_1, (&(((half_t*)V_shared)[0])));
        tl::increase_descriptor_offset<int>(desc_b_1, ((k % 3) * 16384));
        tl::warpgroup_fence_operand(reinterpret_cast<uint32_t*>(acc_s_cast + 0), 32);
        tl::warpgroup_fence_operand(reinterpret_cast<float*>(acc_o + 0), 128);
        tl::warpgroup_arrive();
        #pragma unroll
        for (int i_11 = 0; i_11 < 2; ++i_11) {
          #pragma unroll
          for (int ki_1 = 0; ki_1 < 4; ++ki_1) {
            tl::wgmma_rs<tl::DataType::kFloat16, tl::DataType::kFloat16, tl::DataType::kFloat32, 64, 128, 16, false, true, 1, 1>(reinterpret_cast<const uint32_t*>(acc_s_cast + ((ki_1 * 16) + (i_11 * 8))), uint64_t(desc_b_1 + ((ki_1 * 2048) >> 4)), reinterpret_cast<uint32_t*>(acc_o + (i_11 * 64)), 1);
          }
        }
        tl::warpgroup_commit_batch();
        tl::warpgroup_wait<0>();
        tl::warpgroup_fence_operand(reinterpret_cast<float*>(acc_o + 0), 128);
        tl::warpgroup_fence_operand(reinterpret_cast<uint32_t*>(acc_s_cast + 0), 32);
      }
      program_schedule_mbar[((k % 3) + 9)].arrive();
    }
    #pragma unroll
    for (int i_12 = 0; i_12 < 128; ++i_12) {
      acc_o[i_12] = (acc_o[i_12] / logsum[(((i_12 >> 6) * 2) + ((i_12 & 3) >> 1))]);
    }
    #pragma unroll
    for (int i_13 = 0; i_13 < 16; ++i_13) {
      tl::ptx_stmatrix_m8n8_x4((&(((half_t*)O_shared)[((((((i_13 >> 3) * 8192) + ((((int)threadIdx.x) >> 5) * 2048)) + ((((int)threadIdx.x) & 15) * 128)) + ((i_13 & 7) * 16)) + (((((int)threadIdx.x) & 31) >> 4) * 8))])), __pack_half2(((half_t)acc_o[(i_13 * 8)]), ((half_t)acc_o[((i_13 * 8) + 1)])), __pack_half2(((half_t)acc_o[((i_13 * 8) + 2)]), ((half_t)acc_o[((i_13 * 8) + 3)])), __pack_half2(((half_t)acc_o[((i_13 * 8) + 4)]), ((half_t)acc_o[((i_13 * 8) + 5)])), __pack_half2(((half_t)acc_o[((i_13 * 8) + 6)]), ((half_t)acc_o[((i_13 * 8) + 7)])));
    }
    tl::fence_proxy_async();
    program_schedule_mbar[6].arrive();
  } else {
    if (((int)threadIdx.x) < 256) {
      tl::warpgroup_reg_dealloc<24>();
      #pragma unroll
      for (int i_14 = 0; i_14 < 16; ++i_14) {
        *(uint4*)(((half_t*)Q_shared) + ((((((((((int)threadIdx.x) & 15) >> 3) * 8192) + (i_14 * 512)) + (((((int)threadIdx.x) & 127) >> 4) * 64)) + (((((((int)threadIdx.x) & 127) >> 6) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 32)) + (((((((int)threadIdx.x) & 63) >> 5) + ((((int)threadIdx.x) & 3) >> 1)) & 1) * 16)) + (((((((int)threadIdx.x) & 31) >> 4) + (((int)threadIdx.x) & 1)) & 1) * 8))) = *(uint4*)(Q + (((((((int)blockIdx.y) * 1048576) + (((int)blockIdx.x) * 16384)) + (i_14 * 1024)) + (((int)threadIdx.x) * 8)) - 1024));
      }
      tl::fence_proxy_async();
      program_schedule_mbar[0].arrive();
      #pragma unroll
      for (int i_15 = 0; i_15 < 8; ++i_15) {
        *(uint4*)(((half_t*)V_shared) + ((((((((((int)threadIdx.x) & 15) >> 3) * 4096) + (i_15 * 512)) + (((((int)threadIdx.x) & 127) >> 4) * 64)) + (((((((int)threadIdx.x) & 127) >> 6) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 32)) + (((((((int)threadIdx.x) & 63) >> 5) + ((((int)threadIdx.x) & 3) >> 1)) & 1) * 16)) + (((((((int)threadIdx.x) & 31) >> 4) + (((int)threadIdx.x) & 1)) & 1) * 8))) = *(uint4*)(V + ((((((int)blockIdx.y) * 1048576) + (i_15 * 1024)) + (((int)threadIdx.x) * 8)) - 1024));
      }
      tl::fence_proxy_async();
      program_schedule_mbar[3].arrive();
      tl::__sync_thread_partial(3, 128);
      for (int k_1 = 0; k_1 < 127; ++k_1) {
        if (2 <= k_1) {
          program_schedule_mbar[((k_1 & 1) + 7)].wait((((k_1 >> 1) + 1) & 1));
        }
        #pragma unroll
        for (int i_16 = 0; i_16 < 8; ++i_16) {
          *(uint4*)(((half_t*)K_shared) + ((((((((k_1 & 1) * 8192) + (((((int)threadIdx.x) & 15) >> 3) * 4096)) + (i_16 * 512)) + (((((int)threadIdx.x) & 127) >> 4) * 64)) + (((((((int)threadIdx.x) & 127) >> 6) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 32)) + (((((((int)threadIdx.x) & 63) >> 5) + ((((int)threadIdx.x) & 3) >> 1)) & 1) * 16)) + (((((((int)threadIdx.x) & 31) >> 4) + (((int)threadIdx.x) & 1)) & 1) * 8))) = *(uint4*)(K + (((((((int)blockIdx.y) * 1048576) + (k_1 * 8192)) + (i_16 * 1024)) + (((int)threadIdx.x) * 8)) - 1024));
        }
        tl::fence_proxy_async();
        program_schedule_mbar[((k_1 & 1) + 1)].arrive();
        if (2 <= k_1) {
          program_schedule_mbar[(((k_1 + 1) % 3) + 9)].wait((((k_1 + 4) % 6) / 3));
        }
        #pragma unroll
        for (int i_17 = 0; i_17 < 8; ++i_17) {
          *(uint4*)(((half_t*)V_shared) + (((((((((k_1 + 1) % 3) * 8192) + (((((int)threadIdx.x) & 15) >> 3) * 4096)) + (i_17 * 512)) + (((((int)threadIdx.x) & 127) >> 4) * 64)) + (((((((int)threadIdx.x) & 127) >> 6) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 32)) + (((((((int)threadIdx.x) & 63) >> 5) + ((((int)threadIdx.x) & 3) >> 1)) & 1) * 16)) + (((((((int)threadIdx.x) & 31) >> 4) + (((int)threadIdx.x) & 1)) & 1) * 8))) = *(uint4*)(V + (((((((int)blockIdx.y) * 1048576) + (k_1 * 8192)) + (i_17 * 1024)) + (((int)threadIdx.x) * 8)) + 7168));
        }
        tl::fence_proxy_async();
        program_schedule_mbar[(((k_1 + 1) % 3) + 3)].arrive();
      }
      program_schedule_mbar[8].wait(0);
      tl::__sync_thread_partial(3, 128);
      #pragma unroll
      for (int i_18 = 0; i_18 < 8; ++i_18) {
        *(uint4*)(((half_t*)K_shared) + (((((((((((int)threadIdx.x) & 15) >> 3) * 4096) + (i_18 * 512)) + (((((int)threadIdx.x) & 127) >> 4) * 64)) + (((((((int)threadIdx.x) & 127) >> 6) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 32)) + (((((((int)threadIdx.x) & 63) >> 5) + ((((int)threadIdx.x) & 3) >> 1)) & 1) * 16)) + (((((((int)threadIdx.x) & 31) >> 4) + (((int)threadIdx.x) & 1)) & 1) * 8)) + 8192)) = *(uint4*)(K + ((((((int)blockIdx.y) * 1048576) + (i_18 * 1024)) + (((int)threadIdx.x) * 8)) + 1039360));
      }
      tl::fence_proxy_async();
      program_schedule_mbar[2].arrive();
    } else {
      tl::warpgroup_reg_dealloc<24>();
      program_schedule_mbar[6].wait(0);
      if (tl::tl_shuffle_elect<128>()) {
        tl::tma_store((&(Output[((((int)blockIdx.y) * 1048576) + (((int)blockIdx.x) * 16384))])), (&(((half_t*)O_shared)[0])), 32768);
        tl::tma_store_arrive();
        tl::tma_store_wait<0, true>();
      }
    }
  }
}


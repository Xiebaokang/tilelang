...
extern "C" __global__ void __launch_bounds__(384, 1) main_kernel(const half_t* __restrict__ K, half_t* __restrict__ Output, const half_t* __restrict__ Q, __grid_constant__ const CUtensorMap V_desc) {
  ...
  if (((int)threadIdx.x) < 128) {
    ...
    *(float4*)(scores_max + 0) = make_float4(broadcast_var_3, broadcast_var_3, broadcast_var_3, broadcast_var_3);
    ...
    for (int k = 0; k < 126; ++k) {
      if (tl::tl_shuffle_elect<128>()) {
        pipeline_mbar[((k + 1) % 3)].arrive_and_expect_tx(16384);
        tl::tma_load(V_desc, pipeline_mbar[((k + 1) % 3)], (&(((half_t*)V_shared)[(((k + 1) & 1) * 8192)])), 0, ((k * 64) + 64), ((int)blockIdx.y), 0);
        tl::tma_load(V_desc, pipeline_mbar[((k + 1) % 3)], (&(((half_t*)V_shared)[((((k + 1) & 1) * 8192) + 4096)])), 64, ((k * 64) + 64), ((int)blockIdx.y), 0);
      }
    ...
    if (tl::tl_shuffle_elect<128>()) {
      ...
      tl::tma_load(V_desc, pipeline_mbar[1], (&(((half_t*)V_shared)[8192])), 0, 8128, ((int)blockIdx.y), 0);
      tl::tma_load(V_desc, pipeline_mbar[1], (&(((half_t*)V_shared)[12288])), 64, 8128, ((int)blockIdx.y), 0);
    }

    ...
    *(float4*)(scores_max + 0) = make_float4(broadcast_var_3, broadcast_var_3, broadcast_var_3, broadcast_var_3);
    ...
    for (int k = 0; k < 126; ++k) {
      if (tl::tl_shuffle_elect<128>()) {
        ...
        tl::tma_load(K_desc, pipeline_mbar[((k + 2) % 3)], (&(((half_t*)K_shared)[(((k + 2) % 3) * 8192)])), 0, ((k * 64) + 128), ((int)blockIdx.y), 0);
        tl::tma_load(K_desc, pipeline_mbar[((k + 2) % 3)], (&(((half_t*)K_shared)[((((k + 2) % 3) * 8192) + 4096)])), 64, ((k * 64) + 128), ((int)blockIdx.y), 0);
      }

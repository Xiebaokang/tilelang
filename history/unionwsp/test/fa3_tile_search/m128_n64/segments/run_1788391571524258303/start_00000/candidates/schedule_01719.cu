    ...
    *(float4*)(scores_max + 0) = make_float4(broadcast_var_3, broadcast_var_3, broadcast_var_3, broadcast_var_3);
    ...
    for (int k = 0; k < 126; ++k) {
      if (tl::tl_shuffle_elect<128>()) {
        ...
        tl::tma_load(V_desc, pipeline_mbar[((k + 1) % 3)], (&(((half_t*)V_shared)[(((k + 1) % 3) * 8192)])), 0, ((k * 64) + 64), ((int)blockIdx.y), 0);
        tl::tma_load(V_desc, pipeline_mbar[((k + 1) % 3)], (&(((half_t*)V_shared)[((((k + 1) % 3) * 8192) + 4096)])), 64, ((k * 64) + 64), ((int)blockIdx.y), 0);
      }

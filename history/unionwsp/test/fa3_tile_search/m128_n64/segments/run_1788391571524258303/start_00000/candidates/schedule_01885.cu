...
extern "C" __global__ void __launch_bounds__(384, 1) main_kernel(const half_t* __restrict__ K, half_t* __restrict__ Output, const half_t* __restrict__ Q, const half_t* __restrict__ V) {
  ...
  __shared__ __align__(16) uint64_t program_schedule_mbar_mem[9];
  ...
  if (((int)threadIdx.x) < 128) {
    ...
    *(float4*)(scores_max + 0) = make_float4(broadcast_var_3, broadcast_var_3, broadcast_var_3, broadcast_var_3);
    ...
    for (int k = 0; k < 126; ++k) {
      ...
      program_schedule_mbar[((k & 1) + 7)].arrive();
      *(float4*)(scores_max_prev + 0) = *(float4*)(scores_max + 0);
      #pragma unroll
    ...
    *(float4*)(scores_max + 0) = make_float4(broadcast_var_5, broadcast_var_5, broadcast_var_5, broadcast_var_5);
    ...
    if (((int)threadIdx.x) < 256) {
      ...
      for (int k_1 = 0; k_1 < 127; ++k_1) {
        if (1 <= k_1) {
          program_schedule_mbar[(((k_1 + 1) & 1) + 5)].wait((((k_1 + 3) & 3) >> 1));
        }
        #pragma unroll
        ...
        if (2 <= k_1) {
          program_schedule_mbar[((k_1 & 1) + 7)].wait((((k_1 >> 1) + 1) & 1));
        }
        #pragma unroll
      ...
      program_schedule_mbar[8].wait(0);
      tl::__sync_thread_partial(4, 128);
      #pragma unroll

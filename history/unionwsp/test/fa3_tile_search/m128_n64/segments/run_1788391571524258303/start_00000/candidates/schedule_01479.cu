...
extern "C" __global__ void __launch_bounds__(384, 1) main_kernel(const half_t* __restrict__ K, half_t* __restrict__ Output, const half_t* __restrict__ Q, const half_t* __restrict__ V) {
  ...
  __shared__ __align__(16) uint64_t program_schedule_mbar_mem[12];
  ...
  if (tl::tl_shuffle_elect<0>()) {
    ...
    program_schedule_mbar[10].init(128);
    program_schedule_mbar[11].init(128);
  }
  ...
  if (((int)threadIdx.x) < 128) {
    ...
    *(float4*)(scores_max + 0) = make_float4(broadcast_var_3, broadcast_var_3, broadcast_var_3, broadcast_var_3);
    ...
    for (int k = 0; k < 126; ++k) {
      program_schedule_mbar[((k % 3) + 3)].wait(((k % 6) / 3));
      {
      ...
      program_schedule_mbar[((k % 3) + 9)].arrive();
      *(float4*)(scores_max_prev + 0) = *(float4*)(scores_max + 0);
      #pragma unroll
      ...
      program_schedule_mbar[(((k + 1) & 1) + 7)].arrive();
    ...
    *(float4*)(scores_max + 0) = make_float4(broadcast_var_5, broadcast_var_5, broadcast_var_5, broadcast_var_5);
    ...
    program_schedule_mbar[10].arrive();
    #pragma unroll
    ...
    if (((int)threadIdx.x) < 256) {
      ...
      for (int k_1 = 0; k_1 < 128; ++k_1) {
        if (3 <= k_1) {
          program_schedule_mbar[((k_1 % 3) + 9)].wait((((k_1 / 3) + 1) & 1));
        }
        #pragma unroll
        for (int i_37 = 0; i_37 < 8; ++i_37) {
          *(uint4*)(((half_t*)V_shared) + ((((((((k_1 % 3) * 8192) + (((((int)threadIdx.x) & 15) >> 3) * 4096)) + (i_37 * 512)) + (((((int)threadIdx.x) & 127) >> 4) * 64)) + (((((((int)threadIdx.x) & 127) >> 6) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 32)) + (((((((int)threadIdx.x) & 63) >> 5) + ((((int)threadIdx.x) & 3) >> 1)) & 1) * 16)) + (((((((int)threadIdx.x) & 31) >> 4) + (((int)threadIdx.x) & 1)) & 1) * 8))) = *(uint4*)(V + (((((((int)blockIdx.y) * 1048576) + (k_1 * 8192)) + (i_37 * 1024)) + (((int)threadIdx.x) * 8)) - 1024));
        }
        tl::fence_proxy_async();
        program_schedule_mbar[((k_1 % 3) + 3)].arrive();
      }
    } else {
      tl::warpgroup_reg_dealloc<24>();
      for (int k_2 = 0; k_2 < 128; ++k_2) {
        if (2 <= k_2) {
          program_schedule_mbar[((k_2 & 1) + 7)].wait((((k_2 >> 1) + 1) & 1));
        }
        #pragma unroll

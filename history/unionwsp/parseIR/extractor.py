"""Extract the compact UnionWSP dataflow graph from TileLang TIR."""
...
from ..hardware.resolve import resolve_hardware
from ..hardware.spec import HardwareSpec, OperationInfo
from .graph import (
    BufferAccessKind,
    BufferDescriptor,
    BufferRangeAccess,
...
def _interval_for_index(
    ...
    if isinstance(index, tirx.Ramp):
        if not isinstance(index.stride, tirx.IntImm):
            return None, False
...
def _is_pipeline_loop(loop: tirx.For) -> bool:
    return (
        "num_stages" in loop.annotations
        or "tl_pipeline_order" in loop.annotations
    )
    ...
    def visit(
        ...
        if isinstance(statement, tirx.For):
            if statement.kind == tirx.ForKind.PARALLEL:
                ...
                instruction_kind = self.hardware.classify(
                    _operation_info("parallel", statement, reads, writes)
                )
                self._append(
                    name,
                    statement,
        ...
        if isinstance(statement, tirx.Evaluate) and isinstance(
            ...
            reads, writes = _accessed_buffers(accesses)
            base_name = self._operation_name(
                call_name.removeprefix("tl.tileop."), reads, writes
            )
            instruction_kind = self.hardware.classify(
                _operation_info(call_name, statement, reads, writes)
            )
            self._append(
                base_name,
                statement,
        ...
        if isinstance(statement, tirx.BufferStore):
            accesses = _scalar_accesses(statement)
            reads, writes = _accessed_buffers(accesses)
            base_name = self._operation_name("store", reads, writes)
            instruction_kind = self.hardware.classify(
                _operation_info("store", statement, reads, writes)
            )
            self._append(
                base_name,
                statement,
...
def _shift(
    ...
    if iteration_delta == 0:
        return expression
    return substitute(expression, {loop_var: loop_var + iteration_delta})
...
def _kernel_thread_count(function: tirx.PrimFunc) -> int | None:
    """Return the static input CTA size before or after launch lowering."""
    ...
    def visit(node) -> None:
        ...
        if isinstance(node, tirx.For):
            binding = node.thread_binding
            if binding is None:
                return
            thread_tag = binding.thread_tag
            extent = node.extent
...
def extract_dataflow_graph(
    ...
    *,
    ...
    if hardware is None:
        ...
        if target is None:
            raise ValueError(
                "hardware cannot be inferred: pass hardware or a target"
            )
    ...
    edge_kinds: dict[
        tuple[int, int, int, int], set[DependencyKind]
    ] = {}
    ...
    def add_edge(
        ...
        edge_kinds.setdefault(
            (producer_id, consumer_id, distance, buffer_id), set()
        ).add(kind)
    ...
    for region_id, loop in enumerate(region_loops):
        ...
        if isinstance(loop.extent, tirx.IntImm) and int(loop.extent) <= 1:
            continue
        region_node_ids = {
            node.node_id for node in nodes if node.region_id == region_id
        }
        region_accesses = tuple(
            access for access in accesses if access.node_id in region_node_ids
        )
        ...
        for read in region_accesses:
            ...
            earlier_writes = (
                writer for writer in writers if writer.node_id < read.node_id
            )
            if any(_covers(writer, read, loop.loop_var) for writer in earlier_writes):
                continue
    ...
    edges = tuple(
        ...
        for (producer_id, consumer_id, distance, buffer_id), kinds in sorted(
            edge_kinds.items()
        )
    )

  ...
  if (((int)threadIdx.x) < 256) {
    ...
    for (int k = 0; k < 32; ++k) {
      ...
      if (tl::tl_shuffle_elect<256>() && ((((int)threadIdx.x) >> 5) == 0)) {
        ...
        tl::tma_load((&(((float*)lse_shared)[0])), (&(lse[((((int)blockIdx.x) * 1024) + (k * 32))])), overlap_plan_mbar[6], 128);
      }
      overlap_plan_mbar[4].wait((k & 1));
      overlap_plan_mbar[6].wait((k & 1));
      #pragma unroll
      for (int i_3 = 0; i_3 < 8; ++i_3) {
        *(float2*)(qkT + (i_3 * 2)) = *(float2*)(((float*)qkT_wsp_handoff_5) + ((((((((int)threadIdx.x) >> 5) * 512) + ((i_3 & 1) * 256)) + (((((int)threadIdx.x) & 31) >> 2) * 32)) + ((i_3 >> 1) * 8)) + ((((int)threadIdx.x) & 3) * 2)));
      }

  ...
  if (((int)threadIdx.x) < 256) {
    ...
    for (int k = 0; k < 63; ++k) {
      ...
      *(float4*)(scores_max + 0) = make_float4(broadcast_var_6, broadcast_var_6, broadcast_var_6, broadcast_var_6);
      ...
      for (int i_18 = 0; i_18 < 4; ++i_18) {
        ...
        for (int rv_2 = 0; rv_2 < 2; ++rv_2) {
          scores_max_clear_1[i_18] = max(scores_max_clear_1[i_18], acc_s_v1[((i_18 * 2) + rv_2)]);
        }
        tl::__sync_thread_partial(3, 256);
        scores_max_clear_1[i_18] = tl::AllReduce<tl::MaxOp, 256, 32, 0, tl::NamedBarrier<256>>::run(scores_max_clear_1[i_18], (&(((float*)workspace_3)[0])));
      ...
      for (int i_20 = 0; i_20 < 4; ++i_20) {
        scores_scale[i_20] = exp2f(((scores_max_prev[i_20] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/) - (scores_max[i_20] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/)));
      }
      #pragma unroll
      ...
      for (int i_22 = 0; i_22 < 4; ++i_22) {
        ...
        for (int rv_3 = 0; rv_3 < 2; ++rv_3) {
          scores_sum[i_22] = (scores_sum[i_22] + acc_s_v1[((i_22 * 2) + rv_3)]);
        }
        tl::__sync_thread_partial(3, 256);
        scores_sum[i_22] = tl::AllReduce<tl::SumOp, 256, 32, 0, tl::NamedBarrier<256>>::run(scores_sum[i_22], (&(((float*)workspace_5)[0])));
      ...
      *(float4*)(scores_max + 0) = make_float4(broadcast_var_8, broadcast_var_8, broadcast_var_8, broadcast_var_8);
      ...
      for (int i_33 = 0; i_33 < 4; ++i_33) {
        ...
        tl::__sync_thread_partial(3, 256);
        scores_max_clear_2[i_33] = tl::AllReduce<tl::MaxOp, 256, 32, 0, tl::NamedBarrier<256>>::run(scores_max_clear_2[i_33], (&(((float*)workspace_4)[0])));
      ...
      for (int i_35 = 0; i_35 < 4; ++i_35) {
        scores_scale[i_35] = exp2f(((scores_max_prev[i_35] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/) - (scores_max[i_35] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/)));
      }
      #pragma unroll
      ...
      for (int i_37 = 0; i_37 < 4; ++i_37) {
        ...
        tl::__sync_thread_partial(3, 256);
        scores_sum[i_37] = tl::AllReduce<tl::SumOp, 256, 32, 0, tl::NamedBarrier<256>>::run(scores_sum[i_37], (&(((float*)workspace_6)[0])));
      ...
      for (int i_38 = 0; i_38 < 4; ++i_38) {
        logsum[i_38] = ((logsum[i_38] * scores_scale[i_38]) + scores_sum[i_38]);
      }
      overlap_plan_mbar[3].wait((k & 1));
      {
        half_t A_local_7[16];
    ...
    *(float4*)(scores_max + 0) = make_float4(broadcast_var_10, broadcast_var_10, broadcast_var_10, broadcast_var_10);
    ...
    for (int i_48 = 0; i_48 < 4; ++i_48) {
      scores_max_clear_3[i_48] = -CUDART_INF_F;
      #pragma unroll
      for (int rv_6 = 0; rv_6 < 2; ++rv_6) {
        scores_max_clear_3[i_48] = max(scores_max_clear_3[i_48], acc_s_v1[((i_48 * 2) + rv_6)]);
      }
      tl::__sync_thread_partial(3, 256);
      scores_max_clear_3[i_48] = tl::AllReduce<tl::MaxOp, 256, 32, 0, tl::NamedBarrier<256>>::run(scores_max_clear_3[i_48], (&(((float*)workspace)[0])));
      scores_max_clear_3[i_48] = tl::AllReduce<tl::MaxOp, 4, 1, 0, tl::NamedBarrier<256>>::run(scores_max_clear_3[i_48]);
      scores_max[i_48] = max(scores_max[i_48], scores_max_clear_3[i_48]);
    }
    #pragma unroll
    ...
    for (int i_50 = 0; i_50 < 4; ++i_50) {
      scores_scale[i_50] = exp2f(((scores_max_prev[i_50] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/) - (scores_max[i_50] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/)));
    }
    #pragma unroll
    for (int i_51 = 0; i_51 < 8; ++i_51) {
      acc_s_v1[i_51] = exp2f(((acc_s_v1[i_51] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/) - (scores_max[(i_51 >> 1)] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/)));
    }
    #pragma unroll
    for (int i_52 = 0; i_52 < 4; ++i_52) {
      ...
      for (int rv_7 = 0; rv_7 < 2; ++rv_7) {
        scores_sum[i_52] = (scores_sum[i_52] + acc_s_v1[((i_52 * 2) + rv_7)]);
      }
      tl::__sync_thread_partial(3, 256);
      scores_sum[i_52] = tl::AllReduce<tl::SumOp, 256, 32, 0, tl::NamedBarrier<256>>::run(scores_sum[i_52], (&(((float*)workspace_2)[0])));
      scores_sum[i_52] = tl::AllReduce<tl::SumOp, 4, 1, 0, tl::NamedBarrier<256>>::run(scores_sum[i_52]);
    }
    tl::ptx_stmatrix_m8n8_x4((&(((half_t*)S_shared)[((((((int)threadIdx.x) & 31) >> 3) * 512) + ((((((((int)threadIdx.x) & 31) * 64) + ((((((int)threadIdx.x) >> 7) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 32)) + (((((((int)threadIdx.x) & 127) >> 6) + ((((int)threadIdx.x) & 3) >> 1)) & 1) * 16)) + (((((((int)threadIdx.x) & 63) >> 5) + (((int)threadIdx.x) & 1)) & 1) * 8)) & 511))])), __pack_half2(((half_t)acc_s_v0[0]), ((half_t)acc_s_v0[1])), __pack_half2(((half_t)acc_s_v0[2]), ((half_t)acc_s_v0[3])), __pack_half2(((half_t)acc_s_v0[4]), ((half_t)acc_s_v0[5])), __pack_half2(((half_t)acc_s_v0[6]), ((half_t)acc_s_v0[7])));
    #pragma unroll
    for (int i_53 = 0; i_53 < 4; ++i_53) {
      logsum[i_53] = ((logsum[i_53] * scores_scale[i_53]) + scores_sum[i_53]);
    }

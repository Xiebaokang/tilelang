    ...
    *(float4*)(logsum + 0) = make_float4(broadcast_var_1, broadcast_var_1, broadcast_var_1, broadcast_var_1);
    for (int k = 0; k < 128; ++k) {
      ...
      for (int i_2 = 0; i_2 < 64; ++i_2) {
        acc_o[i_2] = (acc_o[i_2] * scores_scale[(((i_2 >> 5) * 2) + ((i_2 & 3) >> 1))]);
      }
      ...
      overlap_plan_mbar[((k & 1) + 7)].wait(((k & 3) >> 1));
      {
        ...
        for (int ki = 0; ki < 4; ++ki) {
          for (int i_3 = 0; i_3 < 2; ++i_3) {
            tl::ptx_ldmatrix_x4((&(((half_t*)S_shared)[(((((k & 1) * 2048) + (i_3 * 1024)) + (((((int)threadIdx.x) & 15) >> 3) * 512)) + ((((((((int)threadIdx.x) & 15) * 64) + (((((((int)threadIdx.x) & 7) >> 2) + (ki >> 1)) & 1) * 32)) + (((((((int)threadIdx.x) & 3) >> 1) + (ki & 1)) & 1) * 16)) + (((((((int)threadIdx.x) & 31) >> 4) + (((int)threadIdx.x) & 1)) & 1) * 8)) & 511))])), (&(A_local[(i_3 * 8)])));
          }
          for (int i_4 = 0; i_4 < 4; ++i_4) {
            tl::ptx_ldmatrix_x4_trans((&(((half_t*)KV_shared)[((((((k & 1) * 32768) + ((((int)threadIdx.x) >> 5) * 4096)) + (ki * 1024)) + (((((int)threadIdx.x) & 15) >> 3) * 512)) + ((((((((int)threadIdx.x) & 15) * 64) + (((((((int)threadIdx.x) & 7) >> 2) + (i_4 >> 1)) & 1) * 32)) + (((((((int)threadIdx.x) & 3) >> 1) + (i_4 & 1)) & 1) * 16)) + (((((((int)threadIdx.x) & 31) >> 4) + (((int)threadIdx.x) & 1)) & 1) * 8)) & 511))])), (&(B_local[(i_4 * 8)])));
          }
          for (int i_5 = 0; i_5 < 2; ++i_5) {
            for (int j = 0; j < 4; ++j) {
              tl::mma_sync<tl::DataType::kFloat16, tl::DataType::kFloat16, tl::DataType::kFloat32, 16, 8, 16, false, true>(reinterpret_cast<float*>(acc_o + ((i_5 * 32) + (j * 8))), reinterpret_cast<const unsigned*>(A_local + (i_5 * 8)), reinterpret_cast<const unsigned*>(B_local + (j * 8)));
              tl::mma_sync<tl::DataType::kFloat16, tl::DataType::kFloat16, tl::DataType::kFloat32, 16, 8, 16, false, true>(reinterpret_cast<float*>(acc_o + (((i_5 * 32) + (j * 8)) + 4)), reinterpret_cast<const unsigned*>(A_local + (i_5 * 8)), reinterpret_cast<const unsigned*>(B_local + ((j * 8) + 4)));
            }
          }
        }
      }
      ...
      for (int i_6 = 0; i_6 < 4; ++i_6) {
        *(float2*)(acc_s + (i_6 * 2)) = *(float2*)(((float*)acc_s_wsp_handoff_6) + ((((i_6 * 512) + (((((int)threadIdx.x) & 31) >> 2) * 64)) + ((((int)threadIdx.x) >> 5) * 8)) + ((((int)threadIdx.x) & 3) * 2)));
      }
      #pragma unroll
      for (int i_7 = 0; i_7 < 4; ++i_7) {
        ...
        for (int rv = 0; rv < 2; ++rv) {
          scores_sum[i_7] = (scores_sum[i_7] + acc_s[((i_7 * 2) + rv)]);
        }
        tl::__sync_thread_partial(3, 256);
        scores_sum[i_7] = tl::AllReduce<tl::SumOp, 256, 32, 0, tl::NamedBarrier<256>>::run(scores_sum[i_7], (&(((float*)workspace)[0])));

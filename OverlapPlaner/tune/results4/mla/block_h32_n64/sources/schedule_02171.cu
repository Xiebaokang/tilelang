  ...
  if (((int)threadIdx.x) < 256) {
    ...
    *(float4*)(scores_max + 0) = make_float4(broadcast_var_4, broadcast_var_4, broadcast_var_4, broadcast_var_4);
    ...
    for (int i_10 = 0; i_10 < 4; ++i_10) {
      ...
      tl::__sync_thread_partial(3, 256);
      scores_sum[i_10] = tl::AllReduce<tl::SumOp, 256, 32, 0, tl::NamedBarrier<256>>::run(scores_sum[i_10], (&(((float*)workspace_2)[0])));
    ...
    for (int k = 0; k < 127; ++k) {
      overlap_plan_mbar[(((k + 1) & 1) + 2)].wait((((k + 1) & 3) >> 1));
      {
        ...
        for (int i_12 = 0; i_12 < 2; ++i_12) {
          ...
          *(float4*)(acc_s + (i_12 * 4)) = make_float4(broadcast_var_5, broadcast_var_5, broadcast_var_5, broadcast_var_5);
        }
        tl::__sync_thread_partial(3, 256);
        for (int ki_2 = 0; ki_2 < 32; ++ki_2) {
          ...
          for (int i_14 = 0; i_14 < 2; ++i_14) {
            tl::mma_sync<tl::DataType::kFloat16, tl::DataType::kFloat16, tl::DataType::kFloat32, 16, 8, 16, false, true>(reinterpret_cast<float*>(acc_s + (i_14 * 4)), reinterpret_cast<const unsigned*>(A_local_2 + (i_14 * 8)), reinterpret_cast<const unsigned*>(B_local_2 + 0));
          }
        }
      }
      overlap_plan_mbar[(((k + 1) & 1) + 4)].wait((((k + 1) & 3) >> 1));
      {
        ...
        for (int ki_3 = 0; ki_3 < 4; ++ki_3) {
          ...
          for (int i_16 = 0; i_16 < 2; ++i_16) {
            tl::mma_sync<tl::DataType::kFloat16, tl::DataType::kFloat16, tl::DataType::kFloat32, 16, 8, 16, false, true>(reinterpret_cast<float*>(acc_s + (i_16 * 4)), reinterpret_cast<const unsigned*>(A_local_3 + (i_16 * 8)), reinterpret_cast<const unsigned*>(B_local_3 + 0));
          }
        }
      }
      ...
      *(float4*)(scores_max + 0) = make_float4(broadcast_var_6, broadcast_var_6, broadcast_var_6, broadcast_var_6);
      ...
      for (int i_25 = 0; i_25 < 4; ++i_25) {
        ...
        tl::__sync_thread_partial(3, 256);
        scores_sum[i_25] = tl::AllReduce<tl::SumOp, 256, 32, 0, tl::NamedBarrier<256>>::run(scores_sum[i_25], (&(((float*)workspace_1)[0])));

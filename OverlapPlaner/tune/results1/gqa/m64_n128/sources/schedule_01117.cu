  ...
  *(float2*)(scores_max + 0) = make_float2(broadcast_var_3, broadcast_var_3);
  ...
  for (int k = 0; k < 31; ++k) {
    ...
    if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 0)) {
      ...
      tl::tma_load(K_desc, overlap_plan_mbar[1], (&(((half_t*)K_shared)[8192])), 64, ((k * 128) + 128), (((int)blockIdx.y) >> 3), 0);
    }
    if (2 <= k) {
      overlap_plan_mbar[((k & 1) + 5)].wait((((k >> 1) + 1) & 1));
    }
    if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 0)) {
      overlap_plan_mbar[((k & 1) + 2)].arrive_and_expect_tx(32768);
      tl::tma_load(V_desc, overlap_plan_mbar[((k & 1) + 2)], (&(((half_t*)V_shared)[((k & 1) * 16384)])), 0, (k * 128), (((int)blockIdx.y) >> 3), 0);
      tl::tma_load(V_desc, overlap_plan_mbar[((k & 1) + 2)], (&(((half_t*)V_shared)[(((k & 1) * 16384) + 8192)])), 64, (k * 128), (((int)blockIdx.y) >> 3), 0);
    }
    #pragma unroll

    ...
    *(float2*)(scores_max + 0) = make_float2(broadcast_var_2, broadcast_var_2);
    for (int k = 0; k < 32; ++k) {
      ...
      if (tl::tl_shuffle_elect<256>() && ((((int)threadIdx.x) >> 5) == 0)) {
        ...
        tl::tma_load(V_desc, overlap_plan_mbar[3], (&(((half_t*)V_shared)[0])), 0, (k * 128), (((int)blockIdx.y) >> 3), 0);
        tl::tma_load(V_desc, overlap_plan_mbar[3], (&(((half_t*)V_shared)[8192])), 64, (k * 128), (((int)blockIdx.y) >> 3), 0);
      }

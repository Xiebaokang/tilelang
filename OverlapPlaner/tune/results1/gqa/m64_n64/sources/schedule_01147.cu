...
extern "C" __global__ void __launch_bounds__(128, 1) main_kernel(__grid_constant__ const CUtensorMap K_desc, __grid_constant__ const CUtensorMap Output_desc, __grid_constant__ const CUtensorMap Q_desc, __grid_constant__ const CUtensorMap V_desc) {
  ...
  *(float2*)(scores_max + 0) = make_float2(broadcast_var_3, broadcast_var_3);
  ...
  for (int k = 0; k < 63; ++k) {
    ...
    if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 0)) {
      overlap_plan_mbar[1].arrive_and_expect_tx(16384);
      tl::tma_load(K_desc, overlap_plan_mbar[1], (&(((half_t*)K_shared)[0])), 0, ((k * 64) + 64), (((int)blockIdx.y) >> 3), 0);
      tl::tma_load(K_desc, overlap_plan_mbar[1], (&(((half_t*)K_shared)[4096])), 64, ((k * 64) + 64), (((int)blockIdx.y) >> 3), 0);
    }
    if (2 <= k) {
      overlap_plan_mbar[(((k + 1) % 3) + 6)].wait((((k + 4) % 6) / 3));
    }
    if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 0)) {
      overlap_plan_mbar[(((k + 1) % 3) + 2)].arrive_and_expect_tx(16384);
      tl::tma_load(V_desc, overlap_plan_mbar[(((k + 1) % 3) + 2)], (&(((half_t*)V_shared)[(((k + 1) % 3) * 8192)])), 0, ((k * 64) + 64), (((int)blockIdx.y) >> 3), 0);
      tl::tma_load(V_desc, overlap_plan_mbar[(((k + 1) % 3) + 2)], (&(((half_t*)V_shared)[((((k + 1) % 3) * 8192) + 4096)])), 64, ((k * 64) + 64), (((int)blockIdx.y) >> 3), 0);
    }
    #pragma unroll
    ...
    overlap_plan_mbar[((k % 3) + 2)].wait(((k % 6) / 3));
    {
      ...
      tl::increase_descriptor_offset<int>(desc_b_2, ((k % 3) * 16384));

...
extern "C" __global__ void __launch_bounds__(256, 1) main_kernel(__grid_constant__ const CUtensorMap K_desc, __grid_constant__ const CUtensorMap Output_desc, __grid_constant__ const CUtensorMap Q_desc, __grid_constant__ const CUtensorMap V_desc) {
  ...
  void* acc_s_wsp_handoff_2 = ((void*)((char*)buf_dyn_shmem + 81920));
  ...
  if (((int)threadIdx.x) < 128) {
    ...
    *(float2*)(scores_max + 0) = make_float2(broadcast_var_3, broadcast_var_3);
    ...
    for (int k = 0; k < 31; ++k) {
      ...
      if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 0)) {
        ...
        tl::tma_load(K_desc, overlap_plan_mbar[2], (&(((half_t*)K_shared)[8192])), 0, ((k * 128) + 64), (((int)blockIdx.y) >> 3), 0);
        tl::tma_load(K_desc, overlap_plan_mbar[2], (&(((half_t*)K_shared)[12288])), 64, ((k * 128) + 64), (((int)blockIdx.y) >> 3), 0);
      }
      *(float2*)(scores_max_prev + 0) = *(float2*)(scores_max + 0);
      ...
      *(float2*)(scores_max + 0) = make_float2(broadcast_var_4, broadcast_var_4);
      ...
      for (int i_10 = 0; i_10 < 16; ++i_10) {
        *(float2*)(acc_s + (i_10 * 2)) = *(float2*)(((float*)acc_s_wsp_handoff_2) + ((((((((int)threadIdx.x) >> 5) * 1024) + ((i_10 & 1) * 512)) + (((((int)threadIdx.x) & 31) >> 2) * 64)) + ((i_10 >> 1) * 8)) + ((((int)threadIdx.x) & 3) * 2)));
      }
      #pragma unroll
      ...
      overlap_plan_mbar[4].wait((k & 1));
      {
        ...
        for (int ki = 0; ki < 4; ++ki) {
          tl::wgmma_rs<tl::DataType::kFloat16, tl::DataType::kFloat16, tl::DataType::kFloat32, 64, 128, 16, false, true, 1, 1>(reinterpret_cast<const uint32_t*>(acc_s_cast_v0 + (ki * 8)), uint64_t(desc_b + ((ki * 2048) >> 4)), reinterpret_cast<uint32_t*>(acc_o + 0), 1);
        }
      ...
      *(float2*)(scores_max + 0) = make_float2(broadcast_var_5, broadcast_var_5);
      ...
      for (int i_19 = 0; i_19 < 16; ++i_19) {
        *(float2*)(acc_s + (i_19 * 2)) = *(float2*)(((float*)acc_s_wsp_handoff_2) + ((((((((int)threadIdx.x) >> 5) * 1024) + ((i_19 & 1) * 512)) + (((((int)threadIdx.x) & 31) >> 2) * 64)) + ((i_19 >> 1) * 8)) + ((((int)threadIdx.x) & 3) * 2)));
      }
      #pragma unroll
      ...
      overlap_plan_mbar[5].wait((k & 1));
      {
        ...
        for (int ki_1 = 0; ki_1 < 4; ++ki_1) {
          tl::wgmma_rs<tl::DataType::kFloat16, tl::DataType::kFloat16, tl::DataType::kFloat32, 64, 128, 16, false, true, 1, 1>(reinterpret_cast<const uint32_t*>(acc_s_cast_v1 + (ki_1 * 8)), uint64_t(desc_b_1 + ((ki_1 * 2048) >> 4)), reinterpret_cast<uint32_t*>(acc_o + 0), 1);
        }
        tl::warpgroup_commit_batch();
        tl::warpgroup_wait<0>();
        tl::warpgroup_fence_operand(reinterpret_cast<float*>(acc_o + 0), 64);
    ...
    if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 0)) {
      overlap_plan_mbar[2].arrive_and_expect_tx(16384);
      tl::tma_load(K_desc, overlap_plan_mbar[2], (&(((half_t*)K_shared)[8192])), 0, 4032, (((int)blockIdx.y) >> 3), 0);
      tl::tma_load(K_desc, overlap_plan_mbar[2], (&(((half_t*)K_shared)[12288])), 64, 4032, (((int)blockIdx.y) >> 3), 0);
    }
    *(float2*)(scores_max_prev + 0) = *(float2*)(scores_max + 0);
    ...
    *(float2*)(scores_max + 0) = make_float2(broadcast_var_6, broadcast_var_6);
    ...
    for (int i_28 = 0; i_28 < 16; ++i_28) {
      *(float2*)(acc_s + (i_28 * 2)) = *(float2*)(((float*)acc_s_wsp_handoff_2) + ((((((((int)threadIdx.x) >> 5) * 1024) + ((i_28 & 1) * 512)) + (((((int)threadIdx.x) & 31) >> 2) * 64)) + ((i_28 >> 1) * 8)) + ((((int)threadIdx.x) & 3) * 2)));
    }
    #pragma unroll
    ...
    overlap_plan_mbar[5].wait(1);
    {
      ...
      for (int ki_3 = 0; ki_3 < 4; ++ki_3) {
        tl::wgmma_rs<tl::DataType::kFloat16, tl::DataType::kFloat16, tl::DataType::kFloat32, 64, 128, 16, false, true, 1, 1>(reinterpret_cast<const uint32_t*>(acc_s_cast_v1 + (ki_3 * 8)), uint64_t(desc_b_3 + ((ki_3 * 2048) >> 4)), reinterpret_cast<uint32_t*>(acc_o + 0), 1);
      }
      tl::warpgroup_commit_batch();
      tl::warpgroup_wait<0>();
      tl::warpgroup_fence_operand(reinterpret_cast<float*>(acc_o + 0), 64);
    ...
    for (int k_1 = 0; k_1 < 64; ++k_1) {
      ...
      overlap_plan_mbar[((k_1 & 1) + 1)].wait(((k_1 & 3) >> 1));
      {
        ...
        tl::increase_descriptor_offset<int>(desc_b_4, ((k_1 & 1) * 16384));
        tl::warpgroup_fence_operand(reinterpret_cast<float*>(acc_s + 0), 32);
        tl::warpgroup_arrive();
        tl::__sync_thread_partial(4, 128);
        #pragma unroll
        for (int ki_4 = 0; ki_4 < 8; ++ki_4) {
          tl::wgmma_ss<tl::DataType::kFloat16, tl::DataType::kFloat16, tl::DataType::kFloat32, 64, 64, 16, false, false, 1, 1>(uint64_t(desc_a + ((((ki_4 >> 2) * 8192) + ((ki_4 & 3) * 32)) >> 4)), uint64_t(desc_b_4 + ((((ki_4 >> 2) * 8192) + ((ki_4 & 3) * 32)) >> 4)), ((uint32_t*)(acc_s + 0)), 1);
        }
        tl::warpgroup_commit_batch();
        tl::warpgroup_wait<0>();
        tl::warpgroup_fence_operand(reinterpret_cast<float*>(acc_s + 0), 32);
      ...
      for (int i_40 = 0; i_40 < 16; ++i_40) {
        *(float2*)(((float*)acc_s_wsp_handoff_2) + (((((((((int)threadIdx.x) >> 5) * 1024) + ((i_40 & 1) * 512)) + (((((int)threadIdx.x) & 31) >> 2) * 64)) + ((i_40 >> 1) * 8)) + ((((int)threadIdx.x) & 3) * 2)) - 4096)) = *(float2*)(acc_s + (i_40 * 2));
      }
      tl::fence_proxy_async();

...
extern "C" __global__ void __launch_bounds__(256, 1) main_kernel(__grid_constant__ const CUtensorMap K_desc, __grid_constant__ const CUtensorMap Output_desc, __grid_constant__ const CUtensorMap Q_desc, __grid_constant__ const CUtensorMap V_desc) {
  ...
  if (((int)threadIdx.x) < 128) {
    ...
    for (int k = 0; k < 64; ++k) {
      ...
      if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 0)) {
        overlap_plan_mbar[((k & 1) + 3)].arrive_and_expect_tx(16384);
        tl::tma_load(V_desc, overlap_plan_mbar[((k & 1) + 3)], (&(((half_t*)V_shared)[((k & 1) * 8192)])), 0, (k * 64), (((int)blockIdx.y) >> 3), 0);
        tl::tma_load(V_desc, overlap_plan_mbar[((k & 1) + 3)], (&(((half_t*)V_shared)[(((k & 1) * 8192) + 4096)])), 64, (k * 64), (((int)blockIdx.y) >> 3), 0);
      }
      #pragma unroll
      for (int i = 0; i < 32; ++i) {

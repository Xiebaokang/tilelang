  ...
  *(float2*)(scores_max_prev + 0) = *(float2*)(scores_max + 0);
  for (int k = 0; k < 63; ++k) {
    ...
    if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 0)) {
      ...
      tl::tma_load(V_desc, overlap_plan_mbar[2], (&(((half_t*)V_shared)[0])), 0, (k * 64), (((int)blockIdx.y) >> 3), 0);
      tl::tma_load(V_desc, overlap_plan_mbar[2], (&(((half_t*)V_shared)[4096])), 64, (k * 64), (((int)blockIdx.y) >> 3), 0);
    }
    #pragma unroll

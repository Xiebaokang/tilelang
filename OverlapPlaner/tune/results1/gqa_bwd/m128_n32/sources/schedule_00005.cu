...
extern "C" __global__ void __launch_bounds__(256, 1) main_kernel(const float* __restrict__ Delta, __grid_constant__ const CUtensorMap K_desc, __grid_constant__ const CUtensorMap Q_desc, __grid_constant__ const CUtensorMap V_desc, __grid_constant__ const CUtensorMap dK_desc, __grid_constant__ const CUtensorMap dO_desc, float* __restrict__ dQ, __grid_constant__ const CUtensorMap dV_desc, const float* __restrict__ lse) {
  ...
  if (tl::tl_shuffle_elect<256>() && ((((int)threadIdx.x) >> 5) == 0)) {
    ...
    tl::tma_load(dO_desc, overlap_plan_mbar[3], (&(((half_t*)do_1)[0])), 0, 0, ((int)blockIdx.x), 0);
  }
  #pragma unroll
  ...
  if (tl::tl_shuffle_elect<256>() && ((((int)threadIdx.x) >> 5) == 0)) {
    overlap_plan_mbar[3].arrive_and_expect_tx(4096);
    tl::tma_load(dO_desc, overlap_plan_mbar[3], (&(((half_t*)do_1)[0])), 0, 32, ((int)blockIdx.x), 0);
  }
  #pragma unroll
  ...
  if (tl::tl_shuffle_elect<256>() && ((((int)threadIdx.x) >> 5) == 0)) {
    overlap_plan_mbar[4].arrive_and_expect_tx(128);
    tl::tma_load((&(((float*)lse_shared)[0])), (&(lse[((((int)blockIdx.x) * 1024) + 32)])), overlap_plan_mbar[4], 128);
  }
  ...
  for (int i_10 = 0; i_10 < 8; ++i_10) {
    ...
    float2 __10;
      ...
      float2 v__10 = *(float2*)(((float*)lse_shared) + (((i_10 >> 1) * 8) + ((((int)threadIdx.x) & 3) * 2)));
  ...
  for (int k = 0; k < 15; ++k) {
    ...
    if (tl::tl_shuffle_elect<256>() && ((((int)threadIdx.x) >> 5) == 0)) {
      overlap_plan_mbar[3].arrive_and_expect_tx(4096);
      tl::tma_load(dO_desc, overlap_plan_mbar[3], (&(((half_t*)do_1)[0])), 0, ((k * 64) + 64), ((int)blockIdx.x), 0);
    }
    #pragma unroll
    for (int i_16 = 0; i_16 < 4; ++i_16) {
      float broadcast_var_12 = 0x0p+0f/*0.000000e+00*/;
      *(float4*)(dsT + (i_16 * 4)) = make_float4(broadcast_var_12, broadcast_var_12, broadcast_var_12, broadcast_var_12);
    }
    overlap_plan_mbar[3].wait(0);
    {
    ...
    if (tl::tl_shuffle_elect<256>() && ((((int)threadIdx.x) >> 5) == 0)) {
      overlap_plan_mbar[3].arrive_and_expect_tx(4096);
      tl::tma_load(dO_desc, overlap_plan_mbar[3], (&(((half_t*)do_1)[0])), 0, ((k * 64) + 96), ((int)blockIdx.x), 0);
    }
    #pragma unroll
    for (int i_24 = 0; i_24 < 4; ++i_24) {
      float broadcast_var_17 = 0x0p+0f/*0.000000e+00*/;
      *(float4*)(dsT + (i_24 * 4)) = make_float4(broadcast_var_17, broadcast_var_17, broadcast_var_17, broadcast_var_17);
    }
    overlap_plan_mbar[3].wait(1);
    {
    ...
    if (tl::tl_shuffle_elect<256>() && ((((int)threadIdx.x) >> 5) == 0)) {
      overlap_plan_mbar[4].arrive_and_expect_tx(128);
      tl::tma_load((&(((float*)lse_shared)[0])), (&(lse[(((((int)blockIdx.x) * 1024) + (k * 64)) + 96)])), overlap_plan_mbar[4], 128);
    }
    overlap_plan_mbar[4].wait(1);
    __syncthreads();
    #pragma unroll

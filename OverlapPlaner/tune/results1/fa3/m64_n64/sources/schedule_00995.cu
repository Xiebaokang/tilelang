    ...
    *(float2*)(scores_max + 0) = make_float2(broadcast_var_2, broadcast_var_2);
    for (int k = 0; k < 128; ++k) {
      ...
      *(float2*)(scores_max + 0) = make_float2(broadcast_var_3, broadcast_var_3);
      ...
      overlap_plan_mbar[((k % 3) + 8)].arrive();
      #pragma unroll
    ...
    for (int k_1 = 0; k_1 < 128; ++k_1) {
      if (1 <= k_1) {
        overlap_plan_mbar[7].wait(((k_1 + 1) & 1));
      }
      if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 4)) {
        overlap_plan_mbar[1].arrive_and_expect_tx(16384);
        tl::tma_load(K_desc, overlap_plan_mbar[1], (&(((half_t*)K_shared)[0])), 0, (k_1 * 64), ((int)blockIdx.y), 0);
        tl::tma_load(K_desc, overlap_plan_mbar[1], (&(((half_t*)K_shared)[4096])), 64, (k_1 * 64), ((int)blockIdx.y), 0);
      }
      if (3 <= k_1) {
      ...
      if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 4)) {
        overlap_plan_mbar[((k_1 % 3) + 3)].arrive_and_expect_tx(16384);
        tl::tma_load(V_desc, overlap_plan_mbar[((k_1 % 3) + 3)], (&(((half_t*)V_shared)[((k_1 % 3) * 8192)])), 0, (k_1 * 64), ((int)blockIdx.y), 0);
        tl::tma_load(V_desc, overlap_plan_mbar[((k_1 % 3) + 3)], (&(((half_t*)V_shared)[(((k_1 % 3) * 8192) + 4096)])), 64, (k_1 * 64), ((int)blockIdx.y), 0);
      }
      #pragma unroll
      ...
      overlap_plan_mbar[1].wait((k_1 & 1));
      {
        ...
        for (int ki_1 = 0; ki_1 < 8; ++ki_1) {
          tl::wgmma_ss<tl::DataType::kFloat16, tl::DataType::kFloat16, tl::DataType::kFloat32, 64, 64, 16, false, false, 1, 1>(uint64_t(desc_a + ((((ki_1 >> 2) * 8192) + ((ki_1 & 3) * 32)) >> 4)), uint64_t(desc_b_1 + ((((ki_1 >> 2) * 8192) + ((ki_1 & 3) * 32)) >> 4)), ((uint32_t*)(acc_s + 0)), 1);
        }
        tl::warpgroup_commit_batch();
        tl::warpgroup_wait<0>();
        tl::warpgroup_fence_operand(reinterpret_cast<float*>(acc_s + 0), 32);
      ...
      for (int i_13 = 0; i_13 < 16; ++i_13) {
        *(float2*)(((float*)acc_s_wsp_handoff_2) + (((((((((int)threadIdx.x) >> 5) * 1024) + ((i_13 & 1) * 512)) + (((((int)threadIdx.x) & 31) >> 2) * 64)) + ((i_13 >> 1) * 8)) + ((((int)threadIdx.x) & 3) * 2)) - 4096)) = *(float2*)(acc_s + (i_13 * 2));
      }
      tl::fence_proxy_async();

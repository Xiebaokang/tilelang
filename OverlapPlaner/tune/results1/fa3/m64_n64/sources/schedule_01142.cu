  ...
  *(float2*)(scores_max + 0) = make_float2(broadcast_var_3, broadcast_var_3);
  ...
  for (int k = 0; k < 127; ++k) {
    ...
    if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 0)) {
      ...
      tl::tma_load(V_desc, overlap_plan_mbar[((k & 1) + 2)], (&(((half_t*)V_shared)[((k & 1) * 8192)])), 0, (k * 64), ((int)blockIdx.y), 0);
      tl::tma_load(V_desc, overlap_plan_mbar[((k & 1) + 2)], (&(((half_t*)V_shared)[(((k & 1) * 8192) + 4096)])), 64, (k * 64), ((int)blockIdx.y), 0);
    }
    #pragma unroll

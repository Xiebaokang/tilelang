    ...
    *(float2*)(scores_max + 0) = make_float2(broadcast_var_2, broadcast_var_2);
    for (int k = 0; k < 64; ++k) {
      ...
      if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 0)) {
        ...
        tl::tma_load(V_desc, overlap_plan_mbar[4], (&(((half_t*)V_shared)[8192])), 64, (k * 128), ((int)blockIdx.y), 0);
      }
      ...
      for (int k_1 = 0; k_1 < 64; ++k_1) {
        ...
        overlap_plan_mbar[((k_1 & 1) + 1)].wait(((k_1 & 3) >> 1));
        {
          ...
          for (int ki_1 = 0; ki_1 < 8; ++ki_1) {
            tl::wgmma_ss<tl::DataType::kFloat16, tl::DataType::kFloat16, tl::DataType::kFloat32, 64, 128, 16, false, false, 1, 1>(uint64_t(desc_a + ((((ki_1 >> 2) * 8192) + ((ki_1 & 3) * 32)) >> 4)), uint64_t(desc_b_1 + ((((ki_1 >> 2) * 16384) + ((ki_1 & 3) * 32)) >> 4)), ((uint32_t*)(acc_s + 0)), 1);
          }
          tl::warpgroup_commit_batch();
          tl::warpgroup_wait<0>();
          tl::warpgroup_fence_operand(reinterpret_cast<float*>(acc_s + 0), 64);
        ...
        for (int i_13 = 0; i_13 < 32; ++i_13) {
          *(float2*)(((float*)acc_s_wsp_handoff_2) + (((((((((int)threadIdx.x) >> 5) * 2048) + ((i_13 & 1) * 1024)) + (((((int)threadIdx.x) & 31) >> 2) * 128)) + ((i_13 >> 1) * 8)) + ((((int)threadIdx.x) & 3) * 2)) - 8192)) = *(float2*)(acc_s + (i_13 * 2));
        }
        tl::fence_proxy_async();

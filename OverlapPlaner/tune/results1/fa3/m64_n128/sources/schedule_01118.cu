...
extern "C" __global__ void __launch_bounds__(256, 1) main_kernel(__grid_constant__ const CUtensorMap K_desc, half_t* __restrict__ Output, __grid_constant__ const CUtensorMap Q_desc, __grid_constant__ const CUtensorMap V_desc) {
  ...
  if (((int)threadIdx.x) < 128) {
    ...
    *(float2*)(scores_max + 0) = make_float2(broadcast_var_3, broadcast_var_3);
    ...
    for (int k = 0; k < 63; ++k) {
      if (1 <= k) {
        overlap_plan_mbar[(((k + 1) & 1) + 7)].wait((((k + 3) & 3) >> 1));
      }
      if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 0)) {
        overlap_plan_mbar[(((k + 1) & 1) + 3)].arrive_and_expect_tx(32768);
        tl::tma_load(V_desc, overlap_plan_mbar[(((k + 1) & 1) + 3)], (&(((half_t*)V_shared)[(((k + 1) & 1) * 16384)])), 0, ((k * 128) + 128), ((int)blockIdx.y), 0);
        tl::tma_load(V_desc, overlap_plan_mbar[(((k + 1) & 1) + 3)], (&(((half_t*)V_shared)[((((k + 1) & 1) * 16384) + 8192)])), 64, ((k * 128) + 128), ((int)blockIdx.y), 0);
      }
      #pragma unroll

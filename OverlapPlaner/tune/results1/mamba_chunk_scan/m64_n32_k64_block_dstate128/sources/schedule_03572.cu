  ...
  if (((int)threadIdx.x) < 128) {
    ...
    for (int ik = 0; ik < ((((int)blockIdx.y) >> 1) + 1); ++ik) {
      ...
      if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 0)) {
        ...
        tl::tma_load(CB_desc, overlap_plan_mbar[3], (&(((half_t*)cb_shared)[0])), (ik * 64), ((((int)blockIdx.y) >> 1) * 64), 0, (((int)blockIdx.z) >> 3), (((int)blockIdx.z) & 7));
      }
      if (1 <= ik) {
    ...
    for (int i_14 = 0; i_14 < 8; ++i_14) {
      *(float2*)(((float*)acc_wsp_handoff_2) + (((((((((int)threadIdx.x) >> 5) * 512) + ((i_14 & 1) * 256)) + (((((int)threadIdx.x) & 31) >> 2) * 32)) + ((i_14 >> 1) * 8)) + ((((int)threadIdx.x) & 3) * 2)) - 2048)) = *(float2*)(acc + (i_14 * 2));
    }
    tl::fence_proxy_async();

...
extern "C" __global__ void __launch_bounds__(256, 1) main_kernel(__grid_constant__ const CUtensorMap CB_desc, __grid_constant__ const CUtensorMap C_desc, const half_t* __restrict__ D, const half_t* __restrict__ DA, const half_t* __restrict__ Dt, __grid_constant__ const CUtensorMap Output_desc, __grid_constant__ const CUtensorMap Prev_desc, __grid_constant__ const CUtensorMap X_desc) {
  ...
  if (((int)threadIdx.x) < 128) {
    ...
    for (int i_4 = 0; i_4 < 16; ++i_4) {
      *(uint1*)(cb_local + (i_4 * 2)) = *(uint1*)(((half_t*)cb_shared) + ((((((((((int)threadIdx.x) >> 5) * 1024) + ((i_4 & 1) * 512)) + (((((int)threadIdx.x) & 31) >> 2) * 64)) + (((((((int)threadIdx.x) & 31) >> 4) + (i_4 >> 3)) & 1) * 32)) + (((((((int)threadIdx.x) & 15) >> 3) + ((i_4 & 7) >> 2)) & 1) * 16)) + (((((((int)threadIdx.x) & 7) >> 2) + ((i_4 & 3) >> 1)) & 1) * 8)) + ((((int)threadIdx.x) & 3) * 2)));
    }
    ...
    for (int ik = 0; ik < (((int)blockIdx.y) >> 1); ++ik) {
      ...
      for (int i_10 = 0; i_10 < 16; ++i_10) {
        *(uint1*)(cb_local + (i_10 * 2)) = *(uint1*)(((half_t*)cb_shared) + ((((((((((ik + 1) & 1) * 4096) + ((((int)threadIdx.x) >> 5) * 1024)) + ((i_10 & 1) * 512)) + (((((int)threadIdx.x) & 31) >> 2) * 64)) + (((((((int)threadIdx.x) & 31) >> 4) + (i_10 >> 3)) & 1) * 32)) + (((((((int)threadIdx.x) & 15) >> 3) + ((i_10 & 7) >> 2)) & 1) * 16)) + (((((((int)threadIdx.x) & 7) >> 2) + ((i_10 & 3) >> 1)) & 1) * 8)) + ((((int)threadIdx.x) & 3) * 2)));
      }
    ...
    overlap_plan_mbar[(((((int)blockIdx.y) & 3) >> 1) + 7)].wait((((int)blockIdx.y) >> 2));
    {
      ...
      tl::increase_descriptor_offset<int>(desc_b_2, (((((int)blockIdx.y) & 3) >> 1) * 4096));
      tl::warpgroup_fence_operand(reinterpret_cast<uint32_t*>(cb_local + 0), 16);
      tl::warpgroup_fence_operand(reinterpret_cast<float*>(acc + 0), 16);
      tl::warpgroup_arrive();
      #pragma unroll
      for (int ki_2 = 0; ki_2 < 4; ++ki_2) {
        tl::wgmma_rs<tl::DataType::kFloat16, tl::DataType::kFloat16, tl::DataType::kFloat32, 64, 32, 16, false, true, 1, 1>(reinterpret_cast<const uint32_t*>(cb_local + (ki_2 * 8)), uint64_t(desc_b_2 + ((ki_2 * 1024) >> 4)), reinterpret_cast<uint32_t*>(acc + 0), 1);
      }
      tl::warpgroup_commit_batch();
      tl::warpgroup_wait<0>();
      tl::warpgroup_fence_operand(reinterpret_cast<float*>(acc + 0), 16);
    ...
    overlap_plan_mbar[(((((int)blockIdx.y) & 3) >> 1) + 14)].arrive();
    ...
    for (int ik_1 = 0; ik_1 < ((((int)blockIdx.y) >> 1) + 1); ++ik_1) {
      ...
      if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 4)) {
        ...
        tl::tma_load(CB_desc, overlap_plan_mbar[((ik_1 & 1) + 3)], (&(((half_t*)cb_shared)[((ik_1 & 1) * 4096)])), (ik_1 * 64), ((((int)blockIdx.y) >> 1) * 64), 0, (((int)blockIdx.z) >> 3), (((int)blockIdx.z) & 7));
      }
    }
  }
}

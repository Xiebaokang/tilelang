    ...
    if (((int)threadIdx.x) < 256) {
      ...
      for (int ik_1 = 0; ik_1 < (((int)blockIdx.y) + 1); ++ik_1) {
        if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 4)) {
          overlap_plan_mbar[(ik_1 + 5)].arrive_and_expect_tx(16384);
          tl::tma_load(X_desc, overlap_plan_mbar[(ik_1 + 5)], (&(((half_t*)x_shared)[(ik_1 * 8192)])), 0, (((((int)blockIdx.z) >> 3) * 256) + (ik_1 * 128)), ((int)blockIdx.x), (((int)blockIdx.z) & 7));
        }
      }
    } else {
      tl::warpgroup_reg_dealloc<24>();

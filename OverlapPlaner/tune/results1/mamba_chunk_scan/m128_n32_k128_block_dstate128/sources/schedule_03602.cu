  ...
  if (((int)threadIdx.x) < 128) {
    ...
    for (int ik_1 = 0; ik_1 < ((((int)blockIdx.y) >> 1) + 1); ++ik_1) {
      if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 4)) {
        overlap_plan_mbar[(ik_1 + 3)].arrive_and_expect_tx(256);
        tl::tma_load((&(((half_t*)da_k_shared)[(ik_1 * 128)])), (&(DA[(((((((int)blockIdx.z) & 7) * 327680) + (((int)blockIdx.x) * 4096)) + ((((int)blockIdx.z) >> 3) * 256)) + (ik_1 * 128))])), overlap_plan_mbar[(ik_1 + 3)], 256);
      }
    }
  }
}

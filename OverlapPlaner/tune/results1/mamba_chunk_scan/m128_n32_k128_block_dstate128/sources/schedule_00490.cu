  ...
  if (((int)threadIdx.x) < 128) {
    ...
    for (int ik_1 = 0; ik_1 < ((((int)blockIdx.y) >> 1) + 1); ++ik_1) {
      if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 4)) {
        ...
        tl::tma_load(X_desc, overlap_plan_mbar[(ik_1 + 5)], (&(((half_t*)x_shared)[(ik_1 * 4096)])), ((((int)blockIdx.y) & 1) * 32), (((((int)blockIdx.z) >> 3) * 256) + (ik_1 * 128)), ((int)blockIdx.x), (((int)blockIdx.z) & 7));
      }
    }
  }
}

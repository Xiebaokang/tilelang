  ...
  if (((int)threadIdx.x) < 128) {
    ...
    for (int ik = 0; ik < (((int)blockIdx.y) >> 1); ++ik) {
      if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 4)) {
        ...
        overlap_plan_mbar[(ik + 8)].arrive_and_expect_tx(8192);
        tl::tma_load(X_desc, overlap_plan_mbar[(ik + 8)], (&(((half_t*)x_shared)[(ik * 4096)])), ((((int)blockIdx.y) & 1) * 32), (((((int)blockIdx.z) >> 3) * 256) + (ik * 128)), ((int)blockIdx.x), (((int)blockIdx.z) & 7));
      }
    }
    if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 4)) {

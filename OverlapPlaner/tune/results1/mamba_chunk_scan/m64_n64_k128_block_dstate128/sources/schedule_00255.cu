...
extern "C" __global__ void __launch_bounds__(256, 1) main_kernel(__grid_constant__ const CUtensorMap CB_desc, __grid_constant__ const CUtensorMap C_desc, const half_t* __restrict__ D, const half_t* __restrict__ DA, const half_t* __restrict__ Dt, __grid_constant__ const CUtensorMap Output_desc, __grid_constant__ const CUtensorMap Prev_desc, __grid_constant__ const CUtensorMap X_desc, __grid_constant__ const CUtensorMap X_desc_1) {
  ...
  void* cb_shared = ((void*)((char*)buf_dyn_shmem + 65536));
  ...
  if (((int)threadIdx.x) < 128) {
    ...
    if (1 < ((int)blockIdx.y)) {
      ...
      for (int i_15 = 0; i_15 < 64; ++i_15) {
        half_t condval_1;
        if (((((((i_15 >> 2) * 8) + ((((int)threadIdx.x) & 3) * 2)) + (i_15 & 1)) + 192) <= ((((((int)blockIdx.y) * 64) + ((((int)threadIdx.x) >> 5) * 16)) + (((i_15 & 3) >> 1) * 8)) + ((((int)threadIdx.x) & 31) >> 2)))) {
    ...
    if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 4)) {
      ...
      tl::tma_load(X_desc_1, overlap_plan_mbar[5], (&(((half_t*)x_shared)[0])), 0, ((((int)blockIdx.z) >> 3) * 256), ((int)blockIdx.x), (((int)blockIdx.z) & 7));
      overlap_plan_mbar[((((int)blockIdx.y) >> 1) + 5)].arrive_and_expect_tx(16384);
      tl::tma_load(X_desc_1, overlap_plan_mbar[((((int)blockIdx.y) >> 1) + 5)], (&(((half_t*)x_shared)[((((int)blockIdx.y) >> 1) * 8192)])), 0, (((((int)blockIdx.z) >> 3) * 256) + ((((int)blockIdx.y) >> 1) * 128)), ((int)blockIdx.x), (((int)blockIdx.z) & 7));
    }
    tl::__sync_thread_partial(3, 128);
    if (2 <= ((int)blockIdx.y)) {
    ...
    for (int i_16 = 0; i_16 < 16; ++i_16) {
      ...
      *(uint1*)(residual_shared_local_cast_4 + 0) = *(uint1*)(((half_t*)residual_shared) + (((((((((((int)threadIdx.x) >> 5) * 1024) + ((i_16 & 1) * 512)) + (((((int)threadIdx.x) & 31) >> 2) * 64)) + (((((((int)threadIdx.x) & 31) >> 4) + (i_16 >> 3)) & 1) * 32)) + (((((((int)threadIdx.x) & 15) >> 3) + ((i_16 & 7) >> 2)) & 1) * 16)) + (((((((int)threadIdx.x) & 7) >> 2) + ((i_16 & 3) >> 1)) & 1) * 8)) + ((((int)threadIdx.x) & 3) * 2)) - 4096));
      float2 __23;

  ...
  if (((int)threadIdx.x) < 128) {
    ...
    for (int ik = 0; ik < (((int)blockIdx.y) >> 1); ++ik) {
      if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 4)) {
        ...
        tl::tma_load(X_desc_1, overlap_plan_mbar[(ik + 9)], (&(((half_t*)x_shared)[(ik * 8192)])), 0, (((((int)blockIdx.z) >> 3) * 256) + (ik * 128)), ((int)blockIdx.x), (((int)blockIdx.z) & 7));
      }
    }
    if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 4)) {
      overlap_plan_mbar[((((int)blockIdx.y) >> 1) + 9)].arrive_and_expect_tx(16384);
      tl::tma_load(X_desc_1, overlap_plan_mbar[((((int)blockIdx.y) >> 1) + 9)], (&(((half_t*)x_shared)[((((int)blockIdx.y) >> 1) * 8192)])), 0, (((((int)blockIdx.z) >> 3) * 256) + ((((int)blockIdx.y) >> 1) * 128)), ((int)blockIdx.x), (((int)blockIdx.z) & 7));
    }
  }
}

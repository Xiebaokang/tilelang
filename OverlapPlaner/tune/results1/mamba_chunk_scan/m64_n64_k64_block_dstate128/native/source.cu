  ...
  if (((int)threadIdx.x) < 128) {
    ...
    for (int ik = 0; ik < (((int)blockIdx.y) + 1); ++ik) {
      ...
      if (tl::tl_shuffle_elect<128>()) {
        ...
        tl::tma_load(CB_desc, mbarrier[(ik & 1)], (&(((half_t*)cb_shared)[((ik & 1) * 4096)])), (ik * 64), (((int)blockIdx.y) * 64), 0, (((int)blockIdx.z) >> 3), (((int)blockIdx.z) & 7));
      }
    ...
    for (int ik_1 = 0; ik_1 < (((int)blockIdx.y) + 1); ++ik_1) {
      ...
      for (int i_11 = 0; i_11 < 32; ++i_11) {
        half_t condval;
        if (((((((ik_1 * 64) + ((i_11 >> 2) * 8)) + ((((int)threadIdx.x) & 3) * 2)) + (i_11 & 1)) + 64) <= ((((((int)blockIdx.y) * 64) + ((((int)threadIdx.x) >> 5) * 16)) + (((i_11 & 3) >> 1) * 8)) + ((((int)threadIdx.x) & 31) >> 2)))) {

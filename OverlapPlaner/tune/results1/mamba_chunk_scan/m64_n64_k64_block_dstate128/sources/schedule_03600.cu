...
extern "C" __global__ void __launch_bounds__(256, 1) main_kernel(__grid_constant__ const CUtensorMap CB_desc, __grid_constant__ const CUtensorMap C_desc, const half_t* __restrict__ D, const half_t* __restrict__ DA, const half_t* __restrict__ Dt, __grid_constant__ const CUtensorMap Output_desc, __grid_constant__ const CUtensorMap Prev_desc, __grid_constant__ const CUtensorMap X_desc) {
  ...
  if (((int)threadIdx.x) < 128) {
    ...
    for (int i_2 = 0; i_2 < 16; ++i_2) {
      *(float2*)(acc + (i_2 * 2)) = *(float2*)(((float*)acc_wsp_handoff_2) + ((((((((int)threadIdx.x) >> 5) * 1024) + ((i_2 & 1) * 512)) + (((((int)threadIdx.x) & 31) >> 2) * 64)) + ((i_2 >> 1) * 8)) + ((((int)threadIdx.x) & 3) * 2)));
    }
    #pragma unroll
    ...
    for (int ik = 0; ik < ((int)blockIdx.y); ++ik) {
      ...
      if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 0)) {
        overlap_plan_mbar[3].arrive_and_expect_tx(8192);
        tl::tma_load(CB_desc, overlap_plan_mbar[3], (&(((half_t*)cb_shared)[0])), ((ik * 64) + 64), (((int)blockIdx.y) * 64), 0, (((int)blockIdx.z) >> 3), (((int)blockIdx.z) & 7));
      }
    ...
    for (int i_20 = 0; i_20 < 16; ++i_20) {
      *(float2*)(((float*)acc_wsp_handoff_2) + (((((((((int)threadIdx.x) >> 5) * 1024) + ((i_20 & 1) * 512)) + (((((int)threadIdx.x) & 31) >> 2) * 64)) + ((i_20 >> 1) * 8)) + ((((int)threadIdx.x) & 3) * 2)) - 4096)) = *(float2*)(acc + (i_20 * 2));
    }
    tl::fence_proxy_async();

  ...
  if (((int)threadIdx.x) < 128) {
    ...
    for (int ik = 0; ik < ((int)blockIdx.y); ++ik) {
      ...
      for (int i_10 = 0; i_10 < 16; ++i_10) {
        *(uint1*)(cb_local + (i_10 * 2)) = *(uint1*)(((half_t*)cb_shared) + ((((((((((ik + 1) % 3) * 4096) + ((((int)threadIdx.x) >> 5) * 1024)) + ((i_10 & 1) * 512)) + (((((int)threadIdx.x) & 31) >> 2) * 64)) + (((((((int)threadIdx.x) & 31) >> 4) + (i_10 >> 3)) & 1) * 32)) + (((((((int)threadIdx.x) & 15) >> 3) + ((i_10 & 7) >> 2)) & 1) * 16)) + (((((((int)threadIdx.x) & 7) >> 2) + ((i_10 & 3) >> 1)) & 1) * 8)) + ((((int)threadIdx.x) & 3) * 2)));
      }
    ...
    for (int ik_1 = 0; ik_1 < (((int)blockIdx.y) + 1); ++ik_1) {
      ...
      if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 4)) {
        overlap_plan_mbar[((ik_1 % 3) + 2)].arrive_and_expect_tx(8192);
        tl::tma_load(CB_desc, overlap_plan_mbar[((ik_1 % 3) + 2)], (&(((half_t*)cb_shared)[((ik_1 % 3) * 4096)])), (ik_1 * 64), (((int)blockIdx.y) * 64), 0, (((int)blockIdx.z) >> 3), (((int)blockIdx.z) & 7));
      }
      if (2 <= ik_1) {

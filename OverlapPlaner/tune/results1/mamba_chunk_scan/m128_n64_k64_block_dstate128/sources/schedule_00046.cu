  ...
  if (((int)threadIdx.x) < 128) {
    ...
    for (int ik = 0; ik < ((((int)blockIdx.y) * 2) + 1); ++ik) {
      ...
      for (int i_12 = 0; i_12 < 8; ++i_12) {
        tl::ptx_ldmatrix_x4((&(((half_t*)cb_shared)[(((((i_12 & 1) * 4096) + ((((int)threadIdx.x) >> 5) * 1024)) + (((((int)threadIdx.x) & 15) >> 3) * 512)) + ((((((((int)threadIdx.x) & 15) * 64) + ((((i_12 >> 2) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 32)) + (((((i_12 & 3) >> 1) + ((((int)threadIdx.x) & 3) >> 1)) & 1) * 16)) + (((((((int)threadIdx.x) & 31) >> 4) + (((int)threadIdx.x) & 1)) & 1) * 8)) & 511))])), (&(cb_local[(i_12 * 8)])));
      }
      overlap_plan_mbar[8].arrive();

  ...
  if (((int)threadIdx.x) < 256) {
    ...
    *(float2*)(scores_max + 0) = make_float2(broadcast_var_3, broadcast_var_3);
    ...
    for (int i_1 = 0; i_1 < 2; ++i_1) {
      ...
      scores_max_clear[i_1] = tl::AllReduce<tl::MaxOp, 256, 128, 0, tl::NamedBarrier<256>>::run(scores_max_clear[i_1], (&(((float*)workspace_7)[0])));
    ...
    for (int k = 0; k < 63; ++k) {
      ...
      *(float2*)(scores_max + 0) = make_float2(broadcast_var_5, broadcast_var_5);
      ...
      for (int i_19 = 0; i_19 < 2; ++i_19) {
        ...
        tl::__sync_thread_partial(3, 256);
        scores_sum[i_19] = tl::AllReduce<tl::SumOp, 256, 128, 0, tl::NamedBarrier<256>>::run(scores_sum[i_19], (&(((float*)workspace_6)[0])));
    ...
    *(float2*)(scores_max + 0) = make_float2(broadcast_var_6, broadcast_var_6);
    ...
    for (int i_27 = 0; i_27 < 2; ++i_27) {
      ...
      tl::__sync_thread_partial(3, 256);
      scores_sum[i_27] = tl::AllReduce<tl::SumOp, 256, 128, 0, tl::NamedBarrier<256>>::run(scores_sum[i_27], (&(((float*)workspace_5)[0])));

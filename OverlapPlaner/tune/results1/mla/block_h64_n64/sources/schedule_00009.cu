  ...
  for (int k = 0; k < 63; ++k) {
    ...
    *(float2*)(scores_max + 0) = make_float2(broadcast_var_4, broadcast_var_4);
    ...
    for (int i_8 = 0; i_8 < 2; ++i_8) {
      ...
      __syncthreads();
      scores_max_clear_1[i_8] = tl::AllReduce<tl::MaxOp, 256, 128, 0, tl::NamedBarrier<256>>::run(scores_max_clear_1[i_8], (&(((float*)workspace_1)[0])));
    ...
    for (int i_12 = 0; i_12 < 2; ++i_12) {
      ...
      __syncthreads();
      scores_sum[i_12] = tl::AllReduce<tl::SumOp, 256, 128, 0, tl::NamedBarrier<256>>::run(scores_sum[i_12], (&(((float*)workspace_2)[0])));

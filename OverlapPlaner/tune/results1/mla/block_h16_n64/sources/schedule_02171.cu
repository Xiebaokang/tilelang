    ...
    for (int k = 0; k < 127; ++k) {
      ...
      overlap_plan_mbar[(((k + 1) & 1) + 4)].wait((((k + 1) & 3) >> 1));
      {
        ...
        for (int ki_3 = 0; ki_3 < 4; ++ki_3) {
          ...
          tl::ptx_ldmatrix_x2((&(((half_t*)K_pe_shared)[(((((((((((((int)threadIdx.x) >> 5) + ((((int)threadIdx.x) & 31) >> 4)) >> 3) + ((k + 1) & 1)) & 1) * 4096) + ((((((int)threadIdx.x) >> 5) + ((((int)threadIdx.x) & 31) >> 4)) & 7) * 512)) + ((((int)threadIdx.x) & 7) * 64)) + (((((((int)threadIdx.x) & 7) >> 2) + (ki_3 >> 1)) & 1) * 32)) + (((((((int)threadIdx.x) & 3) >> 1) + (ki_3 & 1)) & 1) * 16)) + (((((((int)threadIdx.x) & 15) >> 3) + (((int)threadIdx.x) & 1)) & 1) * 8))])), (&(B_local_3[0])));
      ...
      *(float2*)(scores_max + 0) = make_float2(broadcast_var_6, broadcast_var_6);
      ...
      for (int i_9 = 0; i_9 < 2; ++i_9) {
        ...
        tl::__sync_thread_partial(3, 256);
        scores_max_clear_1[i_9] = tl::AllReduce<tl::MaxOp, 256, 32, 0, tl::NamedBarrier<256>>::run(scores_max_clear_1[i_9], (&(((float*)workspace_2)[0])));

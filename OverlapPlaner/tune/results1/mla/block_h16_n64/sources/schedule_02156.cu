...
extern "C" __global__ void __launch_bounds__(256, 1) main_kernel(__grid_constant__ const CUtensorMap KV_desc, __grid_constant__ const CUtensorMap K_pe_desc, half_t* __restrict__ Output, __grid_constant__ const CUtensorMap Q_desc, __grid_constant__ const CUtensorMap Q_pe_desc) {
  ...
  *(float2*)(scores_max + 0) = make_float2(broadcast_var_4, broadcast_var_4);
  ...
  for (int i_1 = 0; i_1 < 2; ++i_1) {
    ...
    __syncthreads();
    scores_max_clear[i_1] = tl::AllReduce<tl::MaxOp, 256, 32, 0, tl::NamedBarrier<256>>::run(scores_max_clear[i_1], (&(((float*)workspace_4)[0])));
  ...
  *(float2*)(scores_max + 0) = make_float2(broadcast_var_6, broadcast_var_6);
  ...
  for (int i_8 = 0; i_8 < 2; ++i_8) {
    ...
    for (int rv_2 = 0; rv_2 < 2; ++rv_2) {
      scores_max_clear_1[i_8] = max(scores_max_clear_1[i_8], acc_s_v1[((i_8 * 2) + rv_2)]);
    }
    __syncthreads();
    scores_max_clear_1[i_8] = tl::AllReduce<tl::MaxOp, 256, 32, 0, tl::NamedBarrier<256>>::run(scores_max_clear_1[i_8], (&(((float*)workspace_5)[0])));
  ...
  for (int i_11 = 0; i_11 < 4; ++i_11) {
    acc_s_v1[i_11] = exp2f(((acc_s_v1[i_11] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/) - (scores_max[(i_11 >> 1)] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/)));
  }
  #pragma unroll
  for (int i_12 = 0; i_12 < 2; ++i_12) {
    ...
    for (int rv_3 = 0; rv_3 < 2; ++rv_3) {
      scores_sum[i_12] = (scores_sum[i_12] + acc_s_v1[((i_12 * 2) + rv_3)]);
    }
    __syncthreads();
    scores_sum[i_12] = tl::AllReduce<tl::SumOp, 256, 32, 0, tl::NamedBarrier<256>>::run(scores_sum[i_12], (&(((float*)workspace_2)[0])));
  ...
  for (int i_14 = 0; i_14 < 32; ++i_14) {
    acc_o[i_14] = (acc_o[i_14] * scores_scale_v0[((i_14 & 3) >> 1)]);
  }
  for (int k = 0; k < 63; ++k) {
    if (1 <= k) {
    ...
    overlap_plan_mbar[(((k * 2) % 3) + 2)].wait((((k % 3) * 2) / 3));
    {
      ...
      for (int ki_6 = 0; ki_6 < 4; ++ki_6) {
        ...
        for (int i_15 = 0; i_15 < 4; ++i_15) {
          tl::ptx_ldmatrix_x4_trans((&(((half_t*)KV_shared)[(((((((k * 2) % 3) * 32768) + ((((int)threadIdx.x) >> 5) * 4096)) + (ki_6 * 1024)) + (((((int)threadIdx.x) & 15) >> 3) * 512)) + ((((((((int)threadIdx.x) & 15) * 64) + (((((((int)threadIdx.x) & 7) >> 2) + (i_15 >> 1)) & 1) * 32)) + (((((((int)threadIdx.x) & 3) >> 1) + (i_15 & 1)) & 1) * 16)) + (((((((int)threadIdx.x) & 31) >> 4) + (((int)threadIdx.x) & 1)) & 1) * 8)) & 511))])), (&(B_local_6[(i_15 * 8)])));
        }
        for (int j = 0; j < 4; ++j) {
    ...
    for (int i_16 = 0; i_16 < 2; ++i_16) {
      ...
      uint1 __2;
      float2 v__1 = *(float2*)(acc_s_v1 + (i_16 * 2));
      ((half2*)(&__2))[0] = __float22half2_rn(((float2*)(&v__1))[0]);
      *(uint1*)(S_shared_local_cast_1 + 0) = __2;
      *(uint1*)(((half_t*)S_shared) + (((((((i_16 * 512) + (((((int)threadIdx.x) & 31) >> 2) * 64)) + ((((((int)threadIdx.x) >> 7) + ((((int)threadIdx.x) & 31) >> 4)) & 1) * 32)) + (((((((int)threadIdx.x) & 127) >> 6) + ((((int)threadIdx.x) & 15) >> 3)) & 1) * 16)) + (((((((int)threadIdx.x) & 63) >> 5) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 8)) + ((((int)threadIdx.x) & 3) * 2)) + 1024)) = *(uint1*)(S_shared_local_cast_1 + 0);
    }
    *(float2*)(scores_max_prev + 0) = *(float2*)(scores_max + 0);
    ...
    *(float2*)(scores_max + 0) = make_float2(broadcast_var_8, broadcast_var_8);
    ...
    for (int i_17 = 0; i_17 < 2; ++i_17) {
      ...
      for (int rv_4 = 0; rv_4 < 2; ++rv_4) {
        scores_max_clear_2[i_17] = max(scores_max_clear_2[i_17], acc_s_v0[((i_17 * 2) + rv_4)]);
      }
      __syncthreads();
      scores_max_clear_2[i_17] = tl::AllReduce<tl::MaxOp, 256, 32, 0, tl::NamedBarrier<256>>::run(scores_max_clear_2[i_17], (&(((float*)workspace_6)[0])));
    ...
    for (int i_20 = 0; i_20 < 4; ++i_20) {
      acc_s_v0[i_20] = exp2f(((acc_s_v0[i_20] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/) - (scores_max[(i_20 >> 1)] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/)));
    }
    #pragma unroll
    for (int i_21 = 0; i_21 < 2; ++i_21) {
      ...
      for (int rv_5 = 0; rv_5 < 2; ++rv_5) {
        scores_sum[i_21] = (scores_sum[i_21] + acc_s_v0[((i_21 * 2) + rv_5)]);
      }
      __syncthreads();
    ...
    overlap_plan_mbar[((((k * 2) + 1) % 3) + 2)].wait(((((k % 3) * 2) + 1) / 3));
    {
      ...
      for (int ki_9 = 0; ki_9 < 4; ++ki_9) {
        ...
        for (int i_24 = 0; i_24 < 4; ++i_24) {
          tl::ptx_ldmatrix_x4_trans((&(((half_t*)KV_shared)[((((((((k * 2) + 1) % 3) * 32768) + ((((int)threadIdx.x) >> 5) * 4096)) + (ki_9 * 1024)) + (((((int)threadIdx.x) & 15) >> 3) * 512)) + ((((((((int)threadIdx.x) & 15) * 64) + (((((((int)threadIdx.x) & 7) >> 2) + (i_24 >> 1)) & 1) * 32)) + (((((((int)threadIdx.x) & 3) >> 1) + (i_24 & 1)) & 1) * 16)) + (((((((int)threadIdx.x) & 31) >> 4) + (((int)threadIdx.x) & 1)) & 1) * 8)) & 511))])), (&(B_local_9[(i_24 * 8)])));
        }
    ...
    for (int i_25 = 0; i_25 < 2; ++i_25) {
      ...
      uint1 __3;
      float2 v__2 = *(float2*)(acc_s_v0 + (i_25 * 2));
      ((half2*)(&__3))[0] = __float22half2_rn(((float2*)(&v__2))[0]);
      *(uint1*)(S_shared_local_cast_2 + 0) = __3;
      *(uint1*)(((half_t*)S_shared) + ((((((i_25 * 512) + (((((int)threadIdx.x) & 31) >> 2) * 64)) + ((((((int)threadIdx.x) >> 7) + ((((int)threadIdx.x) & 31) >> 4)) & 1) * 32)) + (((((((int)threadIdx.x) & 127) >> 6) + ((((int)threadIdx.x) & 15) >> 3)) & 1) * 16)) + (((((((int)threadIdx.x) & 63) >> 5) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 8)) + ((((int)threadIdx.x) & 3) * 2))) = *(uint1*)(S_shared_local_cast_2 + 0);
    }
    *(float2*)(scores_max_prev + 0) = *(float2*)(scores_max + 0);
    ...
    *(float2*)(scores_max + 0) = make_float2(broadcast_var_10, broadcast_var_10);
    ...
    for (int i_26 = 0; i_26 < 2; ++i_26) {
      ...
      for (int rv_6 = 0; rv_6 < 2; ++rv_6) {
        scores_max_clear_3[i_26] = max(scores_max_clear_3[i_26], acc_s_v1[((i_26 * 2) + rv_6)]);
      }
      __syncthreads();
    ...
    for (int i_29 = 0; i_29 < 4; ++i_29) {
      acc_s_v1[i_29] = exp2f(((acc_s_v1[i_29] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/) - (scores_max[(i_29 >> 1)] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/)));
    }
    #pragma unroll
    for (int i_30 = 0; i_30 < 2; ++i_30) {
      ...
      for (int rv_7 = 0; rv_7 < 2; ++rv_7) {
        scores_sum[i_30] = (scores_sum[i_30] + acc_s_v1[((i_30 * 2) + rv_7)]);
      }
      __syncthreads();
      scores_sum[i_30] = tl::AllReduce<tl::SumOp, 256, 32, 0, tl::NamedBarrier<256>>::run(scores_sum[i_30], (&(((float*)workspace_3)[0])));
      scores_sum[i_30] = tl::AllReduce<tl::SumOp, 4, 1, 0, tl::NamedBarrier<256>>::run(scores_sum[i_30]);
    }
    #pragma unroll
    for (int i_31 = 0; i_31 < 2; ++i_31) {
      logsum[i_31] = ((logsum[i_31] * scores_scale_v1[i_31]) + scores_sum[i_31]);
    }
    #pragma unroll
    for (int i_32 = 0; i_32 < 32; ++i_32) {
      acc_o[i_32] = (acc_o[i_32] * scores_scale_v0[((i_32 & 3) >> 1)]);
    }
  }
  overlap_plan_mbar[2].wait(0);
  {
    ...
    for (int ki_10 = 0; ki_10 < 4; ++ki_10) {
      ...
      for (int i_33 = 0; i_33 < 4; ++i_33) {
        tl::ptx_ldmatrix_x4_trans((&(((half_t*)KV_shared)[(((((((int)threadIdx.x) >> 5) * 4096) + (ki_10 * 1024)) + (((((int)threadIdx.x) & 15) >> 3) * 512)) + ((((((((int)threadIdx.x) & 15) * 64) + (((((((int)threadIdx.x) & 7) >> 2) + (i_33 >> 1)) & 1) * 32)) + (((((((int)threadIdx.x) & 3) >> 1) + (i_33 & 1)) & 1) * 16)) + (((((((int)threadIdx.x) & 31) >> 4) + (((int)threadIdx.x) & 1)) & 1) * 8)) & 511))])), (&(B_local_10[(i_33 * 8)])));
      }
  ...
  for (int i_34 = 0; i_34 < 2; ++i_34) {
    ...
    uint1 __4;
    float2 v__3 = *(float2*)(acc_s_v1 + (i_34 * 2));
    ((half2*)(&__4))[0] = __float22half2_rn(((float2*)(&v__3))[0]);
    *(uint1*)(S_shared_local_cast_3 + 0) = __4;
    *(uint1*)(((half_t*)S_shared) + (((((((i_34 * 512) + (((((int)threadIdx.x) & 31) >> 2) * 64)) + ((((((int)threadIdx.x) >> 7) + ((((int)threadIdx.x) & 31) >> 4)) & 1) * 32)) + (((((((int)threadIdx.x) & 127) >> 6) + ((((int)threadIdx.x) & 15) >> 3)) & 1) * 16)) + (((((((int)threadIdx.x) & 63) >> 5) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 8)) + ((((int)threadIdx.x) & 3) * 2)) + 1024)) = *(uint1*)(S_shared_local_cast_3 + 0);
  }
  #pragma unroll

...
extern "C" __global__ void __launch_bounds__(384, 1) main_kernel(__grid_constant__ const CUtensorMap KV_desc, __grid_constant__ const CUtensorMap K_pe_desc, half_t* __restrict__ Output, __grid_constant__ const CUtensorMap Q_desc, __grid_constant__ const CUtensorMap Q_pe_desc) {
  ...
  void* S_shared = ((void*)((char*)buf_dyn_shmem + 118784));
  void* workspace = ((void*)((char*)buf_dyn_shmem + 122880));
  void* workspace_1 = ((void*)((char*)buf_dyn_shmem + 122880));
    ...
    for (int k = 0; k < 128; ++k) {
      ...
      overlap_plan_mbar[((k & 1) + 3)].wait(((k & 3) >> 1));
      {
        ...
        for (int ki_1 = 0; ki_1 < 4; ++ki_1) {
          ...
          tl::ptx_ldmatrix_x2((&(((half_t*)K_pe_shared)[(((((((((((((int)threadIdx.x) >> 5) + ((((int)threadIdx.x) & 31) >> 4)) >> 3) + (k & 1)) & 1) * 4096) + ((((((int)threadIdx.x) >> 5) + ((((int)threadIdx.x) & 31) >> 4)) & 7) * 512)) + ((((int)threadIdx.x) & 7) * 64)) + (((((((int)threadIdx.x) & 7) >> 2) + (ki_1 >> 1)) & 1) * 32)) + (((((((int)threadIdx.x) & 3) >> 1) + (ki_1 & 1)) & 1) * 16)) + (((((((int)threadIdx.x) & 15) >> 3) + (((int)threadIdx.x) & 1)) & 1) * 8))])), (&(B_local_1[0])));

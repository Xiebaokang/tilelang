...
extern "C" __global__ void __launch_bounds__(384, 1) main_kernel(__grid_constant__ const CUtensorMap KV_desc, __grid_constant__ const CUtensorMap K_pe_desc, half_t* __restrict__ Output, __grid_constant__ const CUtensorMap Q_desc, __grid_constant__ const CUtensorMap Q_pe_desc) {
  ...
  if (((int)threadIdx.x) < 256) {
    ...
    *(float4*)(scores_max + 0) = make_float4(broadcast_var_2, broadcast_var_2, broadcast_var_2, broadcast_var_2);
    for (int k = 0; k < 128; ++k) {
      if (3 <= k) {
        overlap_plan_mbar[((k % 3) + 9)].wait((((k / 3) + 1) & 1));
      }
      if (tl::tl_shuffle_elect<256>() && ((((int)threadIdx.x) >> 5) == 0)) {
        overlap_plan_mbar[((k % 3) + 4)].arrive_and_expect_tx(8192);
        tl::fence_proxy_async();
        tl::tma_load(K_pe_desc, overlap_plan_mbar[((k % 3) + 4)], (&(((half_t*)K_pe_shared)[((k % 3) * 4096)])), 0, (k * 64), 0, 0);
      }
      if (k == 0) {
      ...
      }
      {
        ...
        for (int ki_2 = 0; ki_2 < 4; ++ki_2) {
          ...
          for (int i_14 = 0; i_14 < 4; ++i_14) {
            tl::ptx_ldmatrix_x4_trans((&(((half_t*)KV_shared)[((((((k & 1) * 32768) + ((((int)threadIdx.x) >> 5) * 4096)) + (ki_2 * 1024)) + (((((int)threadIdx.x) & 15) >> 3) * 512)) + ((((((((int)threadIdx.x) & 15) * 64) + (((((((int)threadIdx.x) & 7) >> 2) + (i_14 >> 1)) & 1) * 32)) + (((((((int)threadIdx.x) & 3) >> 1) + (i_14 & 1)) & 1) * 16)) + (((((((int)threadIdx.x) & 31) >> 4) + (((int)threadIdx.x) & 1)) & 1) * 8)) & 511))])), (&(B_local_2[(i_14 * 8)])));
          }

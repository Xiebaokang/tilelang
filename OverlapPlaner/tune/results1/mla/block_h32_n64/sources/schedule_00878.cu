    ...
    for (int k_1 = 0; k_1 < 64; ++k_1) {
      ...
      *(float4*)(scores_max + 0) = make_float4(broadcast_var_5, broadcast_var_5, broadcast_var_5, broadcast_var_5);
      ...
      for (int i_27 = 0; i_27 < 4; ++i_27) {
        ...
        scores_max_clear[i_27] = tl::AllReduce<tl::MaxOp, 256, 32, 256, tl::NamedBarrier<256>>::run(scores_max_clear[i_27], (&(((float*)workspace_2)[0])));
      ...
      for (int i_33 = 0; i_33 < 4; ++i_33) {
        ...
        for (int rv_1 = 0; rv_1 < 2; ++rv_1) {
          scores_sum[i_33] = (scores_sum[i_33] + acc_s_v0[((i_33 * 2) + rv_1)]);
        }
        tl::__sync_thread_partial(4, 256);
        scores_sum[i_33] = tl::AllReduce<tl::SumOp, 256, 32, 256, tl::NamedBarrier<256>>::run(scores_sum[i_33], (&(((float*)workspace_1)[0])));
        scores_sum[i_33] = tl::AllReduce<tl::SumOp, 4, 1, 256, tl::NamedBarrier<256>>::run(scores_sum[i_33]);
      }
      if (1 <= k_1) {
      ...
      for (int i_34 = 0; i_34 < 4; ++i_34) {
        ...
        uint1 __1;
        float2 v_ = *(float2*)(acc_s_v0 + (i_34 * 2));
        ((half2*)(&__1))[0] = __float22half2_rn(((float2*)(&v_))[0]);
        *(uint1*)(S_shared_local_cast + 0) = __1;
        *(uint1*)(((half_t*)S_shared) + ((((((i_34 * 512) + (((((int)threadIdx.x) & 31) >> 2) * 64)) + ((((((int)threadIdx.x) >> 7) + ((((int)threadIdx.x) & 31) >> 4)) & 1) * 32)) + (((((((int)threadIdx.x) & 127) >> 6) + ((((int)threadIdx.x) & 15) >> 3)) & 1) * 16)) + (((((((int)threadIdx.x) & 63) >> 5) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 8)) + ((((int)threadIdx.x) & 3) * 2))) = *(uint1*)(S_shared_local_cast + 0);
      }
      tl::fence_proxy_async();
      ...
      *(float4*)(scores_max + 0) = make_float4(broadcast_var_6, broadcast_var_6, broadcast_var_6, broadcast_var_6);
      ...
      for (int i_43 = 0; i_43 < 4; ++i_43) {
        scores_sum[i_43] = 0x0p+0f/*0.000000e+00*/;
        #pragma unroll
        for (int rv_3 = 0; rv_3 < 2; ++rv_3) {
          scores_sum[i_43] = (scores_sum[i_43] + acc_s_v1[((i_43 * 2) + rv_3)]);
        }
        tl::__sync_thread_partial(4, 256);
        scores_sum[i_43] = tl::AllReduce<tl::SumOp, 256, 32, 256, tl::NamedBarrier<256>>::run(scores_sum[i_43], (&(((float*)workspace_3)[0])));
        scores_sum[i_43] = tl::AllReduce<tl::SumOp, 4, 1, 256, tl::NamedBarrier<256>>::run(scores_sum[i_43]);
      }
      if (1 <= k_1) {
      ...
      for (int i_44 = 0; i_44 < 4; ++i_44) {
        ...
        uint1 __2;
        float2 v__1 = *(float2*)(acc_s_v1 + (i_44 * 2));
        ((half2*)(&__2))[0] = __float22half2_rn(((float2*)(&v__1))[0]);
        *(uint1*)(S_shared_local_cast_1 + 0) = __2;
        *(uint1*)(((half_t*)S_shared) + (((((((i_44 * 512) + (((((int)threadIdx.x) & 31) >> 2) * 64)) + ((((((int)threadIdx.x) >> 7) + ((((int)threadIdx.x) & 31) >> 4)) & 1) * 32)) + (((((((int)threadIdx.x) & 127) >> 6) + ((((int)threadIdx.x) & 15) >> 3)) & 1) * 16)) + (((((((int)threadIdx.x) & 63) >> 5) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 8)) + ((((int)threadIdx.x) & 3) * 2)) + 2048)) = *(uint1*)(S_shared_local_cast_1 + 0);
      }
      tl::fence_proxy_async();

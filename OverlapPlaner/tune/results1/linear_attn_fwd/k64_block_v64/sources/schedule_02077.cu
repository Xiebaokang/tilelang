...
extern "C" __global__ void __launch_bounds__(256, 1) main_kernel(float* __restrict__ FinalState, __grid_constant__ const CUtensorMap K_desc, float* __restrict__ O, const half_t* __restrict__ Q, __grid_constant__ const CUtensorMap V_desc) {
  ...
  if (((int)threadIdx.x) < 128) {
    ...
    for (int ic = 0; ic < 16; ++ic) {
      ...
      overlap_plan_mbar[(ic & 1)].wait(((ic & 3) >> 1));
      ...
      for (int i_2 = 0; i_2 < 4; ++i_2) {
        ...
        *(uint4*)(Q_local_cast_1 + 0) = *(uint4*)(Q + ((((((((((int)blockIdx.z) >> 5) * 4194304) + (ic * 262144)) + (i_2 * 65536)) + ((((int)threadIdx.x) >> 3) * 4096)) + ((((int)blockIdx.z) & 31) * 128)) + (((int)blockIdx.y) * 64)) + ((((int)threadIdx.x) & 7) * 8)));
      ...
      overlap_plan_mbar[((ic & 1) + 4)].arrive();
      #pragma unroll
      ...
      overlap_plan_mbar[((ic & 1) + 6)].arrive();
      {
      ...
      for (int i_5 = 0; i_5 < 8; ++i_5) {
        AtomicAddx4((&(O[((((((((((int)blockIdx.z) >> 5) * 4194304) + (ic * 262144)) + (i_5 * 32768)) + ((((int)threadIdx.x) >> 4) * 4096)) + ((((int)blockIdx.z) & 31) * 128)) + (((int)blockIdx.x) * 64)) + ((((int)threadIdx.x) & 15) * 4))])), *(float4*)(((float*)o_shared) + ((i_5 * 512) + (((int)threadIdx.x) * 4))));
      }
    }
    #pragma unroll
    for (int i_6 = 0; i_6 < 16; ++i_6) {
      *(float2*)(FinalState + ((((((((((int)blockIdx.z) * 16384) + (((int)blockIdx.y) * 8192)) + ((((int)threadIdx.x) >> 5) * 2048)) + ((i_6 & 1) * 1024)) + (((((int)threadIdx.x) & 31) >> 2) * 128)) + (((int)blockIdx.x) * 64)) + ((i_6 >> 1) * 8)) + ((((int)threadIdx.x) & 3) * 2))) = *(float2*)(h + (i_6 * 2));
    }
  } else {

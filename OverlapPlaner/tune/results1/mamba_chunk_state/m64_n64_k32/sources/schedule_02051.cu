...
extern "C" __global__ void __launch_bounds__(256, 1) main_kernel(__grid_constant__ const CUtensorMap B_desc, const half_t* __restrict__ DA, const half_t* __restrict__ Dt, __grid_constant__ const CUtensorMap Output_desc, __grid_constant__ const CUtensorMap X_desc) {
  ...
  if (((int)threadIdx.x) < 128) {
    ...
    for (int ik = 0; ik < 7; ++ik) {
      ...
      overlap_plan_mbar[(((ik + 1) & 1) + 3)].wait((((ik + 1) & 3) >> 1));
      #pragma unroll
    ...
    for (int ik_1 = 0; ik_1 < 8; ++ik_1) {
      if (3 <= ik_1) {
        overlap_plan_mbar[((ik_1 % 3) + 9)].wait(((ik_1 / 3) - 1));
      }
      ...
      if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 4)) {
        overlap_plan_mbar[((ik_1 & 1) + 3)].arrive_and_expect_tx(64);
        tl::tma_load((&(((half_t*)da_shared)[((ik_1 & 1) * 32)])), (&(DA[(((((((int)blockIdx.z) & 7) * 327680) + (((int)blockIdx.x) * 4096)) + ((((int)blockIdx.z) >> 3) * 256)) + (ik_1 * 32))])), overlap_plan_mbar[((ik_1 & 1) + 3)], 64);
      }
      if (2 <= ik_1) {
      ...
      if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 4)) {
        ...
        tl::tma_load(B_desc, overlap_plan_mbar[((ik_1 & 1) + 7)], (&(((half_t*)b_shared)[((ik_1 & 1) * 2048)])), (((int)blockIdx.y) * 64), (((((int)blockIdx.z) >> 3) * 256) + (ik_1 * 32)), 0, (((int)blockIdx.z) & 7));
      }
    }
  }
}

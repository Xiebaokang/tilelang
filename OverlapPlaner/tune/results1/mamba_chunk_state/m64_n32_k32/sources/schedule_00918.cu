...
extern "C" __global__ void __launch_bounds__(256, 1) main_kernel(__grid_constant__ const CUtensorMap B_desc, const half_t* __restrict__ DA, const half_t* __restrict__ Dt, __grid_constant__ const CUtensorMap Output_desc, __grid_constant__ const CUtensorMap X_desc) {
  ...
  if (((int)threadIdx.x) < 128) {
    ...
    for (int ik = 0; ik < 8; ++ik) {
      ...
      overlap_plan_mbar[((ik & 1) + 2)].wait(((ik & 3) >> 1));
      #pragma unroll
      for (int i_2 = 0; i_2 < 4; ++i_2) {
        half_t dt_shared_local_cast_1[2];
        *(uint1*)(dt_shared_local_cast_1 + 0) = *(uint1*)(((half_t*)dt_shared) + ((((ik & 1) * 32) + (i_2 * 8)) + ((((int)threadIdx.x) & 3) * 2)));
        float2 __2;
    ...
    for (int ik_1 = 0; ik_1 < 8; ++ik_1) {
      ...
      if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 4)) {
        overlap_plan_mbar[((ik_1 & 1) + 4)].arrive_and_expect_tx(2048);
        tl::tma_load(B_desc, overlap_plan_mbar[((ik_1 & 1) + 4)], (&(((half_t*)b_shared)[((ik_1 & 1) * 1024)])), (((int)blockIdx.y) * 32), (((((int)blockIdx.z) >> 3) * 256) + (ik_1 * 32)), 0, (((int)blockIdx.z) & 7));
      }
    }
  }
}

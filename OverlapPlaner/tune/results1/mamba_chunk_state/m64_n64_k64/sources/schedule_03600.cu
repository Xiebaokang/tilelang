...
extern "C" __global__ void __launch_bounds__(128, 1) main_kernel(__grid_constant__ const CUtensorMap B_desc, const half_t* __restrict__ DA, const half_t* __restrict__ Dt, __grid_constant__ const CUtensorMap Output_desc, __grid_constant__ const CUtensorMap X_desc) {
  ...
  for (int ik = 0; ik < 3; ++ik) {
    ...
    if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 0)) {
      overlap_plan_mbar[(((ik + 1) & 1) + 3)].arrive_and_expect_tx(128);
      tl::tma_load((&(((half_t*)dt_shared)[(((ik + 1) & 1) * 64)])), (&(Dt[((((((((int)blockIdx.z) & 7) * 327680) + (((int)blockIdx.x) * 4096)) + ((((int)blockIdx.z) >> 3) * 256)) + (ik * 64)) + 64)])), overlap_plan_mbar[(((ik + 1) & 1) + 3)], 128);
    }
    if (ik == 2) {
    ...
    for (int i_2 = 0; i_2 < 8; ++i_2) {
      ...
      *(uint1*)(dt_shared_local_cast_1 + 0) = *(uint1*)(((half_t*)dt_shared) + ((((ik & 1) * 64) + (i_2 * 8)) + ((((int)threadIdx.x) & 3) * 2)));
      float2 __2;

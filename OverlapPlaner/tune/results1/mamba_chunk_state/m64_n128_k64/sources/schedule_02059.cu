...
extern "C" __global__ void __launch_bounds__(256, 1) main_kernel(__grid_constant__ const CUtensorMap B_desc, const half_t* __restrict__ DA, const half_t* __restrict__ Dt, half_t* __restrict__ Output, __grid_constant__ const CUtensorMap X_desc) {
  ...
  void* scale_wsp_handoff_4 = ((void*)((char*)buf_dyn_shmem + 43264));
  ...
  if (((int)threadIdx.x) < 128) {
    ...
    for (int ik = 0; ik < 3; ++ik) {
      ...
      if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 0)) {
        ...
        tl::tma_load((&(((half_t*)dt_shared)[(((ik + 1) & 1) * 64)])), (&(Dt[((((((((int)blockIdx.z) & 7) * 327680) + (((int)blockIdx.x) * 4096)) + ((((int)blockIdx.z) >> 3) * 256)) + (ik * 64)) + 64)])), overlap_plan_mbar[(((ik + 1) & 1) + 4)], 128);
      }
      if (1 <= ik) {
    ...
    for (int ik_1 = 0; ik_1 < 4; ++ik_1) {
      overlap_plan_mbar[((ik_1 & 1) + 2)].wait((ik_1 >> 1));
      #pragma unroll
      for (int i_8 = 0; i_8 < 8; ++i_8) {
        half_t da_shared_local_cast[2];
        *(uint1*)(da_shared_local_cast + 0) = *(uint1*)(((half_t*)da_shared) + ((((ik_1 & 1) * 64) + (i_8 * 8)) + ((((int)threadIdx.x) & 3) * 2)));
        float2 __1;

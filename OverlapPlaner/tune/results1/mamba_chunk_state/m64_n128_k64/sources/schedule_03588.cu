...
extern "C" __global__ void __launch_bounds__(128, 1) main_kernel(__grid_constant__ const CUtensorMap B_desc, const half_t* __restrict__ DA, const half_t* __restrict__ Dt, half_t* __restrict__ Output, __grid_constant__ const CUtensorMap X_desc) {
  ...
  for (int i_2 = 0; i_2 < 8; ++i_2) {
    ...
    *(uint1*)(da_shared_local_cast_1 + 0) = *(uint1*)(((half_t*)da_shared) + ((i_2 * 8) + ((((int)threadIdx.x) & 3) * 2)));
    float2 __2;
  ...
  for (int ik = 0; ik < 3; ++ik) {
    ...
    if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 0)) {
      ...
      tl::tma_load(B_desc, overlap_plan_mbar[3], (&(((half_t*)b_shared)[4096])), 64, ((((((int)blockIdx.z) >> 3) * 256) + (ik * 64)) + 64), 0, (((int)blockIdx.z) & 7));
    }
    ...
    for (int i_7 = 0; i_7 < 8; ++i_7) {
      ...
      *(uint1*)(da_shared_local_cast_3 + 0) = *(uint1*)(((half_t*)da_shared) + ((i_7 * 8) + ((((int)threadIdx.x) & 3) * 2)));
      float2 __4;

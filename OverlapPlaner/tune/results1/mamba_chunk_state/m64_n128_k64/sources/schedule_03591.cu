...
extern "C" __global__ void __launch_bounds__(256, 1) main_kernel(__grid_constant__ const CUtensorMap B_desc, const half_t* __restrict__ DA, const half_t* __restrict__ Dt, half_t* __restrict__ Output, __grid_constant__ const CUtensorMap X_desc) {
  ...
  if (((int)threadIdx.x) < 128) {
    ...
    for (int i_16 = 0; i_16 < 8; ++i_16) {
      ...
      *(uint1*)(da_shared_local_cast_6 + 0) = *(uint1*)(((half_t*)da_shared) + (((i_16 * 8) + ((((int)threadIdx.x) & 3) * 2)) + 64));
      float2 __7;
    ...
    for (int ik_1 = 0; ik_1 < 4; ++ik_1) {
      ...
      if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 4)) {
        overlap_plan_mbar[((ik_1 & 1) + 1)].arrive_and_expect_tx(128);
        tl::tma_load((&(((half_t*)da_shared)[((ik_1 & 1) * 64)])), (&(DA[(((((((int)blockIdx.z) & 7) * 327680) + (((int)blockIdx.x) * 4096)) + ((((int)blockIdx.z) >> 3) * 256)) + (ik_1 * 64))])), overlap_plan_mbar[((ik_1 & 1) + 1)], 128);
      }
    }
  }
}

...
extern "C" __global__ void __launch_bounds__(256, 1) main_kernel(__grid_constant__ const CUtensorMap A_desc, __grid_constant__ const CUtensorMap B_desc, __grid_constant__ const CUtensorMap Ct_desc) {
  ...
  if (((int)threadIdx.x) < 128) {
    ...
    for (int i_10 = 0; i_10 < 32; ++i_10) {
      B_local[i_10] = ((uchar*)B_shared)[(((((((((((int)threadIdx.x) >> 5) * 1024) + ((i_10 >> 4) * 512)) + (((((int)threadIdx.x) & 31) >> 2) * 64)) + (((((((int)threadIdx.x) & 31) >> 4) + ((i_10 & 15) >> 3)) & 1) * 32)) + (((((((int)threadIdx.x) & 15) >> 3) + ((i_10 & 7) >> 2)) & 1) * 16)) + ((i_10 & 3) * 4)) + (((int)threadIdx.x) & 3)) + 4096)];
    }
    overlap_plan_mbar[7].arrive();
    #pragma unroll

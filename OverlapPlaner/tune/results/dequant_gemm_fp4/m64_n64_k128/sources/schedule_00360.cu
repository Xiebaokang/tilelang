...
extern "C" __global__ void __launch_bounds__(256, 1) main_kernel(__grid_constant__ const CUtensorMap A_desc, __grid_constant__ const CUtensorMap B_desc, __grid_constant__ const CUtensorMap Ct_desc) {
  ...
  if (((int)threadIdx.x) < 128) {
    ...
    for (int k = 0; k < 31; ++k) {
      if (1 <= k) {
        overlap_plan_mbar[(((k + 1) & 1) + 4)].wait((((k + 3) & 3) >> 1));
      }
      if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 0)) {
      ...
      for (int i_4 = 0; i_4 < 32; ++i_4) {
        B_local[i_4] = ((uchar*)B_shared)[((((((((((k + 1) & 1) * 4096) + ((((int)threadIdx.x) >> 5) * 1024)) + ((i_4 >> 4) * 512)) + (((((int)threadIdx.x) & 31) >> 2) * 64)) + (((((((int)threadIdx.x) & 31) >> 4) + ((i_4 & 15) >> 3)) & 1) * 32)) + (((((((int)threadIdx.x) & 15) >> 3) + ((i_4 & 7) >> 2)) & 1) * 16)) + ((i_4 & 3) * 4)) + (((int)threadIdx.x) & 3))];
      }

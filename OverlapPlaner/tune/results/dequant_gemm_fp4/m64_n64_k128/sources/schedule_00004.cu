...
extern "C" __global__ void __launch_bounds__(128, 1) main_kernel(__grid_constant__ const CUtensorMap A_desc, __grid_constant__ const CUtensorMap B_desc, __grid_constant__ const CUtensorMap Ct_desc) {
  ...
  float Ct_local[32];
  uchar B_local[32];
  half_t B_dequantize_local_v0[64];
  half_t B_dequantize_local_v1[64];
  ...
  for (int i_2 = 0; i_2 < 64; ++i_2) {
    ...
    B_dequantize_local_v0[i_2] = (*(half_t *)(&(v_)));
  }
  if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 0)) {
  ...
  for (int i_4 = 0; i_4 < 64; ++i_4) {
    ushort v__1 = (((((((((ushort)B_local[(i_4 >> 1)]) >> (((ushort)(i_4 & 1)) * (ushort)4)) & (ushort)15) & (ushort)6) >> (ushort)1) + (ushort)14) | ((((((ushort)B_local[(i_4 >> 1)]) >> (((ushort)(i_4 & 1)) * (ushort)4)) & (ushort)15) >> (ushort)3) << (ushort)5)) << (ushort)10) | ((((((ushort)B_local[(i_4 >> 1)]) >> (((ushort)(i_4 & 1)) * (ushort)4)) & (ushort)15) & (ushort)1) << (ushort)9);
    B_dequantize_local_v1[i_4] = (*(half_t *)(&(v__1)));
  }
  #pragma unroll
  for (int i_5 = 0; i_5 < 8; ++i_5) {
    *(uint4*)(B_dequantize_prev_local_v0 + (i_5 * 8)) = *(uint4*)(B_dequantize_local_v0 + (i_5 * 8));
  }
  for (int k = 0; k < 15; ++k) {
    if (1 <= k) {
      overlap_plan_mbar[((((k * 2) + 2) % 3) + 4)].wait(((((k * 2) + 5) % 6) / 3));
    }
    if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 0)) {
      overlap_plan_mbar[(((k * 2) + 2) % 3)].arrive_and_expect_tx(16384);
      tl::tma_load(A_desc, overlap_plan_mbar[(((k * 2) + 2) % 3)], (&(((half_t*)A_shared)[((((k * 2) + 2) % 3) * 8192)])), ((k * 256) + 256), (((int)blockIdx.y) * 64));
      tl::tma_load(A_desc, overlap_plan_mbar[(((k * 2) + 2) % 3)], (&(((half_t*)A_shared)[(((((k * 2) + 2) % 3) * 8192) + 4096)])), ((k * 256) + 320), (((int)blockIdx.y) * 64));
    }
    overlap_plan_mbar[7].wait(1);
    __syncthreads();
    if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 0)) {
      overlap_plan_mbar[3].arrive_and_expect_tx(4096);
      tl::tma_load(B_desc, overlap_plan_mbar[3], (&(((uchar*)B_shared)[0])), ((k * 128) + 128), (((int)blockIdx.x) * 64));
    }
    overlap_plan_mbar[3].wait(0);
    #pragma unroll
    ...
    for (int i_7 = 0; i_7 < 64; ++i_7) {
      ushort v__2 = (((((((((ushort)B_local[((((i_7 & 3) >> 1) * 16) + (i_7 >> 2))]) >> (((ushort)(i_7 & 1)) * (ushort)4)) & (ushort)15) & (ushort)6) >> (ushort)1) + (ushort)14) | ((((((ushort)B_local[((((i_7 & 3) >> 1) * 16) + (i_7 >> 2))]) >> (((ushort)(i_7 & 1)) * (ushort)4)) & (ushort)15) >> (ushort)3) << (ushort)5)) << (ushort)10) | ((((((ushort)B_local[((((i_7 & 3) >> 1) * 16) + (i_7 >> 2))]) >> (((ushort)(i_7 & 1)) * (ushort)4)) & (ushort)15) & (ushort)1) << (ushort)9);
      B_dequantize_local_v0[i_7] = (*(half_t *)(&(v__2)));
    }
    #pragma unroll
    for (int i_8 = 0; i_8 < 32; ++i_8) {
      *(uint1*)(B_dequantize_prev_local_v1 + (i_8 * 2)) = *(uint1*)(B_dequantize_local_v1 + (((i_8 & 1) * 32) + ((i_8 >> 1) * 2)));
    }
    overlap_plan_mbar[((k * 2) % 3)].wait((((k % 3) * 2) / 3));
    {
    ...
    overlap_plan_mbar[(((k * 2) % 3) + 4)].arrive();
    overlap_plan_mbar[(((k * 2) % 3) + 4)].wait((((k % 3) * 2) / 3));
    if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 0)) {
      overlap_plan_mbar[((k * 2) % 3)].arrive_and_expect_tx(16384);
      tl::tma_load(A_desc, overlap_plan_mbar[((k * 2) % 3)], (&(((half_t*)A_shared)[(((k * 2) % 3) * 8192)])), ((k * 256) + 384), (((int)blockIdx.y) * 64));
      tl::tma_load(A_desc, overlap_plan_mbar[((k * 2) % 3)], (&(((half_t*)A_shared)[((((k * 2) % 3) * 8192) + 4096)])), ((k * 256) + 448), (((int)blockIdx.y) * 64));
    }
    overlap_plan_mbar[7].wait(0);
    __syncthreads();
    if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 0)) {
      overlap_plan_mbar[3].arrive_and_expect_tx(4096);
      tl::tma_load(B_desc, overlap_plan_mbar[3], (&(((uchar*)B_shared)[0])), ((k * 128) + 192), (((int)blockIdx.x) * 64));
    }
    overlap_plan_mbar[3].wait(1);
    #pragma unroll
    for (int i_9 = 0; i_9 < 32; ++i_9) {
      B_local[i_9] = ((uchar*)B_shared)[((((((((((int)threadIdx.x) >> 5) * 1024) + ((i_9 >> 4) * 512)) + (((((int)threadIdx.x) & 31) >> 2) * 64)) + (((((((int)threadIdx.x) & 31) >> 4) + ((i_9 & 15) >> 3)) & 1) * 32)) + (((((((int)threadIdx.x) & 15) >> 3) + ((i_9 & 7) >> 2)) & 1) * 16)) + ((i_9 & 3) * 4)) + (((int)threadIdx.x) & 3))];
    }
    overlap_plan_mbar[7].arrive();
    #pragma unroll
    for (int i_10 = 0; i_10 < 64; ++i_10) {
      ushort v__3 = (((((((((ushort)B_local[(i_10 >> 1)]) >> (((ushort)(i_10 & 1)) * (ushort)4)) & (ushort)15) & (ushort)6) >> (ushort)1) + (ushort)14) | ((((((ushort)B_local[(i_10 >> 1)]) >> (((ushort)(i_10 & 1)) * (ushort)4)) & (ushort)15) >> (ushort)3) << (ushort)5)) << (ushort)10) | ((((((ushort)B_local[(i_10 >> 1)]) >> (((ushort)(i_10 & 1)) * (ushort)4)) & (ushort)15) & (ushort)1) << (ushort)9);
      B_dequantize_local_v1[i_10] = (*(half_t *)(&(v__3)));
    }
    #pragma unroll
    for (int i_11 = 0; i_11 < 8; ++i_11) {
      *(uint4*)(B_dequantize_prev_local_v0 + (i_11 * 8)) = *(uint4*)(B_dequantize_local_v0 + (i_11 * 8));
    }
    overlap_plan_mbar[(((k * 2) + 1) % 3)].wait(((((k % 3) * 2) + 1) / 3));
    {
    ...
    overlap_plan_mbar[((((k * 2) + 1) % 3) + 4)].arrive();
  }
  #pragma unroll
  for (int i_12 = 0; i_12 < 32; ++i_12) {
    *(uint1*)(B_dequantize_prev_local_v1 + (i_12 * 2)) = *(uint1*)(B_dequantize_local_v1 + (((i_12 & 1) * 32) + ((i_12 >> 1) * 2)));
  }

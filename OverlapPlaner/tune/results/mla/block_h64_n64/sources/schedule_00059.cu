...
extern "C" __global__ void __launch_bounds__(256, 1) main_kernel(__grid_constant__ const CUtensorMap KV_desc, __grid_constant__ const CUtensorMap K_pe_desc, half_t* __restrict__ Output, __grid_constant__ const CUtensorMap Q_desc, __grid_constant__ const CUtensorMap Q_pe_desc) {
  ...
  *(float2*)(scores_max + 0) = make_float2(broadcast_var_3, broadcast_var_3);
  ...
  for (int i_1 = 0; i_1 < 2; ++i_1) {
    ...
    __syncthreads();
    scores_max_clear[i_1] = tl::AllReduce<tl::MaxOp, 256, 128, 0, tl::NamedBarrier<256>>::run(scores_max_clear[i_1], (&(((float*)workspace_9)[0])));
  ...
  *(float2*)(scores_max + 0) = make_float2(broadcast_var_4, broadcast_var_4);
  ...
  for (int i_5 = 0; i_5 < 2; ++i_5) {
    scores_max_clear_1[i_5] = -CUDART_INF_F;
    #pragma unroll
    for (int rv_1 = 0; rv_1 < 8; ++rv_1) {
      scores_max_clear_1[i_5] = max(scores_max_clear_1[i_5], acc_s_v1[((((rv_1 & 3) * 4) + (i_5 * 2)) + (rv_1 >> 2))]);
    }
    __syncthreads();
    scores_max_clear_1[i_5] = tl::AllReduce<tl::MaxOp, 256, 128, 0, tl::NamedBarrier<256>>::run(scores_max_clear_1[i_5], (&(((float*)workspace_8)[0])));
    scores_max_clear_1[i_5] = tl::AllReduce<tl::MaxOp, 4, 1, 0, tl::NamedBarrier<256>>::run(scores_max_clear_1[i_5]);
    scores_max[i_5] = max(scores_max[i_5], scores_max_clear_1[i_5]);
  }
  #pragma unroll
  ...
  for (int i_7 = 0; i_7 < 2; ++i_7) {
    scores_scale_v1[i_7] = exp2f(((scores_max_prev[i_7] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/) - (scores_max[i_7] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/)));
  }
  #pragma unroll
  for (int i_8 = 0; i_8 < 16; ++i_8) {
    acc_s_v1[i_8] = exp2f(((acc_s_v1[i_8] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/) - (scores_max[((i_8 & 3) >> 1)] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/)));
  }
  #pragma unroll
  for (int i_9 = 0; i_9 < 2; ++i_9) {
    tl::ptx_stmatrix_m8n8_x4((&(((half_t*)S_shared)[(((((((int)threadIdx.x) & 127) >> 5) * 1024) + (((((int)threadIdx.x) & 15) >> 3) * 512)) + ((((((((int)threadIdx.x) & 15) * 64) + ((((((int)threadIdx.x) >> 7) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 32)) + (((((((int)threadIdx.x) & 3) >> 1) + i_9) & 1) * 16)) + (((((((int)threadIdx.x) & 31) >> 4) + (((int)threadIdx.x) & 1)) & 1) * 8)) & 511))])), __pack_half2(((half_t)acc_s_v0[(i_9 * 8)]), ((half_t)acc_s_v0[((i_9 * 8) + 1)])), __pack_half2(((half_t)acc_s_v0[((i_9 * 8) + 2)]), ((half_t)acc_s_v0[((i_9 * 8) + 3)])), __pack_half2(((half_t)acc_s_v0[((i_9 * 8) + 4)]), ((half_t)acc_s_v0[((i_9 * 8) + 5)])), __pack_half2(((half_t)acc_s_v0[((i_9 * 8) + 6)]), ((half_t)acc_s_v0[((i_9 * 8) + 7)])));
  }
  #pragma unroll
  for (int i_10 = 0; i_10 < 128; ++i_10) {
    acc_o[i_10] = (acc_o[i_10] * scores_scale_v0[((i_10 & 3) >> 1)]);
  }
  overlap_plan_mbar[2].wait(0);
  {
  ...
  for (int k = 0; k < 42; ++k) {
    ...
    *(float2*)(scores_max + 0) = make_float2(broadcast_var_5, broadcast_var_5);
    ...
    for (int i_11 = 0; i_11 < 2; ++i_11) {
      scores_max_clear_2[i_11] = -CUDART_INF_F;
      #pragma unroll
      for (int rv_2 = 0; rv_2 < 8; ++rv_2) {
        scores_max_clear_2[i_11] = max(scores_max_clear_2[i_11], acc_s_v2[((((rv_2 & 3) * 4) + (i_11 * 2)) + (rv_2 >> 2))]);
      }
      __syncthreads();
      scores_max_clear_2[i_11] = tl::AllReduce<tl::MaxOp, 256, 128, 0, tl::NamedBarrier<256>>::run(scores_max_clear_2[i_11], (&(((float*)workspace)[0])));
      scores_max_clear_2[i_11] = tl::AllReduce<tl::MaxOp, 4, 1, 0, tl::NamedBarrier<256>>::run(scores_max_clear_2[i_11]);
      scores_max[i_11] = max(scores_max[i_11], scores_max_clear_2[i_11]);
    }
    #pragma unroll
    ...
    for (int i_13 = 0; i_13 < 2; ++i_13) {
      scores_scale_v2[i_13] = exp2f(((scores_max_prev[i_13] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/) - (scores_max[i_13] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/)));
    }
    #pragma unroll
    for (int i_14 = 0; i_14 < 16; ++i_14) {
      acc_s_v2[i_14] = exp2f(((acc_s_v2[i_14] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/) - (scores_max[((i_14 & 3) >> 1)] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/)));
    }
    #pragma unroll
    for (int i_15 = 0; i_15 < 2; ++i_15) {
      ...
      for (int rv_3 = 0; rv_3 < 8; ++rv_3) {
        scores_sum[i_15] = (scores_sum[i_15] + acc_s_v0[((((rv_3 & 3) * 4) + (i_15 * 2)) + (rv_3 >> 2))]);
      }
      __syncthreads();
      scores_sum[i_15] = tl::AllReduce<tl::SumOp, 256, 128, 0, tl::NamedBarrier<256>>::run(scores_sum[i_15], (&(((float*)workspace_6)[0])));
      scores_sum[i_15] = tl::AllReduce<tl::SumOp, 4, 1, 0, tl::NamedBarrier<256>>::run(scores_sum[i_15]);
    }
    #pragma unroll
    for (int i_16 = 0; i_16 < 2; ++i_16) {
      tl::ptx_stmatrix_m8n8_x4((&(((half_t*)S_shared)[(((((((int)threadIdx.x) & 127) >> 5) * 1024) + (((((int)threadIdx.x) & 15) >> 3) * 512)) + ((((((((int)threadIdx.x) & 15) * 64) + ((((((int)threadIdx.x) >> 7) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 32)) + (((((((int)threadIdx.x) & 3) >> 1) + i_16) & 1) * 16)) + (((((((int)threadIdx.x) & 31) >> 4) + (((int)threadIdx.x) & 1)) & 1) * 8)) & 511))])), __pack_half2(((half_t)acc_s_v1[(i_16 * 8)]), ((half_t)acc_s_v1[((i_16 * 8) + 1)])), __pack_half2(((half_t)acc_s_v1[((i_16 * 8) + 2)]), ((half_t)acc_s_v1[((i_16 * 8) + 3)])), __pack_half2(((half_t)acc_s_v1[((i_16 * 8) + 4)]), ((half_t)acc_s_v1[((i_16 * 8) + 5)])), __pack_half2(((half_t)acc_s_v1[((i_16 * 8) + 6)]), ((half_t)acc_s_v1[((i_16 * 8) + 7)])));
    }
    #pragma unroll
    for (int i_17 = 0; i_17 < 2; ++i_17) {
      logsum[i_17] = ((logsum[i_17] * scores_scale_v0[i_17]) + scores_sum[i_17]);
    }
    #pragma unroll
    for (int i_18 = 0; i_18 < 128; ++i_18) {
      acc_o[i_18] = (acc_o[i_18] * scores_scale_v1[((i_18 & 3) >> 1)]);
    }
    ...
    *(float2*)(scores_max + 0) = make_float2(broadcast_var_6, broadcast_var_6);
    ...
    for (int i_19 = 0; i_19 < 2; ++i_19) {
      scores_max_clear_3[i_19] = -CUDART_INF_F;
      #pragma unroll
      for (int rv_4 = 0; rv_4 < 8; ++rv_4) {
        scores_max_clear_3[i_19] = max(scores_max_clear_3[i_19], acc_s_v0[((((rv_4 & 3) * 4) + (i_19 * 2)) + (rv_4 >> 2))]);
      }
      __syncthreads();
      scores_max_clear_3[i_19] = tl::AllReduce<tl::MaxOp, 256, 128, 0, tl::NamedBarrier<256>>::run(scores_max_clear_3[i_19], (&(((float*)workspace_1)[0])));
      scores_max_clear_3[i_19] = tl::AllReduce<tl::MaxOp, 4, 1, 0, tl::NamedBarrier<256>>::run(scores_max_clear_3[i_19]);
      scores_max[i_19] = max(scores_max[i_19], scores_max_clear_3[i_19]);
    }
    #pragma unroll
    ...
    for (int i_22 = 0; i_22 < 16; ++i_22) {
      acc_s_v0[i_22] = exp2f(((acc_s_v0[i_22] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/) - (scores_max[((i_22 & 3) >> 1)] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/)));
    }
    #pragma unroll
    for (int i_23 = 0; i_23 < 2; ++i_23) {
      ...
      for (int rv_5 = 0; rv_5 < 8; ++rv_5) {
        scores_sum[i_23] = (scores_sum[i_23] + acc_s_v1[((((rv_5 & 3) * 4) + (i_23 * 2)) + (rv_5 >> 2))]);
      }
      __syncthreads();
      scores_sum[i_23] = tl::AllReduce<tl::SumOp, 256, 128, 0, tl::NamedBarrier<256>>::run(scores_sum[i_23], (&(((float*)workspace_4)[0])));
      scores_sum[i_23] = tl::AllReduce<tl::SumOp, 4, 1, 0, tl::NamedBarrier<256>>::run(scores_sum[i_23]);
    }
    #pragma unroll
    for (int i_24 = 0; i_24 < 2; ++i_24) {
      tl::ptx_stmatrix_m8n8_x4((&(((half_t*)S_shared)[(((((((int)threadIdx.x) & 127) >> 5) * 1024) + (((((int)threadIdx.x) & 15) >> 3) * 512)) + ((((((((int)threadIdx.x) & 15) * 64) + ((((((int)threadIdx.x) >> 7) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 32)) + (((((((int)threadIdx.x) & 3) >> 1) + i_24) & 1) * 16)) + (((((((int)threadIdx.x) & 31) >> 4) + (((int)threadIdx.x) & 1)) & 1) * 8)) & 511))])), __pack_half2(((half_t)acc_s_v2[(i_24 * 8)]), ((half_t)acc_s_v2[((i_24 * 8) + 1)])), __pack_half2(((half_t)acc_s_v2[((i_24 * 8) + 2)]), ((half_t)acc_s_v2[((i_24 * 8) + 3)])), __pack_half2(((half_t)acc_s_v2[((i_24 * 8) + 4)]), ((half_t)acc_s_v2[((i_24 * 8) + 5)])), __pack_half2(((half_t)acc_s_v2[((i_24 * 8) + 6)]), ((half_t)acc_s_v2[((i_24 * 8) + 7)])));
    }
    #pragma unroll
    for (int i_25 = 0; i_25 < 2; ++i_25) {
      logsum[i_25] = ((logsum[i_25] * scores_scale_v1[i_25]) + scores_sum[i_25]);
    }
    #pragma unroll
    for (int i_26 = 0; i_26 < 128; ++i_26) {
      acc_o[i_26] = (acc_o[i_26] * scores_scale_v2[((i_26 & 3) >> 1)]);
    }
    overlap_plan_mbar[((k & 1) + 2)].wait(((((k * 3) >> 1) + 1) & 1));
    {
    ...
    *(float2*)(scores_max + 0) = make_float2(broadcast_var_7, broadcast_var_7);
    ...
    for (int i_27 = 0; i_27 < 2; ++i_27) {
      scores_max_clear_4[i_27] = -CUDART_INF_F;
      #pragma unroll
      for (int rv_6 = 0; rv_6 < 8; ++rv_6) {
        scores_max_clear_4[i_27] = max(scores_max_clear_4[i_27], acc_s_v1[((((rv_6 & 3) * 4) + (i_27 * 2)) + (rv_6 >> 2))]);
      }
      __syncthreads();
      scores_max_clear_4[i_27] = tl::AllReduce<tl::MaxOp, 256, 128, 0, tl::NamedBarrier<256>>::run(scores_max_clear_4[i_27], (&(((float*)workspace_5)[0])));
      scores_max_clear_4[i_27] = tl::AllReduce<tl::MaxOp, 4, 1, 0, tl::NamedBarrier<256>>::run(scores_max_clear_4[i_27]);
      scores_max[i_27] = max(scores_max[i_27], scores_max_clear_4[i_27]);
    }
    #pragma unroll
    ...
    for (int i_29 = 0; i_29 < 2; ++i_29) {
      scores_scale_v1[i_29] = exp2f(((scores_max_prev[i_29] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/) - (scores_max[i_29] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/)));
    }
    #pragma unroll
    for (int i_30 = 0; i_30 < 16; ++i_30) {
      acc_s_v1[i_30] = exp2f(((acc_s_v1[i_30] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/) - (scores_max[((i_30 & 3) >> 1)] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/)));
    }
    #pragma unroll
    for (int i_31 = 0; i_31 < 2; ++i_31) {
      ...
      for (int rv_7 = 0; rv_7 < 8; ++rv_7) {
        scores_sum[i_31] = (scores_sum[i_31] + acc_s_v2[((((rv_7 & 3) * 4) + (i_31 * 2)) + (rv_7 >> 2))]);
      }
      __syncthreads();
    ...
    for (int i_32 = 0; i_32 < 2; ++i_32) {
      tl::ptx_stmatrix_m8n8_x4((&(((half_t*)S_shared)[(((((((int)threadIdx.x) & 127) >> 5) * 1024) + (((((int)threadIdx.x) & 15) >> 3) * 512)) + ((((((((int)threadIdx.x) & 15) * 64) + ((((((int)threadIdx.x) >> 7) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 32)) + (((((((int)threadIdx.x) & 3) >> 1) + i_32) & 1) * 16)) + (((((((int)threadIdx.x) & 31) >> 4) + (((int)threadIdx.x) & 1)) & 1) * 8)) & 511))])), __pack_half2(((half_t)acc_s_v0[(i_32 * 8)]), ((half_t)acc_s_v0[((i_32 * 8) + 1)])), __pack_half2(((half_t)acc_s_v0[((i_32 * 8) + 2)]), ((half_t)acc_s_v0[((i_32 * 8) + 3)])), __pack_half2(((half_t)acc_s_v0[((i_32 * 8) + 4)]), ((half_t)acc_s_v0[((i_32 * 8) + 5)])), __pack_half2(((half_t)acc_s_v0[((i_32 * 8) + 6)]), ((half_t)acc_s_v0[((i_32 * 8) + 7)])));
    }
    #pragma unroll
    for (int i_33 = 0; i_33 < 2; ++i_33) {
      logsum[i_33] = ((logsum[i_33] * scores_scale_v2[i_33]) + scores_sum[i_33]);
    }
    #pragma unroll
    for (int i_34 = 0; i_34 < 128; ++i_34) {
      acc_o[i_34] = (acc_o[i_34] * scores_scale_v0[((i_34 & 3) >> 1)]);
    }
  ...
  for (int i_35 = 0; i_35 < 2; ++i_35) {
    ...
    for (int rv_8 = 0; rv_8 < 8; ++rv_8) {
      scores_sum[i_35] = (scores_sum[i_35] + acc_s_v0[((((rv_8 & 3) * 4) + (i_35 * 2)) + (rv_8 >> 2))]);
    }
    __syncthreads();
    scores_sum[i_35] = tl::AllReduce<tl::SumOp, 256, 128, 0, tl::NamedBarrier<256>>::run(scores_sum[i_35], (&(((float*)workspace_2)[0])));
    scores_sum[i_35] = tl::AllReduce<tl::SumOp, 4, 1, 0, tl::NamedBarrier<256>>::run(scores_sum[i_35]);
  }
  #pragma unroll
  for (int i_36 = 0; i_36 < 2; ++i_36) {
    tl::ptx_stmatrix_m8n8_x4((&(((half_t*)S_shared)[(((((((int)threadIdx.x) & 127) >> 5) * 1024) + (((((int)threadIdx.x) & 15) >> 3) * 512)) + ((((((((int)threadIdx.x) & 15) * 64) + ((((((int)threadIdx.x) >> 7) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 32)) + (((((((int)threadIdx.x) & 3) >> 1) + i_36) & 1) * 16)) + (((((((int)threadIdx.x) & 31) >> 4) + (((int)threadIdx.x) & 1)) & 1) * 8)) & 511))])), __pack_half2(((half_t)acc_s_v1[(i_36 * 8)]), ((half_t)acc_s_v1[((i_36 * 8) + 1)])), __pack_half2(((half_t)acc_s_v1[((i_36 * 8) + 2)]), ((half_t)acc_s_v1[((i_36 * 8) + 3)])), __pack_half2(((half_t)acc_s_v1[((i_36 * 8) + 4)]), ((half_t)acc_s_v1[((i_36 * 8) + 5)])), __pack_half2(((half_t)acc_s_v1[((i_36 * 8) + 6)]), ((half_t)acc_s_v1[((i_36 * 8) + 7)])));
  }
  #pragma unroll
  ...
  for (int i_39 = 0; i_39 < 2; ++i_39) {
    ...
    __syncthreads();
    scores_sum[i_39] = tl::AllReduce<tl::SumOp, 256, 128, 0, tl::NamedBarrier<256>>::run(scores_sum[i_39], (&(((float*)workspace_7)[0])));

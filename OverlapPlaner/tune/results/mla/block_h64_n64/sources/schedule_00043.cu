...
extern "C" __global__ void __launch_bounds__(256, 1) main_kernel(__grid_constant__ const CUtensorMap KV_desc, __grid_constant__ const CUtensorMap K_pe_desc, half_t* __restrict__ Output, __grid_constant__ const CUtensorMap Q_desc, __grid_constant__ const CUtensorMap Q_pe_desc) {
  ...
  *(float2*)(scores_max + 0) = make_float2(broadcast_var_4, broadcast_var_4);
  ...
  for (int k = 0; k < 42; ++k) {
    ...
    *(float2*)(scores_max + 0) = make_float2(broadcast_var_5, broadcast_var_5);
    ...
    for (int i_13 = 0; i_13 < 2; ++i_13) {
      ...
      __syncthreads();
      scores_max_clear_2[i_13] = tl::AllReduce<tl::MaxOp, 256, 128, 0, tl::NamedBarrier<256>>::run(scores_max_clear_2[i_13], (&(((float*)workspace_4)[0])));
    ...
    for (int i_17 = 0; i_17 < 2; ++i_17) {
      ...
      __syncthreads();
      scores_sum[i_17] = tl::AllReduce<tl::SumOp, 256, 128, 0, tl::NamedBarrier<256>>::run(scores_sum[i_17], (&(((float*)workspace_5)[0])));
    ...
    *(float2*)(scores_max + 0) = make_float2(broadcast_var_6, broadcast_var_6);
    ...
    for (int i_21 = 0; i_21 < 2; ++i_21) {
      ...
      __syncthreads();
      scores_max_clear_3[i_21] = tl::AllReduce<tl::MaxOp, 256, 128, 0, tl::NamedBarrier<256>>::run(scores_max_clear_3[i_21], (&(((float*)workspace_2)[0])));
    ...
    for (int i_25 = 0; i_25 < 2; ++i_25) {
      ...
      __syncthreads();
      scores_sum[i_25] = tl::AllReduce<tl::SumOp, 256, 128, 0, tl::NamedBarrier<256>>::run(scores_sum[i_25], (&(((float*)workspace_6)[0])));
    ...
    *(float2*)(scores_max + 0) = make_float2(broadcast_var_7, broadcast_var_7);
    ...
    for (int i_33 = 0; i_33 < 2; ++i_33) {
      ...
      __syncthreads();
      scores_sum[i_33] = tl::AllReduce<tl::SumOp, 256, 128, 0, tl::NamedBarrier<256>>::run(scores_sum[i_33], (&(((float*)workspace_7)[0])));
  ...
  for (int i_37 = 0; i_37 < 2; ++i_37) {
    ...
    __syncthreads();
    scores_sum[i_37] = tl::AllReduce<tl::SumOp, 256, 128, 0, tl::NamedBarrier<256>>::run(scores_sum[i_37], (&(((float*)workspace_9)[0])));

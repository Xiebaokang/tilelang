...
extern "C" __global__ void __launch_bounds__(384, 1) main_kernel(__grid_constant__ const CUtensorMap KV_desc, __grid_constant__ const CUtensorMap K_pe_desc, half_t* __restrict__ Output, __grid_constant__ const CUtensorMap Q_desc, __grid_constant__ const CUtensorMap Q_pe_desc) {
  ...
  float acc_o[128];
  float logsum[2];
  float scores_max_v0[2];
  float scores_max_prev[2];
  float acc_s_v0[16];
  float scores_scale_v0[2];
  float scores_max_v1[2];
  float acc_s_v1[16];
  float scores_scale_v1[2];
  float scores_sum[2];
  float scores_max_v0_clear[2];
  float scores_max_v1_clear[2];
  float scores_max_v0_clear_1[2];
  float scores_max_v1_clear_1[2];
  ...
  if (((int)threadIdx.x) < 256) {
    ...
    *(float2*)(scores_max_v0 + 0) = make_float2(broadcast_var_2, broadcast_var_2);
    *(float2*)(scores_max_prev + 0) = *(float2*)(scores_max_v0 + 0);
    float broadcast_var_3 = -CUDART_INF_F;
    *(float2*)(scores_max_v0 + 0) = make_float2(broadcast_var_3, broadcast_var_3);
    ...
    for (int i_1 = 0; i_1 < 2; ++i_1) {
      scores_max_v0_clear[i_1] = -CUDART_INF_F;
      #pragma unroll
      for (int rv = 0; rv < 8; ++rv) {
        scores_max_v0_clear[i_1] = max(scores_max_v0_clear[i_1], acc_s_v0[((((rv & 3) * 4) + (i_1 * 2)) + (rv >> 2))]);
      }
      tl::__sync_thread_partial(3, 256);
      scores_max_v0_clear[i_1] = tl::AllReduce<tl::MaxOp, 256, 128, 0, tl::NamedBarrier<256>>::run(scores_max_v0_clear[i_1], (&(((float*)workspace_2)[0])));
      scores_max_v0_clear[i_1] = tl::AllReduce<tl::MaxOp, 4, 1, 0, tl::NamedBarrier<256>>::run(scores_max_v0_clear[i_1]);
      scores_max_v0[i_1] = max(scores_max_v0[i_1], scores_max_v0_clear[i_1]);
    }
    #pragma unroll
    for (int i_2 = 0; i_2 < 2; ++i_2) {
      scores_max_v0[i_2] = max(scores_max_v0[i_2], scores_max_prev[i_2]);
    }
    #pragma unroll
    for (int i_3 = 0; i_3 < 2; ++i_3) {
      scores_scale_v0[i_3] = exp2f(((scores_max_prev[i_3] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/) - (scores_max_v0[i_3] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/)));
    }
    #pragma unroll
    for (int i_4 = 0; i_4 < 128; ++i_4) {
      acc_o[i_4] = (acc_o[i_4] * scores_scale_v0[((i_4 & 3) >> 1)]);
    }
    for (int k = 0; k < 63; ++k) {
      *(float2*)(scores_max_prev + 0) = *(float2*)(scores_max_v1 + 0);
      float broadcast_var_4 = -CUDART_INF_F;
      *(float2*)(scores_max_v1 + 0) = make_float2(broadcast_var_4, broadcast_var_4);
      #pragma unroll
      for (int i_5 = 0; i_5 < 16; ++i_5) {
        acc_s_v0[i_5] = exp2f(((acc_s_v0[i_5] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/) - (scores_max_v0[((i_5 & 3) >> 1)] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/)));
      }
      ...
      for (int i_6 = 0; i_6 < 2; ++i_6) {
        scores_max_v1_clear[i_6] = -CUDART_INF_F;
        #pragma unroll
        for (int rv_1 = 0; rv_1 < 8; ++rv_1) {
          scores_max_v1_clear[i_6] = max(scores_max_v1_clear[i_6], acc_s_v1[((((rv_1 & 3) * 4) + (i_6 * 2)) + (rv_1 >> 2))]);
        }
        tl::__sync_thread_partial(3, 256);
        scores_max_v1_clear[i_6] = tl::AllReduce<tl::MaxOp, 256, 128, 0, tl::NamedBarrier<256>>::run(scores_max_v1_clear[i_6], (&(((float*)workspace_3)[0])));
        scores_max_v1_clear[i_6] = tl::AllReduce<tl::MaxOp, 4, 1, 0, tl::NamedBarrier<256>>::run(scores_max_v1_clear[i_6]);
        scores_max_v1[i_6] = max(scores_max_v1[i_6], scores_max_v1_clear[i_6]);
      }
      #pragma unroll
      for (int i_7 = 0; i_7 < 2; ++i_7) {
        scores_max_v1[i_7] = max(scores_max_v1[i_7], scores_max_prev[i_7]);
      }
      #pragma unroll
      for (int i_8 = 0; i_8 < 2; ++i_8) {
        scores_scale_v1[i_8] = exp2f(((scores_max_prev[i_8] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/) - (scores_max_v1[i_8] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/)));
      }
      #pragma unroll
      for (int i_9 = 0; i_9 < 2; ++i_9) {
        ...
        for (int rv_2 = 0; rv_2 < 8; ++rv_2) {
          scores_sum[i_9] = (scores_sum[i_9] + acc_s_v0[((((rv_2 & 3) * 4) + (i_9 * 2)) + (rv_2 >> 2))]);
        }
        tl::__sync_thread_partial(3, 256);
        scores_sum[i_9] = tl::AllReduce<tl::SumOp, 256, 128, 0, tl::NamedBarrier<256>>::run(scores_sum[i_9], (&(((float*)workspace_4)[0])));
        scores_sum[i_9] = tl::AllReduce<tl::SumOp, 4, 1, 0, tl::NamedBarrier<256>>::run(scores_sum[i_9]);
      }
      #pragma unroll
      for (int i_10 = 0; i_10 < 2; ++i_10) {
        tl::ptx_stmatrix_m8n8_x4((&(((half_t*)S_shared)[(((((((int)threadIdx.x) & 127) >> 5) * 1024) + (((((int)threadIdx.x) & 15) >> 3) * 512)) + ((((((((int)threadIdx.x) & 15) * 64) + ((((((int)threadIdx.x) >> 7) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 32)) + (((((((int)threadIdx.x) & 3) >> 1) + i_10) & 1) * 16)) + (((((((int)threadIdx.x) & 31) >> 4) + (((int)threadIdx.x) & 1)) & 1) * 8)) & 511))])), __pack_half2(((half_t)acc_s_v0[(i_10 * 8)]), ((half_t)acc_s_v0[((i_10 * 8) + 1)])), __pack_half2(((half_t)acc_s_v0[((i_10 * 8) + 2)]), ((half_t)acc_s_v0[((i_10 * 8) + 3)])), __pack_half2(((half_t)acc_s_v0[((i_10 * 8) + 4)]), ((half_t)acc_s_v0[((i_10 * 8) + 5)])), __pack_half2(((half_t)acc_s_v0[((i_10 * 8) + 6)]), ((half_t)acc_s_v0[((i_10 * 8) + 7)])));
      }
      #pragma unroll
      ...
      *(float2*)(scores_max_v0 + 0) = make_float2(broadcast_var_5, broadcast_var_5);
      #pragma unroll
      for (int i_13 = 0; i_13 < 16; ++i_13) {
        acc_s_v1[i_13] = exp2f(((acc_s_v1[i_13] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/) - (scores_max_v1[((i_13 & 3) >> 1)] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/)));
      }
      overlap_plan_mbar[2].wait(((k + 1) & 1));
      {
        ...
        for (int ki_5 = 0; ki_5 < 32; ++ki_5) {
          tl::wgmma_ss<tl::DataType::kFloat16, tl::DataType::kFloat16, tl::DataType::kFloat32, 64, 32, 16, false, false, 1, 1>(uint64_t(desc_a_5 + ((((ki_5 >> 2) * 8192) + ((ki_5 & 3) * 32)) >> 4)), uint64_t(desc_b_5 + (((((ki_5 >> 2) * 8192) + ((((int)threadIdx.x) >> 7) * 4096)) + ((ki_5 & 3) * 32)) >> 4)), ((uint32_t*)(acc_s_v0 + 0)), ((0 < ki_5) ? 1 : 0));
        }
        tl::warpgroup_commit_batch();
        tl::warpgroup_wait<0>();
        tl::warpgroup_fence_operand(reinterpret_cast<float*>(acc_s_v0 + 0), 16);
      ...
      overlap_plan_mbar[4].wait(((k + 1) & 1));
      {
        ...
        for (int ki_6 = 0; ki_6 < 4; ++ki_6) {
          tl::wgmma_ss<tl::DataType::kFloat16, tl::DataType::kFloat16, tl::DataType::kFloat32, 64, 32, 16, false, false, 1, 1>(uint64_t(desc_a_6 + ((ki_6 * 32) >> 4)), uint64_t(desc_b_6 + ((((((int)threadIdx.x) >> 7) * 4096) + (ki_6 * 32)) >> 4)), ((uint32_t*)(acc_s_v0 + 0)), 1);
        }
        tl::warpgroup_commit_batch();
        tl::warpgroup_wait<0>();
        tl::warpgroup_fence_operand(reinterpret_cast<float*>(acc_s_v0 + 0), 16);
      ...
      for (int i_14 = 0; i_14 < 2; ++i_14) {
        scores_max_v0_clear_1[i_14] = -CUDART_INF_F;
        #pragma unroll
        for (int rv_3 = 0; rv_3 < 8; ++rv_3) {
          scores_max_v0_clear_1[i_14] = max(scores_max_v0_clear_1[i_14], acc_s_v0[((((rv_3 & 3) * 4) + (i_14 * 2)) + (rv_3 >> 2))]);
        }
        tl::__sync_thread_partial(3, 256);
        scores_max_v0_clear_1[i_14] = tl::AllReduce<tl::MaxOp, 256, 128, 0, tl::NamedBarrier<256>>::run(scores_max_v0_clear_1[i_14], (&(((float*)workspace_6)[0])));
        scores_max_v0_clear_1[i_14] = tl::AllReduce<tl::MaxOp, 4, 1, 0, tl::NamedBarrier<256>>::run(scores_max_v0_clear_1[i_14]);
        scores_max_v0[i_14] = max(scores_max_v0[i_14], scores_max_v0_clear_1[i_14]);
      }
      #pragma unroll
      for (int i_15 = 0; i_15 < 2; ++i_15) {
        scores_max_v0[i_15] = max(scores_max_v0[i_15], scores_max_prev[i_15]);
      }
      #pragma unroll
      for (int i_16 = 0; i_16 < 2; ++i_16) {
        scores_scale_v0[i_16] = exp2f(((scores_max_prev[i_16] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/) - (scores_max_v0[i_16] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/)));
      }
      #pragma unroll
      for (int i_17 = 0; i_17 < 2; ++i_17) {
        ...
        for (int rv_4 = 0; rv_4 < 8; ++rv_4) {
          scores_sum[i_17] = (scores_sum[i_17] + acc_s_v1[((((rv_4 & 3) * 4) + (i_17 * 2)) + (rv_4 >> 2))]);
        }
        tl::__sync_thread_partial(3, 256);
        scores_sum[i_17] = tl::AllReduce<tl::SumOp, 256, 128, 0, tl::NamedBarrier<256>>::run(scores_sum[i_17], (&(((float*)workspace)[0])));
        scores_sum[i_17] = tl::AllReduce<tl::SumOp, 4, 1, 0, tl::NamedBarrier<256>>::run(scores_sum[i_17]);
      }
      #pragma unroll
      for (int i_18 = 0; i_18 < 2; ++i_18) {
        tl::ptx_stmatrix_m8n8_x4((&(((half_t*)S_shared)[(((((((int)threadIdx.x) & 127) >> 5) * 1024) + (((((int)threadIdx.x) & 15) >> 3) * 512)) + ((((((((int)threadIdx.x) & 15) * 64) + ((((((int)threadIdx.x) >> 7) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 32)) + (((((((int)threadIdx.x) & 3) >> 1) + i_18) & 1) * 16)) + (((((((int)threadIdx.x) & 31) >> 4) + (((int)threadIdx.x) & 1)) & 1) * 8)) & 511))])), __pack_half2(((half_t)acc_s_v1[(i_18 * 8)]), ((half_t)acc_s_v1[((i_18 * 8) + 1)])), __pack_half2(((half_t)acc_s_v1[((i_18 * 8) + 2)]), ((half_t)acc_s_v1[((i_18 * 8) + 3)])), __pack_half2(((half_t)acc_s_v1[((i_18 * 8) + 4)]), ((half_t)acc_s_v1[((i_18 * 8) + 5)])), __pack_half2(((half_t)acc_s_v1[((i_18 * 8) + 6)]), ((half_t)acc_s_v1[((i_18 * 8) + 7)])));
      }
      #pragma unroll
      for (int i_19 = 0; i_19 < 2; ++i_19) {
        logsum[i_19] = ((logsum[i_19] * scores_scale_v1[i_19]) + scores_sum[i_19]);
      }
      overlap_plan_mbar[3].wait((k & 1));
      {
      ...
      for (int i_20 = 0; i_20 < 128; ++i_20) {
        acc_o[i_20] = (acc_o[i_20] * scores_scale_v0[((i_20 & 3) >> 1)]);
      }
    }
    *(float2*)(scores_max_prev + 0) = *(float2*)(scores_max_v1 + 0);
    float broadcast_var_6 = -CUDART_INF_F;
    *(float2*)(scores_max_v1 + 0) = make_float2(broadcast_var_6, broadcast_var_6);
    #pragma unroll
    for (int i_21 = 0; i_21 < 16; ++i_21) {
      acc_s_v0[i_21] = exp2f(((acc_s_v0[i_21] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/) - (scores_max_v0[((i_21 & 3) >> 1)] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/)));
    }
    overlap_plan_mbar[3].wait(1);
    {
      ...
      for (int ki_8 = 0; ki_8 < 32; ++ki_8) {
        tl::wgmma_ss<tl::DataType::kFloat16, tl::DataType::kFloat16, tl::DataType::kFloat32, 64, 32, 16, false, false, 1, 1>(uint64_t(desc_a_8 + ((((ki_8 >> 2) * 8192) + ((ki_8 & 3) * 32)) >> 4)), uint64_t(desc_b_8 + (((((ki_8 >> 2) * 8192) + ((((int)threadIdx.x) >> 7) * 4096)) + ((ki_8 & 3) * 32)) >> 4)), ((uint32_t*)(acc_s_v1 + 0)), ((0 < ki_8) ? 1 : 0));
      }
      tl::warpgroup_commit_batch();
      tl::warpgroup_wait<0>();
      tl::warpgroup_fence_operand(reinterpret_cast<float*>(acc_s_v1 + 0), 16);
    ...
    overlap_plan_mbar[5].wait(1);
    {
      ...
      for (int ki_9 = 0; ki_9 < 4; ++ki_9) {
        tl::wgmma_ss<tl::DataType::kFloat16, tl::DataType::kFloat16, tl::DataType::kFloat32, 64, 32, 16, false, false, 1, 1>(uint64_t(desc_a_9 + ((ki_9 * 32) >> 4)), uint64_t(desc_b_9 + ((((((int)threadIdx.x) >> 7) * 4096) + (ki_9 * 32)) >> 4)), ((uint32_t*)(acc_s_v1 + 0)), 1);
      }
      tl::warpgroup_commit_batch();
      tl::warpgroup_wait<0>();
      tl::warpgroup_fence_operand(reinterpret_cast<float*>(acc_s_v1 + 0), 16);
    ...
    for (int i_22 = 0; i_22 < 2; ++i_22) {
      scores_max_v1_clear_1[i_22] = -CUDART_INF_F;
      #pragma unroll
      for (int rv_5 = 0; rv_5 < 8; ++rv_5) {
        scores_max_v1_clear_1[i_22] = max(scores_max_v1_clear_1[i_22], acc_s_v1[((((rv_5 & 3) * 4) + (i_22 * 2)) + (rv_5 >> 2))]);
      }
      tl::__sync_thread_partial(3, 256);
      scores_max_v1_clear_1[i_22] = tl::AllReduce<tl::MaxOp, 256, 128, 0, tl::NamedBarrier<256>>::run(scores_max_v1_clear_1[i_22], (&(((float*)workspace_5)[0])));
      scores_max_v1_clear_1[i_22] = tl::AllReduce<tl::MaxOp, 4, 1, 0, tl::NamedBarrier<256>>::run(scores_max_v1_clear_1[i_22]);
      scores_max_v1[i_22] = max(scores_max_v1[i_22], scores_max_v1_clear_1[i_22]);
    }
    #pragma unroll
    for (int i_23 = 0; i_23 < 2; ++i_23) {
      scores_max_v1[i_23] = max(scores_max_v1[i_23], scores_max_prev[i_23]);
    }
    #pragma unroll
    for (int i_24 = 0; i_24 < 2; ++i_24) {
      scores_scale_v1[i_24] = exp2f(((scores_max_prev[i_24] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/) - (scores_max_v1[i_24] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/)));
    }
    #pragma unroll
    for (int i_25 = 0; i_25 < 2; ++i_25) {
      ...
      for (int rv_6 = 0; rv_6 < 8; ++rv_6) {
        scores_sum[i_25] = (scores_sum[i_25] + acc_s_v0[((((rv_6 & 3) * 4) + (i_25 * 2)) + (rv_6 >> 2))]);
      }
      tl::__sync_thread_partial(3, 256);
      scores_sum[i_25] = tl::AllReduce<tl::SumOp, 256, 128, 0, tl::NamedBarrier<256>>::run(scores_sum[i_25], (&(((float*)workspace_7)[0])));
      scores_sum[i_25] = tl::AllReduce<tl::SumOp, 4, 1, 0, tl::NamedBarrier<256>>::run(scores_sum[i_25]);
    }
    #pragma unroll
    for (int i_26 = 0; i_26 < 2; ++i_26) {
      tl::ptx_stmatrix_m8n8_x4((&(((half_t*)S_shared)[(((((((int)threadIdx.x) & 127) >> 5) * 1024) + (((((int)threadIdx.x) & 15) >> 3) * 512)) + ((((((((int)threadIdx.x) & 15) * 64) + ((((((int)threadIdx.x) >> 7) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 32)) + (((((((int)threadIdx.x) & 3) >> 1) + i_26) & 1) * 16)) + (((((((int)threadIdx.x) & 31) >> 4) + (((int)threadIdx.x) & 1)) & 1) * 8)) & 511))])), __pack_half2(((half_t)acc_s_v0[(i_26 * 8)]), ((half_t)acc_s_v0[((i_26 * 8) + 1)])), __pack_half2(((half_t)acc_s_v0[((i_26 * 8) + 2)]), ((half_t)acc_s_v0[((i_26 * 8) + 3)])), __pack_half2(((half_t)acc_s_v0[((i_26 * 8) + 4)]), ((half_t)acc_s_v0[((i_26 * 8) + 5)])), __pack_half2(((half_t)acc_s_v0[((i_26 * 8) + 6)]), ((half_t)acc_s_v0[((i_26 * 8) + 7)])));
    }
    #pragma unroll
    ...
    for (int i_28 = 0; i_28 < 128; ++i_28) {
      acc_o[i_28] = (acc_o[i_28] * scores_scale_v1[((i_28 & 3) >> 1)]);
    }
    #pragma unroll
    for (int i_29 = 0; i_29 < 16; ++i_29) {
      acc_s_v1[i_29] = exp2f(((acc_s_v1[i_29] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/) - (scores_max_v1[((i_29 & 3) >> 1)] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/)));
    }
    #pragma unroll
    for (int i_30 = 0; i_30 < 2; ++i_30) {
      ...
      for (int rv_7 = 0; rv_7 < 8; ++rv_7) {
        scores_sum[i_30] = (scores_sum[i_30] + acc_s_v1[((((rv_7 & 3) * 4) + (i_30 * 2)) + (rv_7 >> 2))]);
      }
      tl::__sync_thread_partial(3, 256);
      scores_sum[i_30] = tl::AllReduce<tl::SumOp, 256, 128, 0, tl::NamedBarrier<256>>::run(scores_sum[i_30], (&(((float*)workspace_1)[0])));
      scores_sum[i_30] = tl::AllReduce<tl::SumOp, 4, 1, 0, tl::NamedBarrier<256>>::run(scores_sum[i_30]);
    }
    #pragma unroll
    for (int i_31 = 0; i_31 < 2; ++i_31) {
      tl::ptx_stmatrix_m8n8_x4((&(((half_t*)S_shared)[(((((((int)threadIdx.x) & 127) >> 5) * 1024) + (((((int)threadIdx.x) & 15) >> 3) * 512)) + ((((((((int)threadIdx.x) & 15) * 64) + ((((((int)threadIdx.x) >> 7) + ((((int)threadIdx.x) & 7) >> 2)) & 1) * 32)) + (((((((int)threadIdx.x) & 3) >> 1) + i_31) & 1) * 16)) + (((((((int)threadIdx.x) & 31) >> 4) + (((int)threadIdx.x) & 1)) & 1) * 8)) & 511))])), __pack_half2(((half_t)acc_s_v1[(i_31 * 8)]), ((half_t)acc_s_v1[((i_31 * 8) + 1)])), __pack_half2(((half_t)acc_s_v1[((i_31 * 8) + 2)]), ((half_t)acc_s_v1[((i_31 * 8) + 3)])), __pack_half2(((half_t)acc_s_v1[((i_31 * 8) + 4)]), ((half_t)acc_s_v1[((i_31 * 8) + 5)])), __pack_half2(((half_t)acc_s_v1[((i_31 * 8) + 6)]), ((half_t)acc_s_v1[((i_31 * 8) + 7)])));
    }
    #pragma unroll

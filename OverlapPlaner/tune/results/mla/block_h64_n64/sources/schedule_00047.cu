...
extern "C" __global__ void __launch_bounds__(256, 1) main_kernel(__grid_constant__ const CUtensorMap KV_desc, __grid_constant__ const CUtensorMap K_pe_desc, half_t* __restrict__ Output, __grid_constant__ const CUtensorMap Q_desc, __grid_constant__ const CUtensorMap Q_pe_desc) {
  ...
  *(float2*)(scores_max + 0) = make_float2(broadcast_var_4, broadcast_var_4);
  ...
  for (int k = 0; k < 42; ++k) {
    ...
    *(float2*)(scores_max + 0) = make_float2(broadcast_var_5, broadcast_var_5);
    ...
    for (int i_16 = 0; i_16 < 2; ++i_16) {
      ...
      __syncthreads();
      scores_sum[i_16] = tl::AllReduce<tl::SumOp, 256, 128, 0, tl::NamedBarrier<256>>::run(scores_sum[i_16], (&(((float*)workspace)[0])));
    ...
    *(float2*)(scores_max + 0) = make_float2(broadcast_var_7, broadcast_var_7);
    ...
    for (int i_28 = 0; i_28 < 2; ++i_28) {
      ...
      __syncthreads();
      scores_max_clear_4[i_28] = tl::AllReduce<tl::MaxOp, 256, 128, 0, tl::NamedBarrier<256>>::run(scores_max_clear_4[i_28], (&(((float*)workspace_7)[0])));
    ...
    for (int i_32 = 0; i_32 < 2; ++i_32) {
      ...
      __syncthreads();
      scores_sum[i_32] = tl::AllReduce<tl::SumOp, 256, 128, 0, tl::NamedBarrier<256>>::run(scores_sum[i_32], (&(((float*)workspace_6)[0])));
  ...
  for (int i_36 = 0; i_36 < 2; ++i_36) {
    ...
    __syncthreads();
    scores_sum[i_36] = tl::AllReduce<tl::SumOp, 256, 128, 0, tl::NamedBarrier<256>>::run(scores_sum[i_36], (&(((float*)workspace_3)[0])));
  ...
  for (int i_39 = 0; i_39 < 2; ++i_39) {
    ...
    __syncthreads();
    scores_sum[i_39] = tl::AllReduce<tl::SumOp, 256, 128, 0, tl::NamedBarrier<256>>::run(scores_sum[i_39], (&(((float*)workspace_4)[0])));

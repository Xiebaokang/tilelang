    ...
    *(float2*)(logsum + 0) = make_float2(broadcast_var_1, broadcast_var_1);
    ...
    for (int i_4 = 0; i_4 < 2; ++i_4) {
      ...
      for (int rv = 0; rv < 16; ++rv) {
        scores_sum[i_4] = (scores_sum[i_4] + acc_s[((((rv & 7) * 4) + (i_4 * 2)) + (rv >> 3))]);
      }
      scores_sum[i_4] = tl::AllReduce<tl::SumOp, 4, 1, 0, tl::NamedBarrier<128>>::run(scores_sum[i_4]);
    }
    overlap_plan_mbar[3].wait(0);
    #pragma unroll
    for (int i_5 = 0; i_5 < 2; ++i_5) {
      scores_scale[i_5] = ((float*)scores_scale_wsp_handoff_3)[((((((int)threadIdx.x) >> 5) * 16) + (i_5 * 8)) + ((((int)threadIdx.x) & 31) >> 2))];
    }
    #pragma unroll
    ...
    for (int k = 0; k < 31; ++k) {
      ...
      for (int i_12 = 0; i_12 < 2; ++i_12) {
        ...
        scores_sum[i_12] = tl::AllReduce<tl::SumOp, 4, 1, 0, tl::NamedBarrier<128>>::run(scores_sum[i_12]);
      }
      overlap_plan_mbar[3].wait(1);
      #pragma unroll
      for (int i_13 = 0; i_13 < 2; ++i_13) {
        scores_scale[i_13] = ((float*)scores_scale_wsp_handoff_3)[((((((int)threadIdx.x) >> 5) * 16) + (i_13 * 8)) + ((((int)threadIdx.x) & 31) >> 2))];
      }
      #pragma unroll
      ...
      for (int i_15 = 0; i_15 < 8; ++i_15) {
        ...
        *(uint2*)(acc_s_cast_v1 + (i_15 * 4)) = __2;
      }
      if (1 <= k) {
      ...
      for (int i_19 = 0; i_19 < 16; ++i_19) {
        *(float2*)(acc_s + (i_19 * 2)) = *(float2*)(((float*)acc_s_wsp_handoff_4) + ((((((((int)threadIdx.x) >> 5) * 1024) + ((i_19 & 1) * 512)) + (((((int)threadIdx.x) & 31) >> 2) * 64)) + ((i_19 >> 1) * 8)) + ((((int)threadIdx.x) & 3) * 2)));
      }
      #pragma unroll
      for (int i_20 = 0; i_20 < 2; ++i_20) {
        ...
        for (int rv_2 = 0; rv_2 < 16; ++rv_2) {
          scores_sum[i_20] = (scores_sum[i_20] + acc_s[((((rv_2 & 7) * 4) + (i_20 * 2)) + (rv_2 >> 3))]);
        }
      ...
      for (int i_21 = 0; i_21 < 2; ++i_21) {
        scores_scale[i_21] = ((float*)scores_scale_wsp_handoff_3)[((((((int)threadIdx.x) >> 5) * 16) + (i_21 * 8)) + ((((int)threadIdx.x) & 31) >> 2))];
      }
      #pragma unroll
      ...
      for (int i_23 = 0; i_23 < 8; ++i_23) {
        uint2 __3;
        float4 v__2 = *(float4*)(acc_s + (i_23 * 4));
        ((half2*)(&__3))[0] = __float22half2_rn(((float2*)(&v__2))[0]);
        ((half2*)(&__3))[1] = __float22half2_rn(((float2*)(&v__2))[1]);
        *(uint2*)(acc_s_cast_v0 + (i_23 * 4)) = __3;
      }
      overlap_plan_mbar[8].wait((k & 1));
      if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 0)) {
        overlap_plan_mbar[5].arrive_and_expect_tx(16384);
        tl::tma_load(V_desc, overlap_plan_mbar[5], (&(((half_t*)V_shared)[0])), 0, ((k * 128) + 128), (((int)blockIdx.y) >> 3), 0);
        tl::tma_load(V_desc, overlap_plan_mbar[5], (&(((half_t*)V_shared)[4096])), 64, ((k * 128) + 128), (((int)blockIdx.y) >> 3), 0);
      }
    ...
    for (int i_25 = 0; i_25 < 32; ++i_25) {
      acc_s[i_25] = 0x0p+0f/*0.000000e+00*/;
    }
    overlap_plan_mbar[1].wait(1);
    {
    ...
    for (int i_26 = 0; i_26 < 16; ++i_26) {
      *(float2*)(((float*)acc_s_wsp_handoff_2) + ((((((((int)threadIdx.x) >> 5) * 1024) + ((i_26 & 1) * 512)) + (((((int)threadIdx.x) & 31) >> 2) * 64)) + ((i_26 >> 1) * 8)) + ((((int)threadIdx.x) & 3) * 2))) = *(float2*)(acc_s + (i_26 * 2));
    }
    tl::fence_proxy_async();
    overlap_plan_mbar[2].arrive();
    overlap_plan_mbar[7].arrive();
    ...
    for (int i_27 = 0; i_27 < 16; ++i_27) {
      *(float2*)(acc_s + (i_27 * 2)) = *(float2*)(((float*)acc_s_wsp_handoff_4) + ((((((((int)threadIdx.x) >> 5) * 1024) + ((i_27 & 1) * 512)) + (((((int)threadIdx.x) & 31) >> 2) * 64)) + ((i_27 >> 1) * 8)) + ((((int)threadIdx.x) & 3) * 2)));
    }
    #pragma unroll
    for (int i_28 = 0; i_28 < 2; ++i_28) {
      ...
      scores_sum[i_28] = tl::AllReduce<tl::SumOp, 4, 1, 0, tl::NamedBarrier<128>>::run(scores_sum[i_28]);
    }
    overlap_plan_mbar[3].wait(1);
    #pragma unroll
    for (int i_29 = 0; i_29 < 2; ++i_29) {
      scores_scale[i_29] = ((float*)scores_scale_wsp_handoff_3)[((((((int)threadIdx.x) >> 5) * 16) + (i_29 * 8)) + ((((int)threadIdx.x) & 31) >> 2))];
    }
    #pragma unroll
    ...
    if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 0)) {
      overlap_plan_mbar[6].arrive_and_expect_tx(16384);
      tl::tma_load(V_desc, overlap_plan_mbar[6], (&(((half_t*)V_shared)[8192])), 0, 4032, (((int)blockIdx.y) >> 3), 0);
      tl::tma_load(V_desc, overlap_plan_mbar[6], (&(((half_t*)V_shared)[12288])), 64, 4032, (((int)blockIdx.y) >> 3), 0);
    }
    ...
    for (int i_34 = 0; i_34 < 8; ++i_34) {
      tl::ptx_stmatrix_m8n8_x4((&(((half_t*)O_shared)[(((((i_34 >> 2) * 4096) + ((((int)threadIdx.x) >> 5) * 1024)) + (((((int)threadIdx.x) & 15) >> 3) * 512)) + ((((((((int)threadIdx.x) & 15) * 64) + (((((((int)threadIdx.x) & 7) >> 2) + ((i_34 & 3) >> 1)) & 1) * 32)) + (((((((int)threadIdx.x) & 3) >> 1) + (i_34 & 1)) & 1) * 16)) + (((((((int)threadIdx.x) & 31) >> 4) + (((int)threadIdx.x) & 1)) & 1) * 8)) & 511))])), __pack_half2(((half_t)acc_o[(i_34 * 8)]), ((half_t)acc_o[((i_34 * 8) + 1)])), __pack_half2(((half_t)acc_o[((i_34 * 8) + 2)]), ((half_t)acc_o[((i_34 * 8) + 3)])), __pack_half2(((half_t)acc_o[((i_34 * 8) + 4)]), ((half_t)acc_o[((i_34 * 8) + 5)])), __pack_half2(((half_t)acc_o[((i_34 * 8) + 6)]), ((half_t)acc_o[((i_34 * 8) + 7)])));
    }
    tl::__sync_thread_partial(3, 128);
    ...
    for (int k_1 = 0; k_1 < 64; ++k_1) {
      ...
      *(float2*)(scores_max + 0) = make_float2(broadcast_var_3, broadcast_var_3);
      ...
      for (int i_35 = 0; i_35 < 16; ++i_35) {
        *(float2*)(acc_s + (i_35 * 2)) = *(float2*)(((float*)acc_s_wsp_handoff_2) + (((((((((int)threadIdx.x) >> 5) * 1024) + ((i_35 & 1) * 512)) + (((((int)threadIdx.x) & 31) >> 2) * 64)) + ((i_35 >> 1) * 8)) + ((((int)threadIdx.x) & 3) * 2)) - 4096));
      }
      #pragma unroll
      for (int i_36 = 0; i_36 < 2; ++i_36) {
        scores_max_clear[i_36] = -CUDART_INF_F;
        #pragma unroll
        for (int rv_4 = 0; rv_4 < 16; ++rv_4) {
          scores_max_clear[i_36] = max(scores_max_clear[i_36], acc_s[((((rv_4 & 7) * 4) + (i_36 * 2)) + (rv_4 >> 3))]);
        }
        scores_max_clear[i_36] = tl::AllReduce<tl::MaxOp, 4, 1, 0, tl::NamedBarrier<128>>::run(scores_max_clear[i_36]);
        scores_max[i_36] = max(scores_max[i_36], scores_max_clear[i_36]);
      }
      #pragma unroll
      ...
      if ((((int)threadIdx.x) % 4) == 0) {
        ...
        for (int i_39 = 0; i_39 < 2; ++i_39) {
          ((float*)scores_scale_wsp_handoff_3)[(((((((int)threadIdx.x) >> 5) * 16) + (i_39 * 8)) + ((((int)threadIdx.x) & 31) >> 2)) - 64)] = scores_scale[i_39];
        }
      }
      tl::fence_proxy_async();
      ...
      for (int i_41 = 0; i_41 < 16; ++i_41) {
        *(float2*)(((float*)acc_s_wsp_handoff_4) + (((((((((int)threadIdx.x) >> 5) * 1024) + ((i_41 & 1) * 512)) + (((((int)threadIdx.x) & 31) >> 2) * 64)) + ((i_41 >> 1) * 8)) + ((((int)threadIdx.x) & 3) * 2)) - 4096)) = *(float2*)(acc_s + (i_41 * 2));
      }
      tl::fence_proxy_async();

...
extern "C" __global__ void __launch_bounds__(256, 1) main_kernel(__grid_constant__ const CUtensorMap K_desc, __grid_constant__ const CUtensorMap Output_desc, __grid_constant__ const CUtensorMap Q_desc, __grid_constant__ const CUtensorMap V_desc) {
  ...
  if (tl::tl_shuffle_elect<256>()) {
    ...
    tl::tma_load(Q_desc, mbarrier[8], (&(((half_t*)Q_shared)[0])), 0, (((int)blockIdx.x) * 64), ((int)blockIdx.y), 0);
    tl::tma_load(Q_desc, mbarrier[8], (&(((half_t*)Q_shared)[4096])), 64, (((int)blockIdx.x) * 64), ((int)blockIdx.y), 0);
  }
    ...
    for (int k_1 = 0; k_1 < 64; ++k_1) {
      ...
      *(float2*)(scores_max + 0) = make_float2(broadcast_var_3, broadcast_var_3);
      ...
      for (int i_2 = 0; i_2 < 2; ++i_2) {
        ...
        scores_max_clear[i_2] = tl::AllReduce<tl::MaxOp, 4, 1, 128, tl::NamedBarrier<128>>::run(scores_max_clear[i_2]);
      ...
      for (int i_6 = 0; i_6 < 2; ++i_6) {
        ...
        scores_sum[i_6] = tl::AllReduce<tl::SumOp, 4, 1, 128, tl::NamedBarrier<128>>::run(scores_sum[i_6]);
      }
      #pragma unroll

...
extern "C" __global__ void __launch_bounds__(128, 1) main_kernel(__grid_constant__ const CUtensorMap CB_desc, __grid_constant__ const CUtensorMap C_desc, const half_t* __restrict__ D, const half_t* __restrict__ DA, const half_t* __restrict__ Dt, __grid_constant__ const CUtensorMap Output_desc, __grid_constant__ const CUtensorMap Prev_desc, __grid_constant__ const CUtensorMap X_desc, __grid_constant__ const CUtensorMap X_desc_1) {
  ...
  if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 0)) {
    ...
    tl::tma_load(CB_desc, overlap_plan_mbar[3], (&(((half_t*)cb_shared)[4096])), 64, ((((int)blockIdx.y) >> 1) * 64), 0, (((int)blockIdx.z) >> 3), (((int)blockIdx.z) & 7));
  }
  ...
  if (3 < ((int)blockIdx.y)) {
    ...
    if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 0)) {
      overlap_plan_mbar[3].arrive_and_expect_tx(16384);
      tl::tma_load(CB_desc, overlap_plan_mbar[3], (&(((half_t*)cb_shared)[0])), 128, ((((int)blockIdx.y) >> 1) * 64), 0, (((int)blockIdx.z) >> 3), (((int)blockIdx.z) & 7));
      tl::tma_load(CB_desc, overlap_plan_mbar[3], (&(((half_t*)cb_shared)[4096])), 192, ((((int)blockIdx.y) >> 1) * 64), 0, (((int)blockIdx.z) >> 3), (((int)blockIdx.z) & 7));
    }
    overlap_plan_mbar[3].wait(1);
    #pragma unroll
    for (int i_7 = 0; i_7 < 8; ++i_7) {
      tl::ptx_ldmatrix_x4((&(((half_t*)cb_shared)[(((((i_7 >> 2) * 4096) + ((((int)threadIdx.x) >> 5) * 1024)) + (((((int)threadIdx.x) & 15) >> 3) * 512)) + ((((((((int)threadIdx.x) & 15) * 64) + (((((((int)threadIdx.x) & 7) >> 2) + ((i_7 & 3) >> 1)) & 1) * 32)) + (((((((int)threadIdx.x) & 3) >> 1) + (i_7 & 1)) & 1) * 16)) + (((((((int)threadIdx.x) & 31) >> 4) + (((int)threadIdx.x) & 1)) & 1) * 8)) & 511))])), (&(cb_local_v1[(i_7 * 8)])));
    }
    overlap_plan_mbar[11].arrive();
  }
  __syncthreads();
  ...
  if ((((int)blockIdx.y) >> 2) == 0) {
    ...
    for (int i_11 = 0; i_11 < 16; ++i_11) {
      ...
      *(float2*)(dt_local_v0 + (i_11 * 2)) = __16;
    }
  } else {
    #pragma unroll
    for (int i_12 = 0; i_12 < 16; ++i_12) {
      half_t dt_shared_local_cast_4[2];
      *(uint1*)(dt_shared_local_cast_4 + 0) = *(uint1*)(((half_t*)dt_shared) + (((i_12 * 8) + ((((int)threadIdx.x) & 3) * 2)) + 128));
      float2 __17;
      uint1 v__12 = *(uint1*)(dt_shared_local_cast_4 + 0);
      ((float2*)(&__17))[0] = __half22float2(((half2*)(&v__12))[0]);
      *(float2*)(dt_local_v1 + (i_12 * 2)) = __17;
    }
  }
  overlap_plan_mbar[((((int)blockIdx.y) >> 2) + 13)].arrive();
  if (4 <= ((int)blockIdx.y)) {
    #pragma unroll
  ...
  if ((((int)blockIdx.y) >> 2) == 0) {
    ...
    for (int i_16 = 0; i_16 < 64; ++i_16) {
      half_t condval;
      if ((((((i_16 >> 2) * 8) + ((((int)threadIdx.x) & 3) * 2)) + (i_16 & 1)) <= (((((((int)blockIdx.y) >> 1) * 64) + ((((int)threadIdx.x) >> 5) * 16)) + (((i_16 & 3) >> 1) * 8)) + ((((int)threadIdx.x) & 31) >> 2)))) {
        condval = cb_local_v0[i_16];
      } else {
        condval = half_t(0x0p+0f/*0.000000e+00*/);
      }
      cb_local_v0[i_16] = condval;
    }
  } else {
    #pragma unroll
    for (int i_17 = 0; i_17 < 64; ++i_17) {
      half_t condval_1;
      if (((((((i_17 >> 2) * 8) + ((((int)threadIdx.x) & 3) * 2)) + (i_17 & 1)) + 128) <= (((((((int)blockIdx.y) >> 1) * 64) + ((((int)threadIdx.x) >> 5) * 16)) + (((i_17 & 3) >> 1) * 8)) + ((((int)threadIdx.x) & 31) >> 2)))) {
  ...
  overlap_plan_mbar[((((int)blockIdx.y) >> 2) + 7)].wait(0);
  if ((((int)blockIdx.y) >> 2) == 0) {
    {
      ...
      for (int ki_2 = 0; ki_2 < 8; ++ki_2) {
        tl::wgmma_rs<tl::DataType::kFloat16, tl::DataType::kFloat16, tl::DataType::kFloat32, 64, 32, 16, false, true, 1, 1>(reinterpret_cast<const uint32_t*>(cb_local_v0 + (ki_2 * 8)), uint64_t(desc_b_2 + ((ki_2 * 1024) >> 4)), reinterpret_cast<uint32_t*>(acc + 0), 1);
      }
      tl::warpgroup_commit_batch();
      tl::warpgroup_wait<0>();
      tl::warpgroup_fence_operand(reinterpret_cast<float*>(acc + 0), 16);
  ...
  } else {
    {
      ...
      for (int ki_3 = 0; ki_3 < 8; ++ki_3) {
        tl::wgmma_rs<tl::DataType::kFloat16, tl::DataType::kFloat16, tl::DataType::kFloat32, 64, 32, 16, false, true, 1, 1>(reinterpret_cast<const uint32_t*>(cb_local_v1 + (ki_3 * 8)), uint64_t(desc_b_3 + ((ki_3 * 1024) >> 4)), reinterpret_cast<uint32_t*>(acc + 0), 1);
      }
      tl::warpgroup_commit_batch();
      tl::warpgroup_wait<0>();
      tl::warpgroup_fence_operand(reinterpret_cast<float*>(acc + 0), 16);
  ...
  overlap_plan_mbar[((((int)blockIdx.y) >> 2) + 15)].arrive();
  d_local[0] = ((float)D[((int)blockIdx.x)]);
  if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 0)) {
    overlap_plan_mbar[10].arrive_and_expect_tx(4096);
    tl::tma_load(X_desc_1, overlap_plan_mbar[10], (&(((half_t*)residual_shared)[0])), ((((int)blockIdx.y) & 1) * 32), (((((int)blockIdx.z) >> 3) * 256) + ((((int)blockIdx.y) >> 1) * 64)), ((int)blockIdx.x), (((int)blockIdx.z) & 7));
  }
  overlap_plan_mbar[10].wait(0);
  #pragma unroll
  for (int i_18 = 0; i_18 < 8; ++i_18) {
    half_t residual_shared_local_cast_5[2];
    *(uint1*)(residual_shared_local_cast_5 + 0) = *(uint1*)(((half_t*)residual_shared) + (((((((((int)threadIdx.x) >> 5) * 512) + ((i_18 & 1) * 256)) + (((((int)threadIdx.x) & 31) >> 2) * 32)) + (((((((int)threadIdx.x) & 31) >> 4) + (i_18 >> 2)) & 1) * 16)) + (((((((int)threadIdx.x) & 15) >> 3) + ((i_18 & 3) >> 1)) & 1) * 8)) + ((((int)threadIdx.x) & 3) * 2)));
    float2 __27;
    uint1 v__19 = *(uint1*)(residual_shared_local_cast_5 + 0);
    ((float2*)(&__27))[0] = __half22float2(((half2*)(&v__19))[0]);
    *(float2*)(residual_local + (i_18 * 2)) = __27;
  }
  #pragma unroll

...
extern "C" __global__ void __launch_bounds__(256, 1) main_kernel(__grid_constant__ const CUtensorMap CB_desc, __grid_constant__ const CUtensorMap C_desc, const half_t* __restrict__ D, const half_t* __restrict__ DA, const half_t* __restrict__ Dt, __grid_constant__ const CUtensorMap Output_desc, __grid_constant__ const CUtensorMap Prev_desc, __grid_constant__ const CUtensorMap X_desc) {
  ...
  if (((int)threadIdx.x) < 128) {
    ...
    if (((int)blockIdx.y) == 1) {
      overlap_plan_mbar[(((int)blockIdx.y) + 8)].wait((((((int)blockIdx.y) + 3) & 3) >> 1));
      {
    ...
    for (int ik = 0; ik < ((int)blockIdx.y); ++ik) {
      if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 4)) {
        ...
        overlap_plan_mbar[(ik + 5)].arrive_and_expect_tx(256);
        tl::tma_load((&(((half_t*)da_k_shared)[((ik * 128) + 128)])), (&(DA[((((((((int)blockIdx.z) & 7) * 327680) + (((int)blockIdx.x) * 4096)) + ((((int)blockIdx.z) >> 3) * 256)) + (ik * 128)) + 128)])), overlap_plan_mbar[(ik + 5)], 256);
        overlap_plan_mbar[(ik + 8)].arrive_and_expect_tx(256);
        tl::tma_load((&(((half_t*)dt_shared)[((ik * 128) + 128)])), (&(Dt[((((((((int)blockIdx.z) & 7) * 327680) + (((int)blockIdx.x) * 4096)) + ((((int)blockIdx.z) >> 3) * 256)) + (ik * 128)) + 128)])), overlap_plan_mbar[(ik + 8)], 256);
        overlap_plan_mbar[(ik + 9)].arrive_and_expect_tx(16384);
        tl::tma_load(X_desc, overlap_plan_mbar[(ik + 9)], (&(((half_t*)x_shared)[(ik * 8192)])), 0, (((((int)blockIdx.z) >> 3) * 256) + (ik * 128)), ((int)blockIdx.x), (((int)blockIdx.z) & 7));
      }
    }
    if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 4)) {

  ...
  if (((int)threadIdx.x) < 128) {
    ...
    for (int ik = 0; ik < ((((int)blockIdx.y) * 2) + 2); ++ik) {
      ...
      overlap_plan_mbar[((ik % 3) + 5)].wait((ik / 3));
      {
        ...
        tl::increase_descriptor_offset<int>(desc_b_1, ((ik % 3) * 8192));
    ...
    for (int ik_1 = 0; ik_1 < ((((int)blockIdx.y) * 2) + 2); ++ik_1) {
      ...
      if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 4)) {
        overlap_plan_mbar[((ik_1 % 3) + 5)].arrive_and_expect_tx(8192);
        tl::tma_load(X_desc_1, overlap_plan_mbar[((ik_1 % 3) + 5)], (&(((half_t*)x_shared)[((ik_1 % 3) * 4096)])), 0, (((((int)blockIdx.z) >> 3) * 256) + (ik_1 * 64)), ((int)blockIdx.x), (((int)blockIdx.z) & 7));
      }
    }
  }
}

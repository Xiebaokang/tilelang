...
extern "C" __global__ void __launch_bounds__(256, 1) main_kernel(__grid_constant__ const CUtensorMap KV_desc, __grid_constant__ const CUtensorMap K_pe_desc, half_t* __restrict__ Output, __grid_constant__ const CUtensorMap Q_desc, __grid_constant__ const CUtensorMap Q_pe_desc) {
  ...
  *(float2*)(scores_max + 0) = make_float2(broadcast_var_4, broadcast_var_4);
  ...
  for (int i_7 = 0; i_7 < 2; ++i_7) {
    ...
    __syncthreads();
    scores_max_clear_1[i_7] = tl::AllReduce<tl::MaxOp, 256, 128, 0, tl::NamedBarrier<256>>::run(scores_max_clear_1[i_7], (&(((float*)workspace_1)[0])));
  ...
  for (int i_11 = 0; i_11 < 8; ++i_11) {
    ...
    uint1 __2;
    float2 v__1 = *(float2*)(acc_s_v1 + (i_11 * 2));
    ((half2*)(&__2))[0] = __float22half2_rn(((float2*)(&v__1))[0]);
    *(uint1*)(S_shared_local_cast_1 + 0) = __2;
    *(uint1*)(((half_t*)S_shared) + ((((((((((((int)threadIdx.x) & 127) >> 5) * 1024) + ((i_11 & 1) * 512)) + (((((int)threadIdx.x) & 31) >> 2) * 64)) + ((((((int)threadIdx.x) >> 7) + ((((int)threadIdx.x) & 31) >> 4)) & 1) * 32)) + (((((((int)threadIdx.x) & 15) >> 3) + (i_11 >> 2)) & 1) * 16)) + (((((((int)threadIdx.x) & 7) >> 2) + ((i_11 & 3) >> 1)) & 1) * 8)) + ((((int)threadIdx.x) & 3) * 2)) + 4096)) = *(uint1*)(S_shared_local_cast_1 + 0);
  }
  overlap_plan_mbar[2].wait(0);
  {
  ...
  for (int k = 0; k < 42; ++k) {
    ...
    *(float2*)(scores_max + 0) = make_float2(broadcast_var_5, broadcast_var_5);
    ...
    for (int i_13 = 0; i_13 < 2; ++i_13) {
      ...
      __syncthreads();
      scores_max_clear_2[i_13] = tl::AllReduce<tl::MaxOp, 256, 128, 0, tl::NamedBarrier<256>>::run(scores_max_clear_2[i_13], (&(((float*)workspace_9)[0])));
    ...
    for (int i_15 = 0; i_15 < 2; ++i_15) {
      scores_scale_v2[i_15] = exp2f(((scores_max_prev[i_15] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/) - (scores_max[i_15] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/)));
    }
    #pragma unroll
    for (int i_16 = 0; i_16 < 16; ++i_16) {
      acc_s_v2[i_16] = exp2f(((acc_s_v2[i_16] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/) - (scores_max[((i_16 & 3) >> 1)] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/)));
    }
    #pragma unroll
    for (int i_17 = 0; i_17 < 2; ++i_17) {
      ...
      for (int rv_3 = 0; rv_3 < 8; ++rv_3) {
        scores_sum[i_17] = (scores_sum[i_17] + acc_s_v0[((((rv_3 & 3) * 4) + (i_17 * 2)) + (rv_3 >> 2))]);
      }
      __syncthreads();
    ...
    for (int i_18 = 0; i_18 < 8; ++i_18) {
      ...
      uint1 __3;
      float2 v__2 = *(float2*)(acc_s_v2 + (i_18 * 2));
      ((half2*)(&__3))[0] = __float22half2_rn(((float2*)(&v__2))[0]);
      *(uint1*)(S_shared_local_cast_2 + 0) = __3;
      *(uint1*)(((half_t*)S_shared) + (((((((((k & 1) * 4096) + (((((int)threadIdx.x) & 127) >> 5) * 1024)) + ((i_18 & 1) * 512)) + (((((int)threadIdx.x) & 31) >> 2) * 64)) + ((((((int)threadIdx.x) >> 7) + ((((int)threadIdx.x) & 31) >> 4)) & 1) * 32)) + (((((((int)threadIdx.x) & 15) >> 3) + (i_18 >> 2)) & 1) * 16)) + (((((((int)threadIdx.x) & 7) >> 2) + ((i_18 & 3) >> 1)) & 1) * 8)) + ((((int)threadIdx.x) & 3) * 2))) = *(uint1*)(S_shared_local_cast_2 + 0);
    }
    #pragma unroll
    for (int i_19 = 0; i_19 < 2; ++i_19) {
      logsum[i_19] = ((logsum[i_19] * scores_scale_v0[i_19]) + scores_sum[i_19]);
    }
    ...
    for (int i_20 = 0; i_20 < 128; ++i_20) {
      acc_o[i_20] = (acc_o[i_20] * scores_scale_v2[((i_20 & 3) >> 1)]);
    }
    ...
    *(float2*)(scores_max + 0) = make_float2(broadcast_var_6, broadcast_var_6);
    ...
    for (int i_23 = 0; i_23 < 2; ++i_23) {
      scores_scale_v0[i_23] = exp2f(((scores_max_prev[i_23] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/) - (scores_max[i_23] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/)));
    }
    #pragma unroll
    for (int i_24 = 0; i_24 < 16; ++i_24) {
      acc_s_v0[i_24] = exp2f(((acc_s_v0[i_24] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/) - (scores_max[((i_24 & 3) >> 1)] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/)));
    }
    #pragma unroll
    for (int i_25 = 0; i_25 < 2; ++i_25) {
      ...
      for (int rv_5 = 0; rv_5 < 8; ++rv_5) {
        scores_sum[i_25] = (scores_sum[i_25] + acc_s_v1[((((rv_5 & 3) * 4) + (i_25 * 2)) + (rv_5 >> 2))]);
      }
      __syncthreads();
      scores_sum[i_25] = tl::AllReduce<tl::SumOp, 256, 128, 0, tl::NamedBarrier<256>>::run(scores_sum[i_25], (&(((float*)workspace_5)[0])));
    ...
    for (int i_26 = 0; i_26 < 8; ++i_26) {
      ...
      uint1 __4;
      float2 v__3 = *(float2*)(acc_s_v0 + (i_26 * 2));
      ((half2*)(&__4))[0] = __float22half2_rn(((float2*)(&v__3))[0]);
      *(uint1*)(S_shared_local_cast_3 + 0) = __4;
      *(uint1*)(((half_t*)S_shared) + (((((((((((k * 3) + 1) & 1) * 4096) + (((((int)threadIdx.x) & 127) >> 5) * 1024)) + ((i_26 & 1) * 512)) + (((((int)threadIdx.x) & 31) >> 2) * 64)) + ((((((int)threadIdx.x) >> 7) + ((((int)threadIdx.x) & 31) >> 4)) & 1) * 32)) + (((((((int)threadIdx.x) & 15) >> 3) + (i_26 >> 2)) & 1) * 16)) + (((((((int)threadIdx.x) & 7) >> 2) + ((i_26 & 3) >> 1)) & 1) * 8)) + ((((int)threadIdx.x) & 3) * 2))) = *(uint1*)(S_shared_local_cast_3 + 0);
    }
    #pragma unroll
    for (int i_27 = 0; i_27 < 2; ++i_27) {
      logsum[i_27] = ((logsum[i_27] * scores_scale_v1[i_27]) + scores_sum[i_27]);
    }
    overlap_plan_mbar[((k & 1) + 2)].wait(((((k * 3) >> 1) + 1) & 1));
    {
    ...
    for (int i_28 = 0; i_28 < 128; ++i_28) {
      acc_o[i_28] = (acc_o[i_28] * scores_scale_v0[((i_28 & 3) >> 1)]);
    }
    ...
    *(float2*)(scores_max + 0) = make_float2(broadcast_var_7, broadcast_var_7);
    ...
    for (int i_29 = 0; i_29 < 2; ++i_29) {
      ...
      __syncthreads();
      scores_max_clear_4[i_29] = tl::AllReduce<tl::MaxOp, 256, 128, 0, tl::NamedBarrier<256>>::run(scores_max_clear_4[i_29], (&(((float*)workspace_2)[0])));
    ...
    for (int i_31 = 0; i_31 < 2; ++i_31) {
      scores_scale_v1[i_31] = exp2f(((scores_max_prev[i_31] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/) - (scores_max[i_31] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/)));
    }
    #pragma unroll
    for (int i_32 = 0; i_32 < 16; ++i_32) {
      acc_s_v1[i_32] = exp2f(((acc_s_v1[i_32] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/) - (scores_max[((i_32 & 3) >> 1)] * 0x1.ec709dbe8903ep-5f/*6.011229e-02*/)));
    }
    #pragma unroll
    for (int i_33 = 0; i_33 < 2; ++i_33) {
      ...
      for (int rv_7 = 0; rv_7 < 8; ++rv_7) {
        scores_sum[i_33] = (scores_sum[i_33] + acc_s_v2[((((rv_7 & 3) * 4) + (i_33 * 2)) + (rv_7 >> 2))]);
      }
      __syncthreads();
      scores_sum[i_33] = tl::AllReduce<tl::SumOp, 256, 128, 0, tl::NamedBarrier<256>>::run(scores_sum[i_33], (&(((float*)workspace_3)[0])));
      scores_sum[i_33] = tl::AllReduce<tl::SumOp, 4, 1, 0, tl::NamedBarrier<256>>::run(scores_sum[i_33]);
    }
    #pragma unroll
    for (int i_34 = 0; i_34 < 8; ++i_34) {
      half_t S_shared_local_cast_4[2];
      uint1 __5;
      float2 v__4 = *(float2*)(acc_s_v1 + (i_34 * 2));
      ((half2*)(&__5))[0] = __float22half2_rn(((float2*)(&v__4))[0]);
      *(uint1*)(S_shared_local_cast_4 + 0) = __5;
      *(uint1*)(((half_t*)S_shared) + (((((((((k & 1) * 4096) + (((((int)threadIdx.x) & 127) >> 5) * 1024)) + ((i_34 & 1) * 512)) + (((((int)threadIdx.x) & 31) >> 2) * 64)) + ((((((int)threadIdx.x) >> 7) + ((((int)threadIdx.x) & 31) >> 4)) & 1) * 32)) + (((((((int)threadIdx.x) & 15) >> 3) + (i_34 >> 2)) & 1) * 16)) + (((((((int)threadIdx.x) & 7) >> 2) + ((i_34 & 3) >> 1)) & 1) * 8)) + ((((int)threadIdx.x) & 3) * 2))) = *(uint1*)(S_shared_local_cast_4 + 0);
    }
    #pragma unroll
    for (int i_35 = 0; i_35 < 2; ++i_35) {
      logsum[i_35] = ((logsum[i_35] * scores_scale_v2[i_35]) + scores_sum[i_35]);
    }
  ...
  for (int i_37 = 0; i_37 < 2; ++i_37) {
    ...
    __syncthreads();
    scores_sum[i_37] = tl::AllReduce<tl::SumOp, 256, 128, 0, tl::NamedBarrier<256>>::run(scores_sum[i_37], (&(((float*)workspace_8)[0])));

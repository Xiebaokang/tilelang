...
extern "C" __global__ void __launch_bounds__(128, 1) main_kernel(__grid_constant__ const CUtensorMap B_desc, const half_t* __restrict__ DA, const half_t* __restrict__ Dt, half_t* __restrict__ Output, __grid_constant__ const CUtensorMap X_desc) {
  ...
  for (int ik = 0; ik < 3; ++ik) {
    ...
    if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 0)) {
      overlap_plan_mbar[(((ik + 1) % 3) + 3)].arrive_and_expect_tx(16384);
      tl::tma_load(B_desc, overlap_plan_mbar[(((ik + 1) % 3) + 3)], (&(((half_t*)b_shared)[(((ik + 1) % 3) * 8192)])), 0, ((((((int)blockIdx.z) >> 3) * 256) + (ik * 64)) + 64), 0, (((int)blockIdx.z) & 7));
      tl::tma_load(B_desc, overlap_plan_mbar[(((ik + 1) % 3) + 3)], (&(((half_t*)b_shared)[((((ik + 1) % 3) * 8192) + 4096)])), 64, ((((((int)blockIdx.z) >> 3) * 256) + (ik * 64)) + 64), 0, (((int)blockIdx.z) & 7));
    }
    #pragma unroll

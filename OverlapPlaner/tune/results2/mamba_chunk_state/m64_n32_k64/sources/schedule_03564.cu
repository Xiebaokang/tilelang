...
extern "C" __global__ void __launch_bounds__(256, 1) main_kernel(__grid_constant__ const CUtensorMap B_desc, const half_t* __restrict__ DA, const half_t* __restrict__ Dt, __grid_constant__ const CUtensorMap Output_desc, __grid_constant__ const CUtensorMap X_desc) {
  ...
  if (((int)threadIdx.x) < 128) {
    ...
    for (int i_18 = 0; i_18 < 32; ++i_18) {
      x_local_v1[i_18] = ((half_t*)x_shared)[(((((((((i_18 >> 2) * 512) + ((((int)threadIdx.x) & 3) * 128)) + (((i_18 & 3) >> 1) * 64)) + ((((((int)threadIdx.x) >> 6) + ((((int)threadIdx.x) & 3) >> 1)) & 1) * 32)) + (((((((int)threadIdx.x) & 63) >> 5) + (((int)threadIdx.x) & 1)) & 1) * 16)) + (((((i_18 & 3) >> 1) + (i_18 & 1)) & 1) * 8)) + ((((int)threadIdx.x) & 31) >> 2)) + 4096)];
    }
    overlap_plan_mbar[11].arrive();
    #pragma unroll
    ...
    for (int ik_1 = 0; ik_1 < 4; ++ik_1) {
      ...
      if (tl::tl_shuffle_elect<128>() && ((((int)threadIdx.x) >> 5) == 4)) {
        ...
        tl::tma_load(B_desc, overlap_plan_mbar[((ik_1 & 1) + 8)], (&(((half_t*)b_shared)[((ik_1 & 1) * 2048)])), (((int)blockIdx.y) * 32), (((((int)blockIdx.z) >> 3) * 256) + (ik_1 * 64)), 0, (((int)blockIdx.z) & 7));
      }
    }
  }
}

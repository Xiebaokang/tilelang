...
extern "C" __global__ void __launch_bounds__(256, 1) main_kernel(__grid_constant__ const CUtensorMap B_desc, const half_t* __restrict__ DA, const half_t* __restrict__ Dt, half_t* __restrict__ Output, __grid_constant__ const CUtensorMap X_desc) {
  ...
  if (((int)threadIdx.x) < 128) {
    ...
    for (int ik = 0; ik < 7; ++ik) {
      overlap_plan_mbar[(ik % 3)].wait(((ik % 6) / 3));
      #pragma unroll
      for (int i_4 = 0; i_4 < 16; ++i_4) {
        x_local[i_4] = ((half_t*)x_shared)[(((((((((ik % 3) * 2048) + ((i_4 >> 2) * 512)) + ((((int)threadIdx.x) & 3) * 128)) + (((i_4 & 3) >> 1) * 64)) + ((((((int)threadIdx.x) >> 6) + ((((int)threadIdx.x) & 3) >> 1)) & 1) * 32)) + (((((((int)threadIdx.x) & 63) >> 5) + (((int)threadIdx.x) & 1)) & 1) * 16)) + (((((i_4 & 3) >> 1) + (i_4 & 1)) & 1) * 8)) + ((((int)threadIdx.x) & 31) >> 2))];
      }
      overlap_plan_mbar[((ik % 3) + 11)].arrive();
      #pragma unroll

    ...
    *(float2*)(scores_max + 0) = make_float2(broadcast_var_9, broadcast_var_9);
    ...
    overlap_plan_mbar[1].wait(1);
    {
      ...
      for (int ki_12 = 0; ki_12 < 8; ++ki_12) {
        tl::wgmma_ss<tl::DataType::kFloat16, tl::DataType::kFloat16, tl::DataType::kFloat32, 64, 64, 16, false, false, 1, 1>(uint64_t(desc_a_7 + (((((ki_12 >> 2) * 16384) + ((((int)threadIdx.x) >> 7) * 8192)) + ((ki_12 & 3) * 32)) >> 4)), uint64_t(desc_b_12 + ((((ki_12 >> 2) * 8192) + ((ki_12 & 3) * 32)) >> 4)), ((uint32_t*)(acc_s + 0)), 1);
      }
      tl::warpgroup_commit_batch();
      tl::warpgroup_wait<0>();
      tl::warpgroup_fence_operand(reinterpret_cast<float*>(acc_s + 0), 64);

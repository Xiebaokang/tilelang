/*!
 * \file lower_overlap_plan.cc
 * \brief Materialize overlap-plan groups in TIRX.
 */
...
public:
  static tirx::IterVar Collect(const tirx::Stmt &body) {
    ...
    ICHECK(collector.thread_x_.defined())
        << "LowerOverlapPlan requires a materialized threadIdx.x "
    ...
    if (op->attr_key == tirx::attr::thread_extent) {
      ...
      if (thread_tag == "threadIdx.x") {
        ICHECK(!thread_x_.defined())
            << "LowerOverlapPlan requires exactly one threadIdx.x extent";
        thread_x_ = iter_var.value();
      } else if (thread_tag.rfind("threadIdx.", 0) == 0) {
        const auto *extent = op->value.as<IntImmNode>();
        ICHECK(extent != nullptr && extent->value == 1)
            << "LowerOverlapPlan currently supports only a one-dimensional "
...
public:
  OperationGroupFilter(const OperationIdMap &operation_ids,
                       ...
                       const LoweringView &plan, int64_t group_id,
  ...
  tirx::Stmt AnnotatePipelineLoop(tirx::For loop, int64_t region_id) const {
    ...
    for (const auto &[key, value] : loop->annotations) {
      if (key != "num_stages" && key != "tl_pipeline_stage" &&
          ...
          key != "tl_pipeline_sync" && key != kRegionIdAnnotation) {
    ...
    for (const tirx::Stmt &statement : statements) {
      ...
      if (operation_ids.empty()) {
        ICHECK(IsPipelineDeclarationStmt(statement) ||
            ...
            << "pipeline region " << region_id << " group " << group_id_
            << " contains an executable statement without an OverlapPlan "
               "operation ID";
...
public:
  GroupBranchBuilder(const LoweringView &plan,
  ...
  tirx::Stmt Build(const tirx::Stmt &body) {
    ...
    for (size_t group_id = 0; group_id < plan_.group_warp_counts.size();
      ...
      if (plan_.setmaxnreg_enabled) {
        ICHECK_GE(plan_.group_register_counts[group_id], 0)
            << "missing register policy for group " << group_id;
        tirx::Stmt set_register_limit = tirx::Evaluate(tirx::Call(
            DataType::Handle(), set_max_nreg(),
            {IntImm(DataType::Int(32), plan_.group_register_counts[group_id]),
             IntImm(DataType::Int(32),
                    plan_.group_register_increase[group_id])}));
...
public:
  ThreadExtentRewriter(const LoweringView &plan,
                       const OverlapIR &program_ir,
  ...
  static tirx::Stmt Rewrite(const tirx::Stmt &body,
    ...
    ICHECK(rewriter.rewritten_)
        << "LowerOverlapPlan did not find the validated threadIdx.x extent";
    return result;
  }
...
class OverlapPlanMetadataCleaner : public tirx::StmtMutator {
private:
  tirx::Stmt VisitStmt_(const tirx::AttrStmtNode *op) final {
    if (op->attr_key == kOperationScope ||
        op->attr_key == kProgramScheduleGroupScope ||
        op->attr_key == kSyncEventScope) {
...
tirx::PrimFunc FinalizeOverlapPlanFunction(tirx::PrimFunc func) {
  if (GetOverlapPlan(func).defined()) {
    OverlapPlanMetadataCleaner cleaner;
    tirx::PrimFuncNode *node = func.CopyOnWrite();
    node->body = cleaner(node->body);
    func = tvm::WithoutAttr(std::move(func), ::tvm::tl::attr::kOverlapPlan);
  }
  if (HasAutoOverlap(func)) {
    func = tvm::WithoutAttr(std::move(func), ::tvm::tl::attr::kAutoOverlap);
  }
  return func;
}

tirx::PrimFunc LowerOverlapPlanGroups(
    tirx::PrimFunc func, const LoweringView &plan,
    const OverlapIR &program_ir) {
...
tvm::transform::Pass LowerOverlapPlan() {
  auto pass_func = [](tirx::PrimFunc func, const IRModule &,
                      tvm::transform::PassContext) {
    ffi::Optional<overlap_plan::OverlapPlan> plan =
        overlap_plan::GetOverlapPlan(func);
    if (!plan.defined()) {
      ICHECK(!overlap_plan::HasAutoOverlap(func))
          << "tl.auto_overlap requires a searched OverlapPlan; there is no "
             "default schedule";
      return func;
    }
    overlap_plan::OverlapIR program_ir =
        overlap_plan::CollectOverlapIR(func);
    program_ir =
        overlap_plan::MatchOverlapPlan(plan.value(), std::move(program_ir));
    overlap_plan::LoweringView view =
        overlap_plan::MakeLoweringView(plan.value(), program_ir);
    overlap_plan::ValidateLoweringView(view, program_ir);
    func = overlap_plan::AttachSharedMemoryAttrs(std::move(func), plan.value(),
                                                 program_ir);
    func = overlap_plan::LowerOverlapPlanGroups(std::move(func), view,
                                                program_ir);
    func = overlap_plan::LowerOverlapPlanBuffers(std::move(func), view,
                                                 program_ir);
    func = overlap_plan::LowerOverlapPlanSynchronization(std::move(func), view,
                                                         program_ir);
    overlap_plan::VerifyLoweredOverlapPlan(func, plan.value(), view,
                                           program_ir);
    return func;
  };
...
tvm::transform::Pass FinalizeOverlapPlan() {
  auto pass_func = [](tirx::PrimFunc func, const IRModule &,
                      tvm::transform::PassContext) {
    return overlap_plan::FinalizeOverlapPlanFunction(std::move(func));
  };
  return tirx::transform::CreatePrimFuncPass(pass_func, 0,
                                             "tl.FinalizeOverlapPlan", {});
}

TVM_FFI_STATIC_INIT_BLOCK() {
  namespace refl = tvm::ffi::reflection;
  refl::GlobalDef()
      .def("tl.transform.LowerOverlapPlan", LowerOverlapPlan)
      .def("tl.transform.FinalizeOverlapPlan", FinalizeOverlapPlan);
}

/*!
 * \file lower_overlap_synchronization.cc
 * \brief Materialize OverlapPlan synchronization channels.
 */
...
tirx::Stmt MarkSyncEvent(size_t channel, SyncEventRole role,
                         tirx::Stmt statement) {
  return tirx::AttrStmt(
      Integer(static_cast<int64_t>(channel)), kSyncEventScope,
      IntImm(DataType::Int(32), static_cast<int64_t>(role)),
      std::move(statement));
}
...
private:
  ProgramSynchronizationLowerer(const LoweringView &plan,
                                const OverlapIR &program_ir)
      : plan_(plan), program_ir_(program_ir) {
    layouts_.resize(plan_.sync_producers.size());
    int64_t total_slots = 0;
    for (size_t channel = 0; channel < plan_.sync_producers.size();
         ++channel) {
      size_t owner = static_cast<size_t>(plan_.sync_event_owners[channel]);
      ICHECK_LE(owner, channel);
      if (owner != channel) {
        layouts_[channel] = layouts_[owner];
        continue;
      }
      int64_t slots = 1;
      if (Scope(channel) == SynchronizationScope::kPerIteration) {
        ...
        if (!plan_.sync_slot_counts.empty()) {
          ...
          // Fall back to max(region stages, buffer versions) when the
          // optional exact slot-count array is absent.
      ...
      layouts_[channel] = ChannelLayout{total_slots, slots};
    ...
    barrier_ = CreateMBarrierBuffer("overlap_plan_mbar",
  ...
  SynchronizationScope Scope(size_t channel) const {
    ...
    ICHECK_GE(code, 0)
        << "OverlapPlan synchronization requires an explicit scope";
    ICHECK_LE(code, 2);
  ...
  tirx::Stmt MakeArrive(size_t channel) const {
    ...
    tirx::Stmt arrive = tirx::Evaluate(tirx::Call(
        ...
        {barrier_ref}));
    tirx::Stmt event = std::move(arrive);
    ...
    return MarkSyncEvent(channel, SyncEventRole::kThreadArrive,
                         std::move(event));
  }
  ...
  tirx::Stmt MakeWait(size_t channel) const {
    ...
    tirx::Stmt event = tirx::Evaluate(tirx::Call(
    ...
    if (Scope(channel) == SynchronizationScope::kRegionBoundary) {
      ...
      if (program_ir_.regions[consumer_region].pipeline_loop.has_value()) {
        ...
        event =
            tirx::IfThenElse(loop->loop_var == loop->min, std::move(event));
      }
    } else if (Scope(channel) == SynchronizationScope::kPerIteration &&
      ...
      event = tirx::IfThenElse(completed_iterations >= distance,
                               std::move(event));
    }
    return MarkSyncEvent(channel, SyncEventRole::kWait, std::move(event));
  }
  ...
  tirx::Stmt VisitStmt_(const tirx::AttrStmtNode *op) final {
    ...
    for (size_t channel = 0; channel < plan_.sync_producers.size();
      ...
      if (plan_.sync_producers[channel] == operation_id->value &&
          ...
          plan_.sync_event_owners[channel] == static_cast<int64_t>(channel)) {
    ...
    if (transaction_channel.has_value()) {
      ...
      body = MarkSyncEvent(channel, SyncEventRole::kAsyncArrive,
                           std::move(body));
    }
...
tirx::PrimFunc LowerOverlapPlanSynchronization(
    tirx::PrimFunc func, const LoweringView &plan,
    const OverlapIR &program_ir) {
  return ProgramSynchronizationLowerer::Lower(std::move(func), plan,
                                              program_ir);
}

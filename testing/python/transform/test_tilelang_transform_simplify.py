...
def test_floordiv_loop_extent_keeps_zero_or_one_symbolic_bound():
    """CTA-dependent 0-or-1 trip counts must not fold to the constant 1.

    ``blockIdx.y // 4`` with ``y`` in ``[0, 8)`` is 0 or 1.  Recording
    ``extent > 0`` before simplifying the extent used to make the FloorDiv
    folder treat that as the singleton 1, which extra-runs pipelined bodies.
    """
    ...
    loop = tvm.tirx.For(
        k,
        0,
        tvm.tirx.floordiv(by.var, 4),
        tvm.tirx.ForKind.SERIAL,
        tvm.tirx.Evaluate(0),
    )
    stmt = tvm.tirx.AttrStmt(by, "thread_extent", 8, loop)
    after = tl.transform.Simplify()(
        tvm.IRModule.from_expr(tvm.tirx.PrimFunc([], stmt))
    )["main"].body
    loop_stmt = after
    while isinstance(loop_stmt, tvm.tirx.AttrStmt):
        loop_stmt = loop_stmt.body
    assert isinstance(loop_stmt, tvm.tirx.For)
    assert not isinstance(loop_stmt.extent, tvm.tirx.IntImm)
    assert "//" in str(loop_stmt.extent)

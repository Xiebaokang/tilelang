...
def _mixed_buffer_pointer_alias_func(pointer_byte_offset: int):
    """Build direct-store + access_ptr accesses on shared.dyn aliases."""

    @T.prim_func(private=True)
    def func():
        storage = T.alloc_buffer((1024,), "uint8", scope="shared.dyn")
        direct_data: T.handle("float16", "shared.dyn") = T.handle_add_byte_offset(
            storage.data, 0
        )
        pointer_data: T.handle("float16", "shared.dyn") = T.handle_add_byte_offset(
            storage.data, pointer_byte_offset
        )
        direct = T.decl_buffer(
            (256,), "float16", data=direct_data, scope="shared.dyn"
        )
        tx = T.launch_thread("threadIdx.x", 128)
        ty = T.launch_thread("threadIdx.y", 1)
        tz = T.launch_thread("threadIdx.z", 1)
        direct[tx] = T.float16(0)
        T.evaluate(
            T.call_extern(
                "handle",
                "consume_pointer",
                T.tvm_access_ptr(
                    T.type_annotation("float16"), pointer_data, 0, 256, 1
                ),
            )
        )
...
def test_mixed_buffer_pointer_disjoint_aliases_need_no_sync():
    """Disjoint aliases remain disjoint across access representation kinds."""
    mod = tvm.IRModule({"main": _mixed_buffer_pointer_alias_func(512)})
    mod = tilelang.transform.ThreadSync("shared.dyn")(mod)
    s = str(mod.script())
    assert 'T.tvm_storage_sync("shared.dyn")' not in s, f"Unexpected sync:\n{s}"


def test_mixed_buffer_pointer_overlapping_aliases_keep_sync():
    """The mixed-access optimization must remain conservative on overlap."""
    mod = tvm.IRModule({"main": _mixed_buffer_pointer_alias_func(0)})
    mod = tilelang.transform.ThreadSync("shared.dyn")(mod)
    s = str(mod.script())
    assert 'T.tvm_storage_sync("shared.dyn")' in s, f"Expected sync:\n{s}"


def _async_tma_wgmma_func(
    wait_barrier_index: int = 0,
    wgmma_wait_count: int = 0,
    wait_before_wgmma: bool = False,
    tma_offset: int = 0,
    descriptor_byte_offset: int = 0,
    pointer_final_write: bool = False,
):
    """Build lowered-style TIR exposing TMA/WGMMA completion semantics."""

    @T.prim_func(private=True)
    def func():
        data = T.alloc_buffer((256,), "float16", scope="shared")
        barriers = T.alloc_buffer((2,), "uint64", scope="shared.barrier")
        descriptor = T.alloc_buffer((1,), "uint64", scope="local.descriptor.wgmma")
        accumulator = T.alloc_buffer((32,), "float32", scope="local")
        tx = T.launch_thread("threadIdx.x", 128)
        ty = T.launch_thread("threadIdx.y", 1)
        tz = T.launch_thread("threadIdx.z", 1)
        T.evaluate(
            T.call_intrin(
                "handle",
                tvm.tirx.op.Op.get("tl.tma_load"),
                T.uint64(0),
                barriers[0],
                T.tvm_access_ptr(
        ...
        T.evaluate(
            T.call_intrin(
                "handle",
                tvm.tirx.op.Op.get("tl.mbarrier_wait_parity"),
                barriers[wait_barrier_index],
                T.int32(0),
            )
        )
        T.evaluate(
            T.call_intrin(
                "handle",
                tvm.tirx.op.Op.get("tl.initialize_wgmma_descriptor"),
                descriptor[0],
                T.tvm_access_ptr(T.type_annotation("float16"), data.data, 0, 128, 1),
                T.int32(1),
                T.int32(1),
                T.int32(64),
            )
        ...
        T.evaluate(
            T.call_intrin(
                "handle",
                tvm.tirx.op.Op.get("tl.increase_descriptor_offset"),
                descriptor[0],
                T.int32(descriptor_byte_offset),
            )
        )
        if wait_before_wgmma:
            T.evaluate(
                T.call_intrin(
                    "handle",
                    tvm.tirx.op.Op.get("tl.warpgroup_wait"),
                    T.int32(0),
                )
            )
        T.evaluate(
            T.call_intrin(
                "handle",
                tvm.tirx.op.Op.get("tl.ptx_wgmma_ss"),
                "m64n64k16",
                T.bool(True),
                T.bool(True),
                "fp16",
        ...
        if not wait_before_wgmma:
            T.evaluate(
                T.call_intrin(
                    "handle",
                    tvm.tirx.op.Op.get("tl.warpgroup_wait"),
                    T.int32(wgmma_wait_count),
                )
            )
        if pointer_final_write:
            T.evaluate(
                T.tvm_access_ptr(
                    T.type_annotation("float16"),
                    data.data,
            ...
            data[descriptor_byte_offset // 2 + 127 - tx] = T.float16(0)

    return func


def test_tma_and_wgmma_completion_events_avoid_redundant_sync():
    """Matched waits complete async accesses without a CTA shared barrier."""
    mod = tvm.IRModule({"main": _async_tma_wgmma_func()})
    mod = tilelang.transform.ThreadSync("shared")(mod)
    s = str(mod.script())
    assert 'T.tvm_storage_sync("shared")' not in s, f"Unexpected sync:\n{s}"


def test_tma_wait_only_acquires_matching_barrier():
    """Waiting on another mbarrier must not complete the TMA write."""
    mod = tvm.IRModule({"main": _async_tma_wgmma_func(wait_barrier_index=1)})
    mod = tilelang.transform.ThreadSync("shared")(mod)
    s = str(mod.script())
    assert 'T.tvm_storage_sync("shared")' in s, f"Expected sync for mismatched barrier:\n{s}"


def test_nonzero_wgmma_wait_keeps_shared_read_live():
    """wait<N> may leave WGMMA work outstanding and stays conservative."""
    mod = tvm.IRModule({"main": _async_tma_wgmma_func(wgmma_wait_count=1)})
    mod = tilelang.transform.ThreadSync("shared")(mod)
    s = str(mod.script())
    assert 'T.tvm_storage_sync("shared")' in s, f"Expected sync for wait<1>:\n{s}"


def test_wait_before_wgmma_does_not_release_future_shared_read():
    """A wait only completes WGMMA operations issued before that wait."""
    mod = tvm.IRModule(
        {"main": _async_tma_wgmma_func(wait_before_wgmma=True)}
    )
    mod = tilelang.transform.ThreadSync("shared")(mod)
    s = str(mod.script())
    assert 'T.tvm_storage_sync("shared")' in s, f"Expected sync after future WGMMA:\n{s}"


def test_wgmma_descriptor_offset_avoids_false_tma_conflict():
    """A descriptor byte offset moves the WGMMA read to another buffer slot."""
    mod = tvm.IRModule(
        {
            "main": _async_tma_wgmma_func(
                wait_barrier_index=1,
                tma_offset=0,
                descriptor_byte_offset=256,
                pointer_final_write=True,
            )
        }
    )
    mod = tilelang.transform.ThreadSync("shared")(mod)
...
def test_loop_carried_tma_wait_acquires_next_iteration_slot():
    """A shifted wait acquires the TMA write issued by the prior iteration."""

    @T.prim_func(private=True)
    def func():
        data = T.alloc_buffer((2048,), "float16", scope="shared")
        barriers = T.alloc_buffer((2,), "uint64", scope="shared.barrier")
        descriptor = T.alloc_buffer((1,), "uint64", scope="local.descriptor.wgmma")
        accumulator = T.alloc_buffer((32,), "float32", scope="local")
        tx = T.launch_thread("threadIdx.x", 128)
        ...
        T.evaluate(
            T.call_intrin(
                ...
                T.uint64(0),
                barriers[0],
                T.tvm_access_ptr(T.type_annotation("float16"), data.data, 0, 1024, 2),
                T.int32(0),
                T.int32(0),
            )
        )
        for k in range(4):
            ...
            T.evaluate(
                T.call_intrin(
                    ...
                    descriptor[0],
                    T.tvm_access_ptr(
                        T.type_annotation("float16"), data.data, 0, 2048, 1
                    ),
                    T.int32(1),
                    T.int32(1),
                    T.int32(64),
    ...
    assert s.count(sync) == 1, f"Unexpected sync placement:\n{s}"
    assert s.index(sync) < s.index("for k in range(4)"), (
        f"A sync was inserted inside the TMA/WGMMA pipeline loop:\n{s}"
    )


def test_loop_carried_disjoint_pointer_writes_need_no_sync():
    """Pointer ranges from adjacent loop iterations can be proven disjoint."""

    @T.prim_func(private=True)
    def func():
        data = T.alloc_buffer((16384,), "float16", scope="shared")
        tx = T.launch_thread("threadIdx.x", 128)
        ty = T.launch_thread("threadIdx.y", 1)
        tz = T.launch_thread("threadIdx.z", 1)
        for i in range(8):
...
def test_loop_carried_disjoint_stmatrix_style_writes_need_no_sync():
    """Small tiled pointer-write loops should not gain a prologue barrier.

    This address is the lowered m128n128 stmatrix layout used by attention
    output stores.  Adjacent loop iterations write disjoint tiles, but the
    combination of floordiv and floormod is difficult to prove symbolically.
    """

    @T.prim_func(private=True)
    def func():
        data = T.alloc_buffer((16384,), "float16", scope="shared.dyn")
        ...
        for i in range(8):
            ...
            T.evaluate(
                T.call_extern(
                    "handle",
                    "stmatrix_style_write",
                    T.tvm_access_ptr(
                        T.type_annotation("float16"), data.data, offset, 8, 2
                    ),
                )
            )
...
def test_warp_specialized_cross_group_access_does_not_insert_partial_sync():
    """Cross-group communication is owned by the WS mbarrier protocol."""
    ...
    mod = tilelang.transform.ThreadSync("shared")(mod)
    s = str(mod.script())
    assert 'T.tvm_storage_sync("shared")' not in s
    assert 'T.tvm_storage_sync("shared",' not in s


def test_warp_specialized_same_group_access_keeps_partial_sync():
    """SIMT shared writes and cross-thread reads in one group still synchronize."""
    ...
    mod = tilelang.transform.ThreadSync("shared")(mod)
    s = str(mod.script())
    assert 'T.tvm_storage_sync("shared",' in s
    assert ", 128)" in s

...
def test_explicit_pipeline_plan_propagates_tma_copy_metadata():
    @T.prim_func
    def before(A: T.Tensor((64,), T.float16), C: T.Tensor((64,), T.float16)):
        S = T.alloc_buffer((16,), dtype=T.float16, scope="shared")
        for i in T.Pipelined(4, order=[0, 1], stage=[0, 1]):
            T.copy(A[i * 16], S)
            T.copy(S, C[i * 16])
    ...
    annos = _collect_pipeline_loop_annotations(mod["main"])
    assert len(annos) == 1
    assert [int(v) for v in annos[0]["software_pipeline_tma_copies"]] == [1, 0]

    mod = tl.transform.InjectSoftwarePipeline()(mod)
    script = mod["main"].script()
    assert "T.tma_copy" in script
    assert "T.mbarrier_wait_parity" in script
    assert "T.ptx_commit_group" not in script
    assert "T.ptx_wait_group" not in script


def test_explicit_tma_pipeline_uses_independent_dataflow_barriers():
    @T.prim_func
    def before(
        A: T.Tensor((64,), T.float16),
        B: T.Tensor((64,), T.float16),
        C: T.Tensor((4,), T.float16),
        D: T.Tensor((4,), T.float16),
    ):
        A_shared = T.alloc_buffer((16,), dtype=T.float16, scope="shared")
        B_shared = T.alloc_buffer((16,), dtype=T.float16, scope="shared")
        for i in T.Pipelined(
            4,
            order=[0, 1, 2, 3],
            stage=[0, 0, 0, 1],
        ):
            T.copy(A[i * 16], A_shared)
            T.copy(B[i * 16], B_shared)
            C[i] = A_shared[0]
            D[i] = B_shared[0]
    ...
    assert len(annos) == 1
    anno = annos[0]
    assert [int(v) for v in anno["software_pipeline_tma_copies"]] == [
        1,
        1,
        0,
        0,
    ...
    mod = tl.transform.InjectSoftwarePipeline()(mod)
    script = mod["main"].script()
    assert 'pipeline_mbar_1 = T.sblock_alloc_buffer((4,), "uint64"' in script
    assert script.count("T.tma_copy") >= 2
    assert script.count("T.mbarrier_wait_parity") >= 2
    assert "T.ptx_commit_group" not in script
    assert "T.ptx_wait_group" not in script

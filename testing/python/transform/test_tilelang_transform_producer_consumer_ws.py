...
def _assert_shared_prelude_syncs_before_ws_split(src: str) -> None:
    """Producer warps help write shared prelude, then leave the consumer path."""
    branch = _find_after(src, "if (((int)threadIdx.x) < 128)")
    last_sync = src.rfind("__syncthreads()", 0, branch)
    assert last_sync >= 0, "missing __syncthreads before WS split"
    copy_markers = (
        "*(uint4*)(((half_t*)",
        "*(uint2*)(((half_t*)",
        "*(uint4*)((half_t*)",
    ...
    copy_to_shared = max(src.rfind(marker, 0, branch) for marker in copy_markers)
    assert copy_to_shared >= 0, "expected a SIMT shared prelude copy before the WS split"
    assert last_sync > copy_to_shared, "shared prelude copies must syncthreads before the WS split"
...
def test_tiled_ws_dual_gemm_shared_accumulator_correctness():
    ...
    _assert_shared_prelude_syncs_before_ws_split(kernel.get_kernel_source())
...
@tilelang.testing.requires_cuda
@tilelang.testing.requires_cuda_compute_version(9, 0)
def test_tiled_ws_shared_prelude_copy_visible_across_waves():
    """SIMT prelude copies must be visible to consumer warps on later CTA waves.

    A 1-CTA dual GEMM can pass by luck: producer stores drain before WGMMA.
    Once the grid exceeds occupancy, consumer WGMMA races on those buffers.
    """
    import torch

    M, N, K0, K1 = 2048, 2048, 64, 128
    block_M, block_N, block_K = 64, 64, 64
    func = dual_gemm_shared_accumulator(M, N, K0, K1, block_M, block_N, block_K, num_stages=2)
    target = determine_target()
    kernel = tilelang.compile(func, target=target, out_idx=[4])
...
if __name__ == "__main__":
    ...
    test_tiled_ws_shared_prelude_copy_visible_across_waves()
